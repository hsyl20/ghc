...
GHC.Rename.Environment contains functions which convert RdrNames into Names.

-}

{-# LANGUAGE CPP, MultiWayIf, NamedFieldPuns #-}

module GHC.Rename.Environment (
        newTopSrcBinder,
...
Looking up a name in the GHC.Rename.Environment.
...
lookupFamInstName (Just cls) tc_rdr  -- Associated type; c.f GHC.Rename.Binding.rnMethodBind
 ...
 * In GHC.Rename.Environment.newTopSrcBinder we spot Exact RdrNames that wrap a
   non-External Name, and make an External name for it. This is
   the name that goes in the GlobalRdrEnv

 * When looking up an occurrence of an Exact name, done in
   GHC.Rename.Environment.lookupExactOcc, we find the Name with the right unique in the
 ...
 * Exact names are also use for purely local binders generated
   ...
   gets looked up in the LocalRdrEnv by GHC.Rename.Environment.lookupOccRn, and
...
-- See Note [Renaming pattern synonym variables] in GHC.Rename.Binding
