...
import GHC.Compiler.StgToCmm.Profiling ( saveCurrentCostCentre,
...
import GHC.IR.Cmm.Syntax.Graph
import GHC.IR.Cmm.Syntax.BlockId
...
import GHC.Entity.Type             ( isUnliftedType )
import GHC.Entity.RepType          ( isVoidTy, countConRepArgs, primRepSlot )
import GHC.Entity.CostCentre       ( CostCentreStack, currentCCS )
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
If shouldInlinePrimOp returns True when called from
GHC.Compiler.StgToCmm.Expression for the purpose of heap check placement, we
*must* inline the primop later in GHC.Compiler.StgToCmm.PrimOp. If we don't
...
-- scrutinee.
-- They're non-void, see Note [Post-unarisation invariants] in
    ...
    cg_alt (con, bndrs, rhs)
      = getCodeScoped             $
        ...
        do { _ <- bindConArgs con base_reg (assertNonVoidIds bndrs)
                    ...
                    -- see Note [Post-unarisation invariants] in
                    -- GHC.IR.Stg.Transformer.Unariser
           ; _ <- cgExpr rhs
           ; return con }
...
cgConApp con stg_args
  ...
  = ASSERT2( stg_args `lengthIs` countConRepArgs con, ppr con <> parens (ppr (countConRepArgs con)) <+> ppr stg_args )
    do  { (idinfo, fcode_init) <- buildDynCon (dataConWorkId con) False
                                     ...
                                     -- in GHC.IR.Stg.Transformer.Unariser
