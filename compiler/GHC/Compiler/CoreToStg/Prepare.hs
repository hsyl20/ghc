...
module GHC.Compiler.CoreToStg.Prepare (
...
import GHC.IR.Core.Analyser.Occurence

import GHC.Entity.Types
import GHC.Builtin.Names
import GHC.Entity.Id.Make             ( realWorldPrimId )
...
import GHC.IR.Core.Analyser.Lint         ( endPassIO )
import GHC.IR.Core.Syntax
import GHC.IR.Core.Transformer.Substitution
import GHC.IR.Core.Syntax.Make hiding( FloatBind(..) )   -- We use our own FloatBind here
...
import GHC.Entity.Name             ( NamedThing(..), nameSrcSpan )
import GHC.Entity.SrcLoc           ( SrcSpan(..), realSrcLocSpan, mkRealSrcLoc )
import Data.Bits
...
9.  Replace (lazy e) by e.  See Note [lazyId magic] in GHC.Entity.Id.Make
...
-- c.f. Note [Injecting implicit bindings] in GHC.IR.Interface.Transformer.Tidier
...
************************************************************************
*                                                                      *
                Simple GHC.IR.Core.Syntax operations
*                                                                      *
************************************************************************
-}
...
Instead GHC.IR.Core.Analyser.Arity.etaExpand gives
                f = /\a -> \y -> let s = h 3 in g s y
-}
