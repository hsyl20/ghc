{-
  ...
  + GHC.Compiler.CmmToAsm
    - The cmmToCmm pass in GHC.Compiler.CmmToAsm calls cmmMakeDynamicReference
      for all labels.
    - nativeCodeGen calls pprImportedSymbol and pprGotDeclaration to output
      all the necessary stuff for imported symbols.
    - The CmmToAsm monad keeps track of a list of imported symbols.
...
import GHC.Types.CLabel ( CLabel, ForeignLabelSource(..), pprCLabel,
                           mkDynamicLinkerLabel, DynamicLinkerLabelInfo(..),
                           dynamicLinkerLabelInfo, mkPicBaseLabel,
                           labelDynamic, externallyVisibleCLabel )

import GHC.Types.CLabel ( mkForeignLabel )
...
-- When needImportedSymbols is defined, the CmmToAsm compiler will keep track of
-- all DynamicLinkerLabels it uses and output each of them using
-- pprImportedSymbol.
