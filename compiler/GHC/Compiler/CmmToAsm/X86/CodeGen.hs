...
import GHC.IR.Cmm.Syntax.DebugBlock           ( DebugBlock(..), UnwindPoint(..), UnwindTable
...
import GHC.Entity.Module           ( primUnitId )
import GHC.IR.Cmm.Printer           ()
import GHC.IR.Cmm.Utils
import GHC.IR.Cmm.Transformer.Switch
import GHC.IR.Cmm.Syntax
import GHC.IR.Cmm.Transformer.Dataflow
import GHC.Entity.Symbol
import GHC.IR.Core.Syntax          ( Tickish(..) )
import GHC.Entity.SrcLoc           ( srcSpanFile, srcSpanStartLine, srcSpanStartCol )

-- The rest:
import GHC.Entity.ForeignCall      ( CCallConv(..) )
...
import GHC.Utils
import GHC.Entity.Unique.Supply       ( getUniqueM )
  ...
  = do
        ...
        return $ if is32bit || os == OSDarwin
                 then e_code `appOL` t_code `appOL` toOL [
                      ...
                      -- GHC.Compiler.CmmToAsm.X86.Ppr/pprDataItem once binutils
                      -- 2.17 is standard.
