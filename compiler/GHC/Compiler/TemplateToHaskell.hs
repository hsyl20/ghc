...
This module converts Template Haskell syntax into GHC.Haskell
-}

{-# LANGUAGE FlexibleContexts #-}
{-# LANGUAGE ScopedTypeVariables #-}

module GHC.Compiler.TemplateToHaskell
   ( convertToHsExpr
   , convertToPat
   , convertToHsDecls
   , convertToHsType
...
import qualified GHC.Types.Class as Class
import GHC.Types.RdrName
import qualified GHC.Types.Name as Name
import GHC.Types.Module
import GHC.Haskell.Parser.Syntax
import qualified GHC.Types.OccName as OccName
import GHC.Types.OccName
import GHC.Types.SrcLoc
import GHC.Types.Type
import qualified GHC.Types.Coercion as Coercion
import GHC.Builtin.Types
import GHC.Builtin.Primitive.Types (eqPrimTyCon)
import GHC.Types.BasicTypes as Hs
...
cvtDec (ClassD ctxt cl tvs fds decs)
  ...
  where
    ...
    -- Very similar to what happens in GHC.Haskell.Parser.Syntax.mkClassDecl
    cvt_at_def decl = case GHC.Haskell.Parser.Syntax.mkATDefault decl of
...
cvtForD (ImportF callconv safety from nm ty)
  ...
  -- and are inserted verbatim, analogous to mkImport in
...
@mkHsOpTyRn@ in GHC.Haskell.Rename.Type), which expects that the input will be completely
...
cvtLit _ = panic "TemplateToHaskell.cvtLit: Unexpected literal"
        -- cvtLit should not be called on IntegerL, RationalL
        -- That precondition is established right here in
        -- GHC.Compiler.TemplateToHaskell, hence panic
...
When we convert all this to GHC.Haskell, the TH.Names are converted with
...
RdrNames] in GHC.Haskell.Rename.Environment.
-}
...
specially in GHC.Compiler.HaskellToCore.Splice,
GHC.Compiler.TemplateToHaskell, and GHC.Haskell.TypeCheck.Splice:

   (a) When desugaring a pattern synonym from GHC.Haskell to TH.Dec in
       GHC.Compiler.HaskellToCore.Splice, we represent its *full* type
       signature in TH, i.e.:

           ForallT univs reqs (ForallT exis provs ty)
              (where ty is the AST representation of t1 -> t2 -> ... -> tn -> t)

   (b) When converting pattern synonyms from TH.Dec to GHC.Haskell in
       GHC.Compiler.TemplateToHaskell, we convert their TH type
