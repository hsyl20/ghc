...
module GHC.Compiler.HaskellToCore.Coverage (addTicksToBinds, hpcInitCode) where
...
import GHC.Entity.Module as Module
...
import GHC.Entity.Name.Set hiding (FreeVars)
...
-- Note [inline sccs]
--
-- It should be reasonable to add ticks to INLINE functions; however currently
-- this tickles a bug later on because the
-- GHC.IR.Stg.Transformer.CostCentreCollector pass does not look inside
-- unfoldings to find CostCentres.  It would be difficult to fix that, because
-- GHC.IR.Stg.Transformer.CostCentreCollector currently works on STG and not
-- Core (and since it also generates CostCentres for CAFs, changing this would
-- be difficult too).
--
