...
-- currently this tickles a bug later on because the GHC.IR.Stg.Transformer.CostCentreCollecter pass
-- does not look inside unfoldings to find CostCentres.  It would be
-- difficult to fix that, because GHC.IR.Stg.Transformer.CostCentreCollecter currently works on STG and
