...
Pattern-matching bindings (GHC.IR.Haskell.Syntax.Binding and MonoBinds)

Handles @GHC.IR.Haskell.Syntax.Binding@; those at the top level require different handling,
...
module GHC.Compiler.HaskellToCore.Binding
   ( dsTopLHsBinds
   , dsLHsBinds
   , decomposeRuleLhs
   , dsSpec
...
import {-# SOURCE #-}   GHC.Compiler.HaskellToCore.Expression( dsLExpr )
import {-# SOURCE #-}   GHC.Compiler.HaskellToCore.Match( matchWrapper )
...
import GHC.Entity.Literal             ( Literal(MachStr) )
import GHC.IR.Core.Transformer.Simple ( simpleOptExpr )
import GHC.IR.Core.Analyser.Occurence ( occurAnalyseExpr )
import GHC.IR.Core.Syntax.Make
import GHC.IR.Core.Utils
import GHC.IR.Core.Analyser.Arity     ( etaExpand )
...
import GHC.Entity.Id.Make(proxyHashId)
...
dsHsBind dflags
  ...
  = -- See Note [AbsBinds wrappers] in GHC.IR.Haskell.Syntax.Binding
...
--
-- Other decisions about whether to inline are made in `calcUnfoldingGuidance`
-- but the decision about whether to then expose the unfolding in the interface
-- file is made in `GHC.IR.Interface.Transformer.Tidier.addExternal` using this
-- information.
------------------------
...
Instead we want to generate
        M.f = ...f_lcl...
        f_lcl = M.f
Now all is cool. The RULES are attached to M.f (by GHC.IR.Core.Transformer.Pipeline),
...
In order to force the Ids in the binding group they are passed around in the
dsHsBind family of functions, and later seq'ed in
GHC.Compiler.HaskellToCore.Expression.ds_val_bind.
...
GHC.IR.Haskell.Utils.isUnliftedHsBind.
...
The restrictions are:
  ...
  2. Unlifted binds must also be banged. (There is no trouble to compile an unbanged
     ...
     of GHC.Compiler.HaskellToCore.Expression.ds_val_bind.
...
is an unbound meta-tyvar.  The zonker in GHC.IR.Haskell.TypeChecker.Syntax is careful not to
turn the free alpha into Any (as it usually does).  Instead it turns it
into a TyVar 'a'.  See GHC.IR.Haskell.TypeChecker.Syntax Note [Zonking the LHS of a RULE].
...
dsHsWrapper (WpCompose c1 c2) = do { w1 <- dsHsWrapper c1
                                   ; w2 <- dsHsWrapper c2
                                   ; return (w1 . w2) }
 -- See comments on WpFun in GHC.IR.Haskell.TypeChecker.Evidence for an explanation of what
