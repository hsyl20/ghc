...
Pattern-matching bindings (GHC.Haskell.Syntax.Binding and MonoBinds)

Handles @GHC.Haskell.Syntax.Binding@; those at the top level require different handling,
...
import GHC.Types.Literal             ( Literal(MachStr) )
...
dsHsBind dflags
  ...
  = -- See Note [AbsBinds wrappers] in GHC.Haskell.Syntax.Binding
...
-- file is made in `GHC.Interface.Tidy.addExternal` using this
-- information.
------------------------
...
Instead we want to generate
        M.f = ...f_lcl...
        f_lcl = M.f
Now all is cool. The RULES are attached to M.f (by GHC.Core.Monad),
...
-- See Note [Handling SPECIALISE pragmas] in GHC.Haskell.TypeCheck.Binding
...
See Note [Rules for seq] in GHC.Types.Id.Make for the details.
