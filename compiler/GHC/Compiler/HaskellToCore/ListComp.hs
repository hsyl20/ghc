...
module GHC.Compiler.HaskellToCore.ListComp
   ( dsListComp
   , dsPArrComp
   , dsMonadComp
   )
...
import {-# SOURCE #-} GHC.Compiler.HaskellToCore.Expression ( dsExpr, dsLExpr
...
dePArrComp (LetStmt lds@(L _ ds) : qs) pa cea = do
    ...
    let projBody = mkCoreLet (NonRec let'v clet) $
        ...
        errMsg   = text "dePArrComp: internal error!"
...
dePArrComp (ParStmt {} : _) _ _ =
  panic "dePArrComp: malformed comprehension AST: ParStmt"
dePArrComp (TransStmt {} : _) _ _ = panic "dePArrComp: TransStmt"
dePArrComp (RecStmt   {} : _) _ _ = panic "dePArrComp: RecStmt"
dePArrComp (ApplicativeStmt   {} : _) _ _ =
  panic "dePArrComp: ApplicativeStmt"
...
dePArrParComp qss quals = do
  ...
  where
    deParStmt []             =
      ...
      panic "dePArrComp: Empty parallel list comprehension"
...
parrElemType e  =
  case splitTyConApp_maybe (exprType e) of
    Just (tycon, [ty])
      | tycon == parrTyCon -> ty
    _                      -> panic "parrElemType: not a parallel array type"
