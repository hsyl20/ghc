...
import GHC.IR.Haskell.TypeChecker.MutableType ( checkForLevPolyX
                                              , formatLevPolyErr )
...
mkDsEnvs dflags mod rdr_env type_env fam_inst_env msg_var pmvar
  ...
  = let if_genv = IfGblEnv { if_doc       = text "mkDsEnvs",
        ...
        gbl_env = DsGblEnv { ds_mod     = mod
                           ...
                           , ds_parr_bi = panic "mkDsEnvs: uninitialised ds_parr_bi"
...
but we can't, because of the fixM in GHC.Compiler.HaskellToCore.Arrow. It's all
OK, though: Core Lint will catch an error here.
...
dsLoadModule doc mod
  = do { env    <- getGblEnv
       ...
       ; case iface of
           Failed err      -> pprPanic "dsLoadModule: failed to load" (err $$ doc)
