...
--
-- It also defines a bunch of knownKeyNames, in the same way as is done in
-- GHC.Builtin.Names.  It's much more convenient to do it here, because
-- otherwise we have to recompile GHC.Builtin.Names whenever we add a Name,
-- which is a Royal Pain (triggers other recompilation).
-----------------------------------------------------------------------------

module GHC.Compiler.HaskellToCore.Splice ( dsBracket ) where
...
-- To avoid clashes with GHC.Compiler.HaskellToCore.Splice.varName we must
-- make a local alias for OccName.varName. We do this by removing varName from
-- the import of GHC.Types.Name below and by using a qualified import of OccName.
import qualified GHC.Types.OccName as OccName

import GHC.Types.Module
import GHC.Types.Id
import GHC.Types.Name hiding( isVarOcc, isTcOcc, varName, tcName )
...
-- see Note [Pattern synonym type signatures and Template Haskell] in
...
globalVar name
  ...
  where
      ...
      mk_varg | OccName.isDataOcc name_occ = mkNameG_dName
              ...
              | otherwise                  =
                  pprPanic "HaskellToCore.Splices.globalVar" (ppr name)
