...
module GHC.Compiler.HaskellToCore.Match.Literal
   ( dsLit, dsOverLit, dsOverLit', hsLitKey
   , tidyLitPat, tidyNPat
...
import GHC.IR.Haskell.TypeChecker.Syntax ( shortCutLit )
...
*                                                                      *
                Desugaring literals
  [used to be in GHC.Compiler.HaskellToCore.Expression, but
  GHC.Compiler.HaskellToCore.Splices needs it, and it's nice to avoid a loop]
*                                                                      *
************************************************************************
...
tidyNPat tidy_lit_pat (OverLit val False _ ty) mb_neg _eq outer_ty
  ...
  | not type_change, isStringTy ty, Just str_lit <- mb_str_lit
     ...
     -- Trac #9238 and Note [Rules for floating-point comparisons] in
