...
module GHC.Compiler.HaskellToCore.Match.Check (
...
import GHC.Compiler.HaskellToCore.Match.TermEqOracle
...
import GHC.Entity.Types (CompleteMatch(..))

import GHC.Compiler.HaskellToCore.Monad
import GHC.IR.Haskell.TypeChecker.Constraint.Simplifier (tcCheckSatisfiability)
import GHC.IR.Haskell.TypeChecker.Type (toTcType, isStringTy, isIntTy, isWordTy)
import GHC.Data.Bag
import GHC.Utils.Error
import GHC.Entity.Var                  (EvVar)
import GHC.Entity.Type
import GHC.Entity.Unique.Supply
import GHC.Compiler.HaskellToCore.GuardedRHS (isTrueLHsExpr)
...
1. We normalise the outermost type family redex, data family redex or newtype,
   using pmTopNormaliseType_maybe (in GHC.Entity.FamilyInstance). This computes 3
   things:
...
-- | Translate an overloaded literal
-- (see `tidyNPat' in GHC.Compiler.HaskellToCore.Match.Literal)
...
in GHC.Compiler.HaskellToCore.Match.Expr.
...
in GHC.IR.Haskell.Syntax.Pattern. This handles bug #4139 ( see example
...
2. The term oracle (`tmOracle` in GHC.Compiler.HaskellToCore.Match.TermEqOracle) uses
