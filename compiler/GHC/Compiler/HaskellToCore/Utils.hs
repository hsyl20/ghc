...
module GHC.Compiler.HaskellToCore.Utils (
        EquationInfo(..),
        firstPat, shiftEqns,
...
import {-# SOURCE #-} GHC.Compiler.HaskellToCore.Match      ( matchSimply )
import {-# SOURCE #-} GHC.Compiler.HaskellToCore.Expression ( dsLExpr )

import GHC.IR.Haskell.Syntax
import GHC.IR.Haskell.TypeChecker.Syntax
import GHC.IR.Haskell.TypeChecker.Type( tcSplitTyConApp )
...
import GHC.Entity.Name( isInternalName )
...
In fact, even GHC.IR.Core.Transformer.Substitution.simplOptExpr will do this,
and simpleOptExpr runs on the output of the desugarer, so all is well by the end
of the desugaring pass.
  ...
  where
    ...
    isPArrFakeAlts (alt:alts) =
      case (isPArrFakeCon (alt_pat alt), isPArrFakeAlts alts) of
        (True , True ) -> True
        (False, False) -> False
        _              -> panic "isPArrFakeAlts: you may not mix `[:...:]' with `PArr' patterns"
    isPArrFakeAlts [] = panic "isPArrFakeAlts: unexpectedly found an empty list of PArr fake alternatives"
...
mkPArrCase dflags var ty sorted_alts fail = do
  ...
  where
    elemTy      = case splitTyConApp (idType var) of
        (_, [elemTy]) -> elemTy
        _             -> panic panicMsg
    panicMsg    = "mkCoAlgCaseMatchResult: not a parallel array?"
...
The [Core syntax let/app invariant] means that, other things being equal,
...
mkCoreAppDs s fun arg = mkCoreApp s fun arg
                        -- The rest is done in GHC.IR.Core.Syntax.Make
...
mkSelectorBinds :: [[Tickish Id]] -- ^ ticks to add, possibly
                ...
                -- binds (see Note [Desugar Strict binds] in
                -- GHC.Compiler.HaskellToCore.Binding) and all the desugared
                -- binds
