...
module GHC.Compiler.CoreToStg ( coreToStg, coreExprToStg ) where
...
import GHC.Entity.Demand          ( isUsedOnce )
...
coreToStgExpr (Case scrut _ _ [])
  = coreToStgExpr scrut
    ...
    -- If the case alternatives are empty, the scrutinee must diverge or raise
    -- an exception, so we can just dive into it.
    --
  ...
  where
    vars_alt (con, binders, rhs)
      ...
      = -- This case is a bit smelly.
        -- See Note [Nullary unboxed tuple] in GHC.Entity.Type
...
minusFVBinder v fv = fv `delVarEnv` v
        ...
        -- c.f. GHC.IR.Core.Analyser.FreeVars.delBinderFV
