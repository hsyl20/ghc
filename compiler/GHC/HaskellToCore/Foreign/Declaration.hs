...
Desugaring foreign declarations (see also GHC.HaskellToCore.Foreign.Call).
-}
...
\end{verbatim}
so we reuse the desugaring code in @GHC.HaskellToCore.Foreign.Call@ to deal with these.
-}
...
typeTyCon ty
  ...
  = tc
  | otherwise
  = pprPanic "GHC.HaskellToCore.Foreign.Declaration.typeTyCon" (ppr ty)
