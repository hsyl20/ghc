...
module GHC.HaskellToCore.Match.Check (
...
import GHC.HaskellToCore.GuardedRHS (isTrueLHsExpr)
...
import GHC.Util.Monad.ListT (ListT(..), fold, select)
...
1. We normalise the outermost type family redex, data family redex or newtype,
   using pmTopNormaliseType_maybe (in GHC.CoreTypes.FamilyInstance). This computes 3
   things:
   ...
   in GHC.CoreTypes.FamilyInstance
...
-- (see `tidyNPat' in GHC.HaskellToCore.Match.Literal)
...
in GHC.HaskellToCore.Match.Expression.
...
2. The term oracle (`tmOracle` in GHC.HaskellToCore.Match.TermEqOracle) uses
