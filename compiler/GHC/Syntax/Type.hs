...
import GHC.Syntax.PlaceHolder ( PlaceHolder(..) )
...
* The explicit presence of these wrappers specifies, in the
  GHC.Syntax, exactly where implicit quantification is allowed, and
...
data HsImplicitBndrs pass thing   -- See Note [HsType binders]
  = HsIB { hsib_vars :: PostRn pass [Name] -- Implicitly-bound kind & type vars
         ...
         , hsib_closed :: PostRn pass Bool 
          ...
          -- GHC.TypeCheck.Type.decideKindGeneralisationPlan
    }
...
data HsTyVarBndr pass
  = UserTyVar        -- no explicit kinding
         (Located (IdP pass))
        -- See Note [Located RdrNames] in GHC.Syntax.Expression
  | KindedTyVar
         (Located (IdP pass))
...
-- occurrences).
--
-- See Note [HsRecField and HsRecUpdField] in GHC.Syntax.Pattern and
-- Note [Disambiguating record fields] in GHC.TypeCheck.Expression.
-- See Note [Located RdrNames] in GHC.Syntax.Expression
