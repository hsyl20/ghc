...
--
-- * 'Var' is a synonym for the 'Id.Id' type but it may additionally potentially
-- contain type variables, which have a 'GHC.Entity.TypeAndCoercion.Kind' rather
-- than a 'GHC.Entity.TypeAndCoercion.Type' and only contain some extra details
-- during typechecking.
--
--   These 'Var' names may either be global or local, see "Var#globalvslocal"
--
-- #globalvslocal#
...
import {-# SOURCE #-} GHC.Entity.TypeAndCoercion( Type, Kind, pprKind )
import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Type( TcTyVarDetails, 
                                                      pprTcTyVarDetails,
                                                      vanillaSkolemTv )
import {-# SOURCE #-} GHC.Entity.Id.Info ( IdDetails, IdInfo, coVarDetails,
                                         isCoVarDetails, vanillaIdInfo,
                                         pprIdDetails )

import GHC.Entity.Name hiding (varName)
import GHC.Entity.Unique ( Uniquable, Unique, getKey, getUnique
...
-- large number of SOURCE imports of GHC.Entity.Id :-(
-}
...
-- See Note [TyBinders and ArgFlags] in GHC.Entity.TypeAndCoercion
...
-- home in GHC.Entity.TypeAndCoercion, because it's used in
-- GHC.Entity.DataConstructor .hs-boot file
...
mkTcTyVar name kind details
  = -- NB: 'kind' may be a coercion kind; cf, 'GHC.IR.Haskell.TypeChecker.MutableType.newMetaCoVar'
...
-- Ids, because GHC.Entity.Id uses 'mkGlobalId' etc with different types
