...
--
-- * 'Id' represents names that not only have a 'Name.Name' but also a
-- 'GHC.Entity.TypeAndCoercion.Type' and some additional details (a
-- 'IdInfo.IdInfo' and one of 'Var.LocalIdDetails' or 'IdInfo.GlobalIdDetails')
-- that are added, modified and inspected by various compiler passes. These
-- 'Var.Var' names may either be global or local, see "Var#globalvslocal"
--
...
module GHC.Entity.Id (
        ...
        -- ** Modifying an Id
        setIdName, setIdUnique, GHC.Entity.Id.setIdType,
...
import GHC.Entity.Var( Id, CoVar, DictId, JoinId,
            InId,  InVar,
            OutId, OutVar,
            idInfo, idDetails, setIdDetails, globaliseId, varType,
            isId, isLocalId, isGlobalId, isExportedId )
import qualified GHC.Entity.Var as Var
 ...
 * We want to treat them as free variables for the purpose of
   dependency analysis (e.g. GHC.IR.Core.Analyser.FreeVars.exprFreeVars).
...
That is what is happening in, say tidy_insts in GHC.IR.Interface.Transformer.Tidier.
...
isJoinId :: Var -> Bool
-- It is convenient in GHC.IR.Core.Transformer.LevelSetter.lvlMFE to apply isJoinId
...
-- See Note [Specialisations and RULES in IdInfo] in GHC.Entity.Id.Info
...
-- See Note [The state-transformer hack] in GHC.IR.Core.Analyser.Arity
...
This transfer is used in two places:
        GHC.IR.Core.Transformer.FloatOut (long-distance let-floating)
        GHC.IR.Core.Transformer.Simplifier.Utils.abstractFloats (short-distance let-floating)
