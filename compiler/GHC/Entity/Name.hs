...
import {-# SOURCE #-} GHC.Entity.TypeAndCoercion( TyThing )
...
-- (see Note [The Name Cache] in GHC.IR.Interface.Environment), so don't just
-- call mkExternalName with some fresh unique without populating the Name Cache
...
-- 'GHC.IR.Interface.Syntax.putIfaceTopBndr' and
-- 'GHC.IR.Interface.Syntax.getIfaceTopBndr' for serializing binding 'Name's.
-- See 'UserData' for the rationale for this distinction.
...
-- See Note [Printing original names] in GHC.Entity.Types
...
This pprPrefixName stuff is really only used when printing GHC.IR.Haskell,
...
type being pretty printed; eg the pretty-printing code in GHC.Entity.TypeAndCoercion.
