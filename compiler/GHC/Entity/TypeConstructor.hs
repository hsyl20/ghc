...
import {-# SOURCE #-} GHC.Entity.DataConstructor
                                 ( DataCon, dataConExTyVars
...
* The tyConTyVars of the representation tycon are the tyvars that the
  ...
  GHC.IR.Haskell.TypeChecker.Deriving, where we bring these tyvars into scope
...
GHC.IR.Haskell.TypeChecker.TypeDeclaration), as do vanilla synonym tycons.
...
at role N. Promoted data constructors' type arguments are at role R. All kind
arguments are at role N.
...
data TyCon
  ...
  | FamilyTyCon {
        ...
        tyConArity   :: Arity,            -- ^ Arity
            ...
            -- GHC.IR.Haskell.TypeChecker.Validity.checkConsistentFamInst
...
2.  When checking a type/class declaration (in module
    GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration), we come upon knowledge
    ...
    shape of the tycon's kind. Then, using these initial kinds, we kind-check
    the body of the tycon (class methods, data constructors, etc.), filling in
    the metavariables in the tycon's initial kind.  We then generalize to get
    the tycon's final, fixed kind. Finally, once this has happened for all
    tycons in a mutually recursive group, we can desugar the lot.
    ...
    GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration, until the real full
    ...
    GHC.IR.Haskell.TypeChecker.UserType.splitTelescopeTvs!) Instead of trying,
    ...
    in GHC.IR.Haskell.TypeChecker.UserType.
...
-- corresponding 'TyCon'. It is recomended to use
-- 'GHC.Entity.TypeAndCoercion.funTyCon' if you want this functionality
