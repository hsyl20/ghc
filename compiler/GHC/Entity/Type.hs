...
-- We import the representation and primitive functions from
...
import GHC.Entity.Unique  ( nonDetCmpUnique )
import GHC.Entity.SrcLoc  ( SrcSpan )
import GHC.Entity.OccName ( OccName )
import GHC.Entity.Name    ( mkInternalName )
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
As noted in Note [Forall coercions] in GHC.Entity.TypeAndCoercion, a ForAllCo is
a bit redundant.  It stores a TyVar and a Coercion, where the kind of the TyVar
always matches the left-hand kind of the coercion. This is convenient lots of
the time, but not when mapping a function over a coercion.
...
some variant of the dreaded KPush algorithm (c.f.
GHC.IR.Core.Transformer.Simple.pushCoDataCon).
...
Notably this plays a role in tcTySigs in GHC.IR.Haskell.TypeChecker.Binding
...
-- (See Note [The polymorphism rule of join points] in GHC.IR.Core.Syntax.)
-- Returns False also if the type simply doesn't have enough arguments.
--
...
-- See also Note [Non-trivial definitional equality] in GHC.Entity.TypeAndCoercion.
...
nonDetCmpTypeX :: RnEnv2 -> Type -> Type -> Ordering  -- Main workhorse
    -- See Note [Non-trivial definitional equality] in GHC.Entity.TypeAndCoercion
