...
module GHC.Entity.Id.Make (
        ...
        -- Re-export error Ids
        module GHC.IR.Core.Transformer.ConstantFolder
    ) where
  ...
  where
    ...
    info | new_tycon
         = base_info `setInlinePragInfo` alwaysInlinePragma
                   ...
                   -- See Note [Single-method classes] in GHC.IR.Haskell.TypeChecker.Instance
         ...
         = base_info `setRuleInfo` mkRuleInfo [rule]
                   ...
                   -- See Note [ClassOp/DFun selection] in GHC.IR.Haskell.TypeChecker.Instance
       ...
       ; let wrap_id = mkGlobalId (DataConWrapId data_con) wrap_name wrap_ty wrap_info
             wrap_info = noCafIdInfo
                         ...
                         `setStrictnessInfo`    wrap_sig
                             -- We need to get the CAF info right here because GHC.IR.Interface.Transformer.Tidier
...
-- a spliting operation (cf, GHC.IR.Haskell.TypeChecker.Pattern.tcConPat).
...
mkDictFunId :: Name      -- Name to use for the dict fun;
            ...
            -> [Type]
            -> Id
-- Implements the DFun Superclass Invariant (see GHC.IR.Haskell.TypeChecker.Instance)
    ...
    inline_prag
         = alwaysInlinePragma `setInlinePragmaActivation` ActiveAfter
                 NoSourceText 0
                  ...
                  -- (see GHC.IR.Core.Transformer.Substitution.simple_app) won't inline 'seq' on the
...
There is a special typing rule for ($) in GHC.IR.Haskell.TypeChecker.Expression, so the type of ($)
...
a) In source Haskell its second arg can have an unboxed type
      x `seq` (v +# w)
   But see Note [Typing rule for seq] in GHC.IR.Haskell.TypeChecker.Expression, which
   explains why we give seq itself an ordinary type
         seq :: forall a b. a -> b -> b
   and treat it as a language construct from a typing point of view.

b) Its fixity is set in GHC.IR.Interface.Loader.ghcPrimIface

c) It has quite a bit of desugaring magic.
   See GHC.Compiler.HaskellToCore.Utils Note [Desugaring seq (1)] and (2) and (3)
...
Next, we add a built-in rule (see GHC.IR.Core.Transformer.ConstantFolder),
