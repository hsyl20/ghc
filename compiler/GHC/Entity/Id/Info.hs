...
data IdDetails
  ...
  | RecSelId
    ...
    }                           -- See Note [Naughty record selectors] in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration
...
In GHC.IR.Interface.Transformer.Tidier, when the LocalId becomes a GlobalId, its RULES are stripped off
...
-- CafInfo is used to build Static Reference Tables (see
-- GHC.Compiler.StgToCmm.*)
