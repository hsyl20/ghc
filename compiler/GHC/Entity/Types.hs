...
\section[GHC.Entity.Types]{Types for the per-module compiler}
-}
...
import GHC.Entity.ClassInstance    ( InstEnv, ClsInst, identicalClsInstHead )
...
import GHC.Entity.Id.Info           ( IdDetails(..), RecSelParent(..))
import GHC.Entity.Type

import GHC.IR.Haskell.Syntax.Annotation    ( ApiAnns )
import GHC.Entity.Annotation      ( Annotation, AnnEnv, mkAnnEnv, plusAnnEnv )
...
import GHC.Builtin.Names         ( gHC_PRIM, ioTyConName, printName
                                 , mkInteractiveModule , eqTyConName )
import GHC.Builtin.Types
import GHC.Packages     hiding  ( Version(..) )
import GHC.Program.CmdLineParser
import GHC.Config.Flags hiding  ( WarnReason(..) )
import GHC.Program.Driver.Phases ( Phase, HscSource(..), isHsBootOrSig
                                 , hscSourceString )
...
import GHC.Utils
import GHC.Entity.Unique.DeterSet
...
data HscEnv
  = HscEnv {
        ...
        hsc_type_env_var :: Maybe (Module, IORef TypeEnv)
                ...
                -- the 'IfGblEnv'. See 'GHC.IR.Haskell.TypeChecker.Types.tcg_type_env_var' for
                -- 'GHC.IR.Haskell.TypeChecker.Types.TcGblEnv'.  See also Note [hsc_type_env_var hack]
...
data ModGuts
  = ModGuts {
        ...
        mg_rules     :: ![CoreRule],     -- ^ Before the core pipeline starts, contains
                                         -- See Note [Overall plumbing for rules] in
...
Where do interactively-bound Ids come from?
  ...
  - GHCi REPL Stmts   e.g.
    ...
    GHC.IR.Haskell.TypeChecker.Module.externaliseAndTidyId, so they get Names
    like Ghic4.foo.

  - Ids bound by the debugger etc have Names constructed by
    GHC.IR.Interface.Environment.newInteractiveBinder; at the call sites it is
    followed by mkVanillaGlobal or mkVanillaGlobalWithInfo.  So again, they are
    all Global, External.
...
-- names returned by GHC.IR.Interface.Loader.ifaceDeclImplicitBndrs, in the sense that
-- TyThing.getOccName should define a bijection between the two lists.
-- This invariant is used in GHC.IR.Interface.Loader.loadDecl (see note [Tricky iface loop])
...
data Dependencies
  = Deps { dep_mods   :: [(ModuleName, IsBootInterface)]
         ...
         , dep_pkgs   :: [(InstalledUnitId, Bool)]
                        ...
                        -- (Safe Haskell). See Note [GHC.IR.Haskell.Renamer.ImportExport . Tracking Trust Transitively]
  ...
  deriving( Eq )
        ...
        -- See 'GHC.IR.Haskell.TypeChecker.Types.ImportAvails' for details on dependencies.
...
data ExternalPackageState
  = EPS {
        eps_is_boot :: !(ModuleNameEnv (ModuleName, IsBootInterface)),
                ...
                -- direct from 'GHC.IR.Haskell.TypeChecker.Types.imp_dep_mods'
...
data HsParsedModule = HsParsedModule {
    ...
    -- See note [Api annotations] in GHC.IR.Haskell.Syntax.Annotation
  }
...
This stuff is in here, rather than (say) in GHC.Interactive.Linker, because the
latter stuff is the *dynamic* linker, and isn't present in a stage-1 compiler
-}
...
dsGetCompleteMatches in GHC.Compiler.HaskellToCore.Splices accomplishes this lookup.

Also see Note [Typechecking Complete Matches] in
GHC.IR.Haskell.TypeChecker.Binding for a more detailed explanation for how GHC
