...
module GHC.StgToCmm.Expression ( cgExpr ) where
...
import GHC.StgToCmm.Profiling ( saveCurrentCostCentre,
...
import GHC.CoreTypes.Type             ( isUnliftedType )
import GHC.CoreTypes.RepType          ( isVoidTy, countConRepArgs, primRepSlot )
import GHC.CoreTypes.CostCentre       ( CostCentreStack, currentCCS )
...
GHC.StgToCmm.Expression for the purpose of heap check placement, we
*must* inline the primop later in GHC.StgToCmm.PrimOp. If we don't
...
cgIdApp fun_id args = do
    ...
    let cg_fun_id   = cg_id fun_info
           ...
           -- See Note [Externalise when splitting] in GHC.StgToCmm.Monad
