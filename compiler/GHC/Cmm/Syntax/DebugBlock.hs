...
module GHC.Cmm.Syntax.DebugBlock (
...
import GHC.Cmm.Printer.Expression ( pprExpr )
 ...
 * C-- begins life in StgToCmm without any unwind information. This is because
   we haven't actually done any register assignment or stack layout yet, so
   there is no need for unwind information.
...
See also: Note [Unwinding information in the NCG] in GHC.Compiler.CmmToAsm.
-}
