...
import GHC.Program.CmdLineParser hiding (WarnReason(..))
import qualified GHC.Program.CmdLineParser as Cmd
...
import GHC.Entity.BasicTypes       ( IntWithInf, treatZeroAsInf )
...
-- See Note [Updating flag description in the User's Guide]
   ...
   | Opt_OmitYields
   | Opt_FunToThunk                     -- allow GHC.IR.Core.Transformer.WorkerWrapper.Utils.mkWorkerArgs to remove all value lambdas
...
data DynFlags = DynFlags {
  ...
  -- Unfolding control
  -- See Note [Discounts and thresholds] in GHC.IR.Core.Transformer.Inliner
...
optLevelFlags -- see Note [Documenting optimisation flags]
  = [ ([0,1,2], Opt_DoLambdaEtaExpansion)
    ...
    , ([1,2],   Opt_EnableRewriteRules)  -- Off for -O0; see Note [Scoping for Builtin rules]
                                         -- in GHC.IR.Core.Transformer.ConstantFolder
    , ([1,2],   Opt_FloatIn)
 ...
 * Eta reduction wasn't happening in the simplifier, but it was
   happening in CorePrep, on
        $fBla = MkDict (/\a. K a)
 * Result: rhsIsStatic told GHC.IR.Interface.Transformer.Tidier that $fBla might
...
data FilesToClean = FilesToClean {
  ...
  -- 'GHC.Utils.FileCleanup.cleanCurrentModuleTempFiles' is called, or otherwise
