...
import GHC.Driver.CmdLineParser hiding (WarnReason(..))
import qualified GHC.Driver.CmdLineParser as Cmd
import GHC.Config.Constants
import GHC.Util.Panic
import qualified GHC.Util.PrettyPrint.Colour as Col
import GHC.Util
import GHC.Data.Maybe
import GHC.Util.Monad
import qualified GHC.Util.PrettyPrint as Pretty
import GHC.CoreTypes.SrcLoc
import GHC.CoreTypes.BasicTypes       ( IntWithInf, treatZeroAsInf )
import GHC.Data.FastString
import GHC.Util.Outputable
import Foreign.C        ( CInt(..) )
import System.IO.Unsafe ( unsafeDupablePerformIO )
import {-# SOURCE #-} GHC.Util.Error ( Severity(..), MsgDoc, mkLocMessageAnn
                                      , getCaretDiagnostic, dumpSDoc )
import GHC.Util.Json
import GHC.Util.SysTools.Terminal ( stderrSupportsAnsiColors )
...
-- See Note [Updating flag description in the User's Guide]
   ...
   | Opt_OmitYields
   | Opt_FunToThunk                     -- allow GHC.Core.WorkerWrap.Utils.mkWorkerArgs to remove all value lambdas
...
data DynFlags = DynFlags {
  ...
  -- Unfolding control
  -- See Note [Discounts and thresholds] in GHC.Core.Inliner
  ...
  -- | MsgDoc output action: use "GHC.Util.Error" instead of this if you can
 ...
 * Eta reduction wasn't happening in the simplifier, but it was
   happening in CorePrep, on
        $fBla = MkDict (/\a. K a)
 * Result: rhsIsStatic told GHC.Interface.Tidy that $fBla might
   have CAF refs but the eta-reduced version (MkDict K) obviously doesn't
...
data FilesToClean = FilesToClean {
  ...
  -- 'GHC.Util.FileCleanup.cleanCurrentModuleTempFiles' is called, or otherwise
