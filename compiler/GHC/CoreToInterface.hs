...
import GHC.CoreTypes.Id.Make          ( noinlineIdName )
...
import GHC.CoreTypes.Demand           ( isTopSig )
...
toIfaceTypeX fr (TyVarTy tv)   -- See Note [TcTyVars in IfaceType] 
                               -- in GHC.Interface.Type
...
toIfUnfolding lb (CoreUnfolding { uf_tmpl = rhs
  ...
  = Just $ HsUnfold lb $
    case src of
        ...
        -- GHC.Interface.Tidy would have stuck in NoUnfolding.
