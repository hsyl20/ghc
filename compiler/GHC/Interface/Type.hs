...
import GHC.CoreTypes.TyCon hiding ( pprPromotionQuote )
...
type IfExtName = Name   -- An External or WiredIn Name can appear in
...
-- See Note [Preserve OneShotInfo] in GHC.Core.Tidy
-- and Note [The oneShot function] in GHC.CoreTypes.Id.Make
data IfaceOneShot    
  = IfaceNoOneShot
  | IfaceOneShot
...
in GHC.Builtin.Primitive.Types for details) which all must be rendered with
different surface syntax during pretty-printing. Which syntax we use depends
upon,
...
interface file. To avoid an import loop between IfaceType and
GHC.CoreTypes.Type.Internal we only keep the hole's Unique, since that is all we
need to print.
-}
...
-- See equivalent function in GHC.CoreTypes.Type.Internal
...
-- and Note [The equality types story] in GHC.Builtin.Primitive.Types
