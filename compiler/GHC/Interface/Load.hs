...
* BUT, if the TyCon is a wired-in TyCon, we don't really need its interface; but
  ...
  That is what GHC.Interface.Load.loadWiredInHomeIface does. It's called
  from GHC.Interface.TypeCheck.{tcImportDecl, checkWiredInTyCon,
  ifCheckWiredInThing}
...
in one-shot mode; see notes with hsc_HPT decl in GHC.Types.Base).
