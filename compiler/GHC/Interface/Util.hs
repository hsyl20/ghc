...
import GHC.CoreToInterface
import GHC.Config.Flags.Fingerprint

import GHC.HaskellToCore.Usage ( mkUsageInfo, mkUsedNames, mkDependencies )
...
import GHC.Util             hiding ( eqListBy )
  ...
  = do
    let semantic_mod = canonicalizeHomeModule (hsc_dflags hsc_env) (moduleName this_mod)
        ...
        fixities    = sortBy (comparing fst)
          ...
          -- See Note [Deterministic UniqFM] in GHC.Data.UniqueDFM
          -- for more details.
        warns       = src_warns
...
Names too: see Note [Binders in Template Haskell] in
GHC.Haskell.Template, and Trac #5362 for an example.
...
coAxBranchToIfaceBranch' tc (CoAxBranch { cab_tvs = tvs, cab_cvs = cvs
  ...
  where
    ...
    -- See Note [CoAxBranch type variables] in GHC.CoreTypes.Coercion.Axiom
