...
import GHC.CoreTypes.Type.Internal    -- needs to build types & coercions in a knot
...
'TyCon', in this case?  There are three cases:
    ...
    1) tcHiBootIface in GHC.Interface.TypeCheck: We're typechecking an
...
typecheckIfacesForMerging mod ifaces tc_env_var =
  ...
  initIfaceLcl mod (text "typecheckIfacesForMerging") False $ do
    ...
    -- serialize them out.
    -- See Note [rnIfaceNeverExported] in GHC.Interface.Rename
...
tc_ax_branch prev_branches
  ...
  = bindIfaceTyConBinders_AT
      (map (\b -> TvBndr b (NamedTCB Inferred)) tv_bndrs) $ \ tvs ->
         -- The _AT variant is needed here; see Note [CoAxBranch type variables]
         -- in GHC.CoreTypes.Coercion.Axiom
...
-- See Note [Integer literals] in GHC.CoreTypes.Literal
