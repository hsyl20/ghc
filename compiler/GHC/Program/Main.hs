...
-- in "GHC.Program.Driver.Pipeline".
--
...
import GHC.IR.Core.Transformer.Tidier         ( tidyExpr )
import GHC.Entity.Type    ( Type, Kind )
import GHC.IR.Core.Analyser.Lint         ( lintInteractiveExpr )
...
import GHC.IR.Haskell.TypeChecker.Module
import GHC.IR.Interface.TypeChecker   ( typecheckIface )
import GHC.IR.Haskell.TypeChecker
import GHC.Entity.Name.Cache          ( initNameCache )
import GHC.IR.Interface.Loader        ( ifaceStats, initExternalPackageState )
import GHC.Builtin.Utils
import GHC.IR.Interface.Utils
import GHC.Compiler.HaskellToCore
import GHC.IR.Core.Transformer.Pipeline
import GHC.IR.Interface.Transformer.Tidier
...
import GHC.Entity.CostCentre.Initialise
import GHC.Entity.TypeConstructor
import GHC.Entity.Name
import GHC.IR.Stg.Transformer.Pipeline ( stg2stg )
...
import GHC.Utils.Platform ( platformOS, osSubsectionsViaSymbols )

import GHC.Utils.Outputable
import GHC.Entity.Name.Environment
import GHC.IR.Haskell.Analyser.Stats         ( ppSourceStats )
...
hscIncrementalCompile always_do_basic_recompilation_check m_tc_result
  ...
  = do
    ...
    -- files. See GHC.IR.Haskell.TypeChecker.Types.TcGblEnv.tcg_type_env_var.
...
-- GHC.IR.Haskell.Renamer.ImportExport.rnImportDecl for where package trust dependencies for a module are
-- collected and unioned.  Specifically see the Note [GHC.IR.Haskell.Renamer.ImportExport . Tracking Trust
-- Transitively] and the Note [GHC.IR.Haskell.Renamer.ImportExport . Trust Own Package].
...
checkSafeImports dflags tcg_env
  ...
  where
    ...
    condense (_, [])   = panic "GHC.Program.Main.condense: Pattern match failure!"
...
IO monad as explained in Note [Interactively-bound Ids in GHCi] in GHC.Entity.Types
-}
 ...
 runInteractiveHsc hsc_env0 $ do
    ...
    let tcs = filterOut isImplicitTyCon (mg_tcs simpl_mg)
        ...
        ext_ids = [ id | id <- bindersOfBinds core_binds
            ...
            -- (as decided by GHC.IR.Interface.Transformer.Tidier), since those are the only ones
            -- that might later be looked up by name.  But we can exclude
            --    - DFunIds, which are in 'cls_insts' (see Note [ic_tythings] in GHC.Entity.Types
            --    - Implicit Ids, which are implicit in tcs
            -- c.f. GHC.IR.Haskell.TypeChecker.Module.runTcInteractive, which reconstructs the TypeEnv
...
{-
  ...
  When we want to evaluate something GHC.IR.Haskell.TypeChecker.Module.runTcInteractive pulls out this
