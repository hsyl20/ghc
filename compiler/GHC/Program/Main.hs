...
-- GHC.IR.Haskell.Renamer.ImportExport.rnImportDecl for where package trust
-- dependencies for a module are collected and unioned.  Specifically see the
-- Note [GHC.IR.Haskell.Renamer.ImportExport . Tracking Trust Transitively] and
-- the Note [GHC.IR.Haskell.Renamer.ImportExport . Trust Own Package].
...
checkSafeImports dflags tcg_env
  ...
  where
    ...
    condense (_, [])   = panic "condense: Pattern match failure!"
...
IO monad as explained in Note [Interactively-bound Ids in GHCi] in
GHC.Entity.Types
-}
 ...
 runInteractiveHsc hsc_env0 $ do
    ...
    let tcs = filterOut isImplicitTyCon (mg_tcs simpl_mg)
        ...
        ext_ids = [ id | id <- bindersOfBinds core_binds
            ...
            -- (as decided by GHC.IR.Interface.Transformer.Tidier), since those
            -- are the only ones that might later be looked up by name.  But we
            -- can exclude
            --    - DFunIds, which are in 'cls_insts' (see Note [ic_tythings] in
            --      GHC.Entity.Types)
            --    - Implicit Ids, which are implicit in tcs
            -- c.f. GHC.IR.Haskell.TypeChecker.Module.runTcInteractive, which
...
{-
  ...
  GHC.IR.Haskell.TypeChecker.Module.runTcInteractive pulls out this fixity
  ...
  updateFixityEnv, which will stuff it back into InteractiveContext, to be used
  in evaluating the next statement.
