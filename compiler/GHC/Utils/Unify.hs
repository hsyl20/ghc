...
import GHC.Entity.Type hiding ( getTvSubstEnv )
import GHC.Entity.Coercion hiding ( getCvSubstEnv )
import GHC.Entity.TypeConstructor
import GHC.Entity.TypeAndCoercion hiding ( getTvSubstEnv, getCvSubstEnv )
import GHC.Utils
...
eta-reduced; see Note [Eta reduction for data family axioms] in GHC.IR.Haskell.TypeChecker.Instance.
...
-- second call to tcUnifyTys in GHC.IR.Haskell.TypeChecker.FunctionalDependency.checkClsFD
...
GHC.IR.Haskell.TypeChecker.Unify, and the constraint solver itself in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser), the pure
