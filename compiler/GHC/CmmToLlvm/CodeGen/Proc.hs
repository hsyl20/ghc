...
--
module GHC.CmmToLlvm.CodeGen.Proc ( genLlvmProc ) where
...
import GHC.Util.CodeGen.Platform ( activeStgRegs, callerSaves )
...
import GHC.Util.Outputable hiding (panic, pprPanic)
import qualified GHC.Util.Outputable as Outputable
    ...
    let lmconv = case target of
            ForeignTarget _ (ForeignConvention conv _ _ _) ->
              case conv of
                 ...
                 CCallConv    -> CC_Ccc
                 CApiConv     -> CC_Ccc
                 PrimCallConv -> panic "GHC.CmmToLlvm.CodeGen.Proc.genCall: PrimCallConv"
                 JavaScriptCallConv -> panic "GHC.CmmToLlvm.CodeGen.Proc.genCall: JavaScriptCallConv"
    ...
    case getVarType vaddr of
        ...
        other ->
            pprPanic "genStore: ptr not right type!"
                    (GHC.Cmm.Printer.pprExpr addr <+> text (
            ...
            if getVarType vx == getVarType vy
                then do
                    ...
                    let style = mkCodeStyle CStyle
                        ...
                        cmmToStr = (lines . toString . GHC.Cmm.Printer.pprExpr)
...
genLoad_slow atomic e ty meta = runExprData $ do
    ...
    case getVarType iptr of
         ...
         other -> do pprPanic "exprToVar: CmmLoad expression is not right type!"
                        (GHC.Cmm.Printer.pprExpr e <+> text (
                            "Size of Ptr: " ++ show (llvmPtrBits dflags) ++
                            ", Size of var: " ++ show (llvmWidthInBits dflags other) ++
...
-- | Error functions
panic :: String -> a
panic s = Outputable.panic $ "GHC.CmmToLlvm.CodeGen.Proc." ++ s

pprPanic :: String -> SDoc -> a
pprPanic s d = Outputable.pprPanic ("GHC.CmmToLlvm.CodeGen.Proc." ++ s) d
