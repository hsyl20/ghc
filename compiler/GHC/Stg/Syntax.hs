...
@Cmm@): basically, what we have is a stylised form of @GHC.Core.Syntax@,
...
import GHC.CoreTypes.ForeignCall ( ForeignCall )
import GHC.CoreTypes.Id
import GHC.CoreTypes.Id.Info     ( mayHaveCafRefs )
import GHC.CoreTypes.Literal     ( Literal, literalType )
import GHC.CoreTypes.Module      ( Module )
import GHC.Util.Outputable
import GHC.Packages           ( isDllName )
import GHC.Util.Platform
import GHC.Core.Printer    ( {- instances -} )
import GHC.Builtin.Primitive.Operations ( PrimOp, PrimCall )
import GHC.CoreTypes.TyCon       ( PrimRep(..), TyCon )
import GHC.CoreTypes.Type        ( Type )
import GHC.CoreTypes.RepType     ( typePrimRep1 )
import GHC.CoreTypes.Unique      ( Unique )
import GHC.Util
...
-- `GHC.Interface.Tidy` is consistent with reality.
--
...
-- is that `GHC.Interface.Tidy` computed the CAF info on the
...
stgIdHasCafRefs id =
  ...
  -- have CAF refs. At this point (after GHC.Interface.Tidy)
...
Very like in @GHC.Core.Syntax@ (except no type-world stuff).
