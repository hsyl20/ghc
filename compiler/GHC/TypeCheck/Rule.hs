...
module GHC.TypeCheck.Rule ( tcRules ) where
...
import GHC.TypeCheck.Evidence   ( mkTcCoVarCo )
import GHC.CoreTypes.Type
import GHC.CoreTypes.Id
import GHC.CoreTypes.Var                        ( EvVar )
import GHC.CoreTypes.BasicTypes                 ( RuleName )
...
tcRule (HsRule name act hs_bndrs lhs fv_lhs rhs fv_rhs)
  = addErrCtxt (ruleCtxt $ snd $ unLoc name)  $
    do { traceTc "---- Rule ------" (pprFullRuleName name)
       ...
       -- c.f. GHC.TypeCheck.Simplify.simplifyInfer
       ...
       -- during zonking (see GHC.TypeCheck.Syntax.zonkRule)
