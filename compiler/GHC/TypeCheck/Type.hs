...
import GHC.TypeCheck.Simplify ( solveEqualities )
import GHC.TypeCheck.Util.CoreType
import GHC.TypeCheck.Syntax        ( zonkSigType )
import GHC.TypeCheck.Instantiation ( tcInstBinders, tcInstBinder )
...
with GHC.TypeCheck.Validity.checkValidType.
...
-- See Note [Distinguishing tuple kinds] in GHC.Syntax.Type
...
look at the TyCon or Class involved.
  ...
  * GHC.TypeCheck.Syntax.zonkTcTypeToType also can safely
...
tcExplicitTKBndrsX new_tv orig_hs_tvs thing_inside
  = go orig_hs_tvs $ \ tvs ->
    do { (result, bound_tvs) <- thing_inside tvs
         ...
         -- See Note [Bad telescopes] in GHC.TypeCheck.Validity.
...
-- pass in GHC.TypeCheck.TypeDecl. (Never in
 ...
 * The pattern (T p1 p2) creates a *skolem* type variable 'a_sk',
   ...
   GHC.TypeCheck.Error.getSkolemInfo can thereby find the binding
   site for the skolem.
