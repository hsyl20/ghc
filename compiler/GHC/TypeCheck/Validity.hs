...
import GHC.TypeCheck.Simplify (simplifyAmbiguityCheck)
import GHC.CoreTypes.Type.Internal
import GHC.TypeCheck.Util.CoreType hiding ( sizeType, sizeTypes )
...
import GHC.TypeCheck.Monad        -- TcType, amongst others
import GHC.TypeCheck.Environment  ( tcGetInstEnvs, tcInitTidyEnv, tcInitOpenTidyEnv )
...
import GHC.TypeCheck.FamilyInstance( makeInjectivityErrors )
...
'GHC.TypeCheck.FunctionalDependency.grow'.
-}
...
-- See Note [Extended defaults]" in GHC.TypeCheck.Environment
...
GHC.TypeCheck.Solver.Interact Note [Instance and Given
overlap].  As that Note discusses, for the most part the clever stuff in
GHC.TypeCheck.Solver.Interact means that we don't use a
...
circumstances that GHC.TypeCheck.Solver.Interact.matchInstEnv
...
instances] in GHC.TypeCheck.Solver.Interact).  However, we
