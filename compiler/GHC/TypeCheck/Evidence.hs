...
GHC.TypeCheck.Solver.Interact) because we can't have infinitely
many instances.
...
2. In a function call, instead of simply passing the given IP, we first
   ...
   (see GHC.TypeCheck.Solver.Interact.interactDict)
...
3. We default any insoluble CallStacks to the empty CallStack. Suppose
   ...
   (see GHC.TypeCheck.Simplify.simpl_top and
   GHC.TypeCheck.Simplify.defaultCallStacks)
...
-- Note [Type-checking overloaded labels] in GHC.TypeCheck.Expression.
