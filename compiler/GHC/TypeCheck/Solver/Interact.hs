...
module GHC.TypeCheck.Solver.Interact (
...
import GHC.TypeCheck.Solver.Canonicalise as Canonicaliser
import GHC.TypeCheck.Solver.Flatten
import GHC.TypeCheck.Unify( canSolveByUnification )
import GHC.CoreTypes.Var.Set
import GHC.CoreTypes.Type as Type
import GHC.CoreTypes.Kind              ( isConstraintKind )
import GHC.CoreTypes.Instance     ( DFunInstType, lookupInstEnv
                                    , instanceDFunId )
import GHC.CoreTypes.Coercion.Axiom    ( sfInteractTop, sfInteractInert )

import GHC.TypeCheck.Util.Monadic (newMetaTyVars)
...
import GHC.CoreTypes.Id      ( idType, isNaughtyRecordSelector )
import GHC.CoreTypes.Coercion.Axiom ( TypeEqn, CoAxiom(..), CoAxBranch(..), fromBranches )
import GHC.CoreTypes.Class
import GHC.CoreTypes.TyCon
import GHC.CoreTypes.DataCon( dataConWrapId )
import GHC.CoreTypes.FieldLabel
import GHC.TypeCheck.FunctionalDependency
import GHC.TypeCheck.FamilyInstance
import GHC.CoreTypes.FamilyInstance
import GHC.CoreTypes.Type.Unify ( tcUnifyTyWithTFs )

import GHC.TypeCheck.Evidence
import GHC.Util.Outputable

import GHC.TypeCheck.Util
import GHC.TypeCheck.Solver.Monad
import GHC.Data.Bag
import GHC.Util.Monad ( concatMapM )
...
import GHC.CoreTypes.Unique( hasKey )
...
the unflattening that takes place in step (1). See GHC.TypeCheck.Solver.Flatten,
...
we keep?  More subtle than you might think!
  ...
  * Constraints coming from the same level (i.e. same implication)
       ...
       - Always get rid of InstSC ones if possible, since they are less
         ...
         See Note [Solving superclass constraints] in GHC.TypeCheck.Instance
...
See also Note [Injective type families] in GHC.CoreTypes.TyCon
...
--     See GHC.TypeCheck.Util.Ct invariants.
--
...
GHC.TypeCheck.Solver.Monad; avoiding double unifications is
...
GHC.TypeCheck.FunctionalDependency (nicer now!).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
c.f. Note [The flattening story] in GHC.TypeCheck.Solver.Flatten
...
* For type-nat stuff the derived constraints include type families;
  ...
  (GHC.TypeCheck.Solver.Flatten.unflatten).
...
see Note [Simplifiable given constraints] in GHC.TypeCheck.Validity.
...
-- in GHC.TypeCheck.Evidence.  The coercion happens in 2 steps:
--
