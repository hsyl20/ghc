...
module GHC.TypeCheck.Solver.Flatten(
   FlattenMode(..),
...
import GHC.TypeCheck.Unify( occCheckExpand )
...
GHC.TypeCheck.Solver.Canonicalise.canEqTyVar.

See also Note [Conservative unification check] in
GHC.TypeCheck.Unify, which gives other examples where lazy
...
GHC.TypeCheck.Solver.Canonicalise.homogeniseRhsKind).
...
(unlike TyConAppCo), see Note [TcCoercions] in GHC.TypeCheck.Evidence.
...
GHC.TypeCheck.Solver.Interact.kick_out, by only kicking out an
inert only if 
