...
import {-# SOURCE #-} GHC.TypeCheck.Match
                           ( tcGRHSsPat , tcMatchesFun )
import {-# SOURCE #-} GHC.TypeCheck.Expression
                           ( tcMonoExpr )
import {-# SOURCE #-} GHC.TypeCheck.PatternSynonym
                           ( tcInferPatSynDecl, tcCheckPatSynDecl
                           , tcPatSynBuilderBind )
import GHC.Core.Syntax     (Tickish (..))
import GHC.CoreTypes.CostCentre  (mkUserCC)
import GHC.Config.Flags
import GHC.Data.FastString
import GHC.Syntax
import GHC.CoreTypes.Base       ( isHsBootOrSig )
import GHC.TypeCheck.Signature
...
import GHC.TypeCheck.Validity        ( checkValidType )
...
@tcBindsAndThen@ typechecks a @GHC.Syntax.Bind@.  The "and then"
...
tc_group top_lvl sig_fn prag_fn (Recursive, binds) closed thing_inside
  =     -- To maximise polymorphism, we do a new
        ...
        -- See Note [Polymorphic recursion] in GHC.Syntax.Bind.
...
-- GHC.TypeCheck.Environment.tcExtendIdEnv
  ...
  = checkNoErrs $  -- The checkNoErrs ensures that if the type is ambiguous
    ...
    do { fam_envs <- tcGetFamInstEnvs
       ; let (_co, mono_ty') = normaliseType fam_envs Nominal mono_ty
               ...
               -- GHC.TypeCheck.Solver.Flatten) so do it here to make it
...
tcVect (HsVectTypeOut _ _ _)
  = panic "GHC.TypeCheck.Bind.tcVect: Unexpected 'HsVectTypeOut'"
...
tcVect (HsVectClassOut _)
  = panic "GHC.TypeCheck.Bind.tcVect: Unexpected 'HsVectClassOut'"
...
tcVect (HsVectInstOut _)
  = panic "GHC.TypeCheck.Bind.tcVect: Unexpected 'HsVectInstOut'"
...
tcMonoBinds is_rec sig_fn no_gen
  ...
  =     -- Note [Single function non-recursive binding special-case]
    ...
    do  { ((co_fn, matches'), rhs_ty)
            <- tcInferInst $ \ exp_ty ->
                  -- tcInferInst: see GHC.TypeCheck.Unify,
...
We typecheck pattern bindings as follows.  First tcLhs does this:
  ...
  3. Invoke tcLetPat to typecheck the pattern.
     ...
     - We pass in the current TcLevel.  This is captured by
       GHC.TypeCheck.Pattern.tcLetPat, and put into the pc_lvl field
     ...
     - When we come to a binder (GHC.TypeCheck.Pattern.tcPatBndr),
