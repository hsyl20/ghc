...
import GHC.TypeCheck.Signature( TcPragEnv, lookupPragEnv, addInlinePrags )
...
import GHC.TypeCheck.Validity( arityErr )
...
tcPatBndr penv@(PE { pe_ctxt = LetPat { pc_lvl    = bind_lvl
  ...
  -- Note [Typechecking pattern bindings] in GHC.TypeCheck.Bind
...
-- See Note [Typechecking pattern bindings], item (4) in GHC.TypeCheck.Bind
--
...
GHC.TypeCheck.Match.  So we tauify the pat_ty before proceeding.
