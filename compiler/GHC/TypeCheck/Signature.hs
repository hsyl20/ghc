...
module GHC.TypeCheck.Signature(
       TcSigInfo(..),
...
import GHC.TypeCheck.Unify         ( tcSkolemise, unifyType )
import GHC.TypeCheck.Instantiation ( topInstantiate )
import GHC.TypeCheck.Environment   ( tcLookupId )
...
* tcInstSig: TcIdSigInfo -> TcIdSigInst
  ...
  binder.  You can see this in GHC.TypeCheck.Bind.tcLhsId.
...
impedance-matching check in GHC.TypeCheck.Bind.mkExport

See Note [Signature skolems] in GHC.TypeCheck.Util.CoreType

None of this applies to a function binding with a complete
signature, which doesn't use tcInstSig.
See GHC.TypeCheck.Bind.tcPolyCheck.
-}
...
3. Note that the HsWrapper can transform *any* function with the right
   ...
   GHC.TypeCheck.Instance.mk_meth_spec_prags
-}
