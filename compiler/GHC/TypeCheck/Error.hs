...
import GHC.TypeCheck.Unify     ( occCheckForErrors
                                            , OccCheckResult(..) )
...
import GHC.CoreTypes.Type.Unify                      ( tcMatchTys )
import GHC.CoreTypes.Module
import GHC.TypeCheck.FamilyInstance
import GHC.CoreTypes.FamilyInstance            ( flattenTys )
...
import GHC.Syntax.Bind     ( PatSynBind(..) )
...
import GHC.Data.Bag
import GHC.Util.Error              ( ErrMsg, errDoc, pprLocErrMsg )
import GHC.CoreTypes.BasicTypes
import GHC.CoreTypes.ConLike            ( ConLike(..) )
import GHC.Util
import GHC.TypeCheck.Environment ( tcLookupIdMaybe, tcInitTidyEnv )
import {-# SOURCE #-} GHC.TypeCheck.Simplify  ( tcSubsumes )
...
import GHC.CoreTypes.Var.Free ( fvVarList, unionFV )
...
we run into a type mismatch in GHC.TypeCheck.Unify, we normally
...
GHC.TypeCheck.Error.
GHC.TypeCheck.Error.reportTidyWanteds does not print the errors and
...
reportWanteds ctxt tc_lvl (WC { wc_simple = simples, wc_impl = implics })
 ...
 where
    ...
    -- (see GHC.TypeCheck.Util.insolubleWantedCt) is caught here, otherwise
...
mkTyVarEqErr' dflags ctxt report ct oriented tv1 co1 ty2
  ...
  , isUserSkolem ctxt tv1
    ...
    -- way round; see
    -- GHC.TypeCheck.Solver.Canonicalise.canEqTyVarTyVar
...
We don't re-use the code in GHC.TypeCheck.Solver.Canonicalise
