...
module GHC.TypeCheck.TypeDecl (
        tcTyAndClassDecls, tcAddImplicits,

        -- Functions used by GHC.TypeCheck.Instance to check
...
import GHC.TypeCheck.Validity
import GHC.TypeCheck.Syntax
import GHC.TypeCheck.TypeDeclUtil
import GHC.TypeCheck.Class
import {-# SOURCE #-} GHC.TypeCheck.Instance( tcInstDecls1 )
import GHC.TypeCheck.Deriving   ( DerivInfo )
import GHC.TypeCheck.Evidence   ( tcCoercionKind, isEmptyTcEvBinds )
import GHC.TypeCheck.Unify      ( checkConstraints )
...
Then:
  ...
  * During GHC.TypeCheck.Type.kcTyVar we look in the *local*
    env, to get the known kind for T.

  * But in GHC.TypeCheck.Type.ds_type (and ds_var_app in
  ...
  = -- Closed type families are a little tricky, because they contain the definition
    ...
    do { traceTc "Closed type family:" (ppr tc_name)
       ...
       ; branches <- mapM (tcTyFamInstEqn tc_fam_tc Nothing) eqns
         ...
         -- GHC.TypeCheck.Validity.checkValidCoAxiom
         -- Example: tc265
         ...
         -- differentiate names.
         -- See [Zonking inside the knot] in GHC.TypeCheck.Type
...
tcTyFamInstEqn :: TcTyCon -> Maybe ClsInstInfo -> LTyFamInstEqn GhcRn
               -> TcM CoAxBranch
-- Needs to be here, not in GHC.TypeCheck.Instance, because closed
...
checkValidTyCon tc
  ...
  where
    ...
    -- See Note [GADT record selectors] in GHC.TypeCheck.TypeDeclUtil
  ...
  = do { -- strip off the duplicates and look for ill-scoped things
       ...
       ; checkValidTelescope (pprTyVars vis_tvs) stripped_tvs extra
         `and_if_that_doesn't_error`
           ...
           -- It reports errors around Note [Dependent LHsQTyVars] in GHC.TypeCheck.Type
         when duplicate_vars (
...
2. Internally, GHC uses TypeApplications to instantiate the default method in an
   ...
   GHC.TypeCheck.Instance.  Thus, GHC needs to know exactly what
...
-- -dcore-lint is enabled. See Note [Role inference] in GHC.TypeCheck.TypeDeclUtil
