...
import GHC.CoreTypes.Id         ( idType )
...
import GHC.TypeCheck.Error
import GHC.TypeCheck.Evidence
import GHC.TypeCheck.Solver.Interact
import GHC.TypeCheck.Solver.Canonicalise   ( makeSuperClasses
                                                   , solveCallStack )
import GHC.TypeCheck.Util.Monadic  as TcM
import GHC.TypeCheck.Monad         as TcM
import GHC.TypeCheck.Solver.Monad  as TcS
import GHC.TypeCheck.Util.CoreType
import GHC.TypeCheck.Unify                 ( tcSubType_NC )
import GHC.Data.TrieMap       () -- DV: for now
import GHC.CoreTypes.Type
import GHC.Builtin.Types      ( liftedRepTy )
import GHC.CoreTypes.Type.Unify   ( tcMatchTyKi )
import GHC.Util
import GHC.CoreTypes.Var
import GHC.CoreTypes.Var.Set
import GHC.Data.UniqueSet
import GHC.CoreTypes.BasicTypes   ( IntWithInf, intGtLimit )
import GHC.Util.Error        ( emptyMessages )
...
captureTopConstraints thing_inside
  = do { static_wc_var <- TcM.newTcRef emptyWC ;
       ...
       -- See GHC.TypeCheck.Monad Note [Constraints and errors]
       ...
       ; case mb_res of
           ...
           Left {}   -> do { _ <- reportUnsolved lie; failM } }
                ...
                -- this function is here instead of GHC.TypeCheck.Monad
...
-- See Note [Overview of implicit CallStacks] in GHC.TypeCheck.Evidence
...
How is this implemented? It's complicated! So we'll step through it all:
 ...
 1) `GHC.CoreTypes.Instance.lookupInstEnv`
 ...
 2) `GHC.TypeCheck.Solver.Interact.matchClassInst`
 ...
 3) `GHC.TypeCheck.Solver.Interact.doTopReactDict`
 ...
 4) `GHC.TypeCheck.Simplify.simplifyTop`:
       ...
       * For unresolved constraints (all types), call
         `GHC.TypeCheck.Error.reportUnsolved`, while for resolved
         but unsafe overlapping dictionary constraints, call
         `GHC.TypeCheck.Error.warnAllUnsolved`. Both functions
       ...
       * In the case of `warnAllUnsolved` for resolved, but unsafe
         ...
         it) and call `GHC.TypeCheck.recordUnsafeInfer` to mark the
         module we are compiling as unsafe, passing the warning message along as
         the reason.

 5) `GHC.TypeCheck.Error.*Unsolved`
 ...
 6) `GHC.TypeCheck.recordUnsafeInfer`
 ...
 * Gather the constraints for the RHS with ambient level *one more than*
   ...
   in GHC.TypeCheck.Bind.tcPolyInfer
...
solveWanteds wc@(WC { wc_simple = simples, wc_impl = implics })
  = do { traceTcS "solveWanteds {" (ppr wc)
       ...
       ; wc1 <- solveSimpleWanteds simples
                ...
                -- See Note [Rewrite insolubles] in
...
-- See Note [The superclass story] in GHC.TypeCheck.Solver.Canonicalise
...
*  To find (a), when we have two Given constraints,
   ...
   GHC.TypeCheck.Solver.Interact.solveOneFromTheOther
...
* We don't report redundant givens for *every* implication; only for those which
  ...
  GHC.TypeCheck.Simplify.warnRedundantGivens:
   ...
   - More subtly, in a function definition
     ...
     other set False in
        - GHC.TypeCheck.Bind.tcSpecPrag
        - GHC.TypeCheck.Bind.tcTySig

  This decision is taken in setImplicationStatus, rather than
  GHC.TypeCheck.Error so that we can discard implication
...
GHC.TypeCheck.Util.CoreType.  for the leftover implication.
...
GHC.TypeCheck.Simplify prior to Oct 2014).
