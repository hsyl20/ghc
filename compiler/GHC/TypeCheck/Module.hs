...
import {-# SOURCE #-} GHC.TypeCheck.Splice ( finishTH )
...
import GHC.TypeCheck.Instantiation( deeplyInstantiate )
...
import GHC.TypeCheck.Annotation
import GHC.TypeCheck.Bind
import GHC.Haskell.Parser.HeaderInfo   ( mkPrelImports )
import GHC.TypeCheck.Default
import GHC.TypeCheck.Environment
import GHC.TypeCheck.Rule
import GHC.TypeCheck.Foreign
import GHC.TypeCheck.Instance
...
import GHC.Util
import GHC.Data.Bag
import GHC.TypeCheck.Instantiation (tcGetInsts)
...
tcRnModuleTcRnM hsc_env hsc_src
 ...
 = setSrcSpan loc $
   do { let { explicit_mod_hdr = isJust maybe_mod } ;
        ...
        -- interface files, via
        -- GHC.TypeCheck.Environment.setGlobalTypeEnv
...
tcRnStmt hsc_env rdr_stmt
  = runTcInteractive hsc_env $ do {
    ...
    global_ids <- mapM (externaliseAndTidyId this_mod) zonked_ids ;
        -- Note [Interactively-bound Ids in GHCi] in GHC.CoreTypes.Base
...
-- in GHCi] in GHC.CoreTypes.Base for more details. We do this lifting by trying
...
tcRnExpr hsc_env mode rdr_expr
  = runTcInteractive hsc_env $
    ...
    -- (GHC.TypeCheck.Bind.mkInferredPolyId). See Trac #10321
