...
\section[GHC.TypeCheck.Expression]{Typecheck an expression}
-}
...
module GHC.TypeCheck.Expression
...
import GHC.TypeCheck.Bind       ( chooseInferredQuantifiers
                                           , tcLocalBinds )
import GHC.TypeCheck.Signature     ( tcUserTypeSig, tcInstSig )
import GHC.TypeCheck.Simplify
                                           ( simplifyInfer , InferMode(..) )
import GHC.TypeCheck.FamilyInstance( tcGetFamInstEnvs
                                           , tcLookupDataFamInst )
...
import GHC.TypeCheck.Arrow
import GHC.TypeCheck.Match
import GHC.TypeCheck.Type
import GHC.TypeCheck.PatternSynonym( tcPatSynBuilderOcc
                                           , nonBidirectionalErr )
import GHC.TypeCheck.Pattern
...
import GHC.Util
import GHC.CoreTypes.Var.Environment         ( emptyTidyEnv, mkInScopeSet )
import GHC.Data.List.SetOps
import GHC.Data.Maybe
import GHC.Util.Outputable as Outputable
import GHC.Data.FastString
import Control.Monad
import GHC.CoreTypes.Class                   (classTyCon)
...
See also Note [seqId magic] in GHC.CoreTypes.Id.Make
-}
...
tcExpr (HsIf Nothing pred b1 b2) res_ty    -- Ordinary 'if'
  = do { pred' <- tcMonoExpr pred (mkCheckExpType boolTy)
       ; res_ty <- tauifyExpType res_ty
           ...
           -- in GHC.TypeCheck.Match (See #10619)
...
-- See Note [Grand plan for static forms] in GHC.Util.CodeGen.StaticPtrTable
-- for an overview.
-- To type check
--      (static e) :: p a
...
GHC.TypeCheck.TypeDecl), at least in the case of
GADTs. Consider
  ...
  = ASSERT( notNull rbnds )
    do  { -- STEP -2: typecheck the record_expr, the record to be updated
        ...
        ; let fixed_tvs = getFixedTyVars upd_fld_occs con1_tvs relevant_cons
              ...
              --   c.f. GHC.TypeCheck.Util.Monadic.newMetaTyVars
...
tcExpr (PArrSeq _ _) _
  = panic "GHC.TypeCheck.Expression.tcExpr: Infinite parallel array!"
...
tcInferFun fun
  = tcInferSigma fun
      -- NB: tcInferSigma; see GHC.TypeCheck.Unify
...
just like in GHC.TypeCheck.Bind.decideGeneralisationPlan
...
Note [Converting strings] in GHC.Haskell.Template
...
See also Note [HsRecField and HsRecUpdField] in GHC.Syntax.Pattern.
-}
...
mixedSelectors _ _ =
  panic "GHC.TypeCheck.Expression: mixedSelectors emptylists"
