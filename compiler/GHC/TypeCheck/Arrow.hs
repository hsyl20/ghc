...
module GHC.TypeCheck.Arrow ( tcProc ) where
...
import GHC.TypeCheck.Syntax( hsLPatType )
...
import GHC.CoreTypes.Id( mkLocalId )
...
tcArrDoStmt env ctxt (RecStmt { recS_stmts = stmts, recS_later_ids = later_names
  ...
  = do  { let tup_names = rec_names ++ filterOut (`elem` rec_names) later_names
        ...
        ; thing <- thing_inside res_ty
                ...
                --      (see note [RecStmt] in GHC.Syntax.Expression)
