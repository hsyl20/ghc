...
import GHC.CoreTypes.Kind              ( isLiftedTypeKind )
...
import GHC.StgToCmm.Layout  ( ArgRep(..), toArgRep, argRepSizeW
                                     , mkVirtHeapOffsets, mkVirtConstrOffsets )
import GHC.RTS.Storage hiding (WordOff, ByteOff, wordsToBytes)
...
maybe_is_tagToEnum_call app
  ...
  where
    extract_constr_Names ty
           ...
           -- the DataCon.  See GHC.CoreTypes.DataCon for details.
           | otherwise
...
-- and Note [Bottoming expressions] in GHC.Core.Util:
