...
This is accomplished through a combination of mechanisms:
  ...
  2. The knownKeyNames (which consist of the basicKnownKeyNames from
     ...
     GHC.IR.Interface.Environment.  This initialization ensures that when
     the type checker or renamer (both of which use
     GHC.IR.Interface.Environment) look up an original name (i.e. a pair of a
     Module and an OccName) for a known-key name they get the correct Unique.
...
things,
  ...
  b) The known infinite families of names are specially serialised by
     GHC.IR.Interface.Binary.putName, with that special treatment detected when
     we read back to ensure that we get back to the correct uniques. See Note
     [Symbol table representation of names] in GHC.IR.Interface.Binary and Note
     [How tuples work] in GHC.Builtin.Types.
...
by the user. For those things that *can* appear in source programs,
  ...
  c) GHC.IR.Interface.Environment.lookupOrigNameCache uses isBuiltInOcc_maybe to
     map built-in syntax directly onto the corresponding name, rather than
     trying to find it in the original-name cache.
...
import GHC.Config.Build ( cIntegerLibraryType, IntegerLibrary(..) )
...
-- plugins
pLUGINS :: Module
pLUGINS = mkThisGhcModule (fsLit "GHC.Plugin.Types")
...
-- homogeneous equality. See Note [The equality types story] in
...
--------------------- Template Haskell ------------------------
-- GHC.Builtin.Names.TemplateHaskell: USES ClassUniques 200-299
---------------------------------------------------------------
...
--------------------- Template Haskell ------------------------
-- GHC.Builtin.Names.TemplateHaskell: USES TyConUniques 200-299
---------------------------------------------------------------
...
---------------------- Template Haskell -------------------------
-- GHC.Builtin.Names.TemplateHaskell: USES DataUniques 200-250
-----------------------------------------------------------------
...
--------------------- Template Haskell ------------------------
-- GHC.Builtin.Names.TemplateHaskell: USES IdUniques 200-499
---------------------------------------------------------------
...
(@GHC.IR.Haskell.TypeChecker.Deriving@).
-}
