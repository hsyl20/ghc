...
import GHC.IR.Cmm.Syntax.Type
import GHC.Entity.Demand
import GHC.Entity.OccName          ( OccName, pprOccName, mkVarOccFS )
import GHC.Entity.TypeConstructor  ( TyCon, isPrimTyCon, PrimRep(..) )
import GHC.Entity.Type
import GHC.Entity.RepType          ( typePrimRep1, tyConPrimRep1 )
import GHC.Entity.BasicTypes       ( Arity, Fixity(..), FixityDirection(..),
                                     Boxity(..), SourceText(..) )
import GHC.Entity.ForeignCall      ( CLabelString )
import GHC.Entity.Unique           ( Unique, mkPrimOpIdUnique )
import GHC.Utils.Outputable
import GHC.Data.FastString
import GHC.Entity.Module           ( UnitId )
...
Note [primOpIsCheap]
~~~~~~~~~~~~~~~~~~~~
@primOpIsCheap@, as used in \tr{GHC.IR.Core.Transformer.Simplifier.Utils}.
...
'seq' for primops that can fail, so we don't treat them as cheap.
-}
...
calculating unfolding sizes; see GHC.IR.Core.Transformer.Inliner.sizeExpr.
-}
...
primOpCodeSizeDefault = 1
  -- GHC.IR.Core.Transformer.Inliner.primOpSize already takes into account
  -- primOpOutOfLine and adds some further costs for the args in that case.
