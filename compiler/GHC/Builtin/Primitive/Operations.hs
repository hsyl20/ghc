...
import GHC.CoreTypes.OccName          ( OccName, pprOccName, mkVarOccFS )
import GHC.CoreTypes.TyCon  ( TyCon, isPrimTyCon, PrimRep(..) )
import GHC.CoreTypes.Type
import GHC.CoreTypes.RepType          ( typePrimRep1, tyConPrimRep1 )
import GHC.CoreTypes.BasicTypes       ( Arity, Fixity(..), FixityDirection(..),
                                     Boxity(..), SourceText(..) )
import GHC.CoreTypes.ForeignCall      ( CLabelString )
import GHC.CoreTypes.Unique           ( Unique, mkPrimOpIdUnique )
import GHC.Util.Outputable
import GHC.Data.FastString
import GHC.CoreTypes.Module           ( UnitId )
...
calculating unfolding sizes; see GHC.Core.Inliner.sizeExpr.
-}
...
primOpCodeSizeDefault = 1
  -- GHC.Core.Inliner.primOpSize already takes into account
