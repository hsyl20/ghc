...
import GHC.IR.Haskell.TypeChecker.Type     ( TcType, tcEqType )
import GHC.Entity.TypeConstructor      ( TyCon, FamTyConFlav(..), mkFamilyTyCon
                  , Injectivity(..) )
import GHC.Entity.Coercion   ( Role(..) )
import GHC.IR.Haskell.TypeChecker.Types  ( Xi )
import GHC.Entity.Coercion.Axiom    ( CoAxiomRule(..), BuiltInSynFamily(..), TypeEqn )
import GHC.Entity.Name       ( Name, BuiltInSyntax(..) )
