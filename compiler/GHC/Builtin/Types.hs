...
--   C.f module GHC.Builtin.Primitive.Types
module GHC.Builtin.Types (
        -- * Helper functions defined here
        mkWiredInTyConName, -- This is used in GHC.Builtin.TypeNats to define
                            -- the built-in functions for evaluation.
...
import {-# SOURCE #-} GHC.Entity.Id.Make( mkDataConWorkId, mkDictSelId )
...
import {-# SOURCE #-} GHC.Entity.ConstructorLike
import GHC.Entity.TypeConstructor
import GHC.Entity.Class            ( Class, mkClass )
import GHC.Entity.RdrName
import GHC.Entity.Name as Name
import GHC.Entity.Name.Environment ( NameEnv, mkNameEnv, lookupNameEnv )
import GHC.Entity.Name.Set         ( NameSet, mkNameSet, elemNameSet )
import GHC.Entity.BasicTypes       ( Arity, Boxity(..), TupleSort(..), ConTagZ,
                                     SourceText(..) )
import GHC.Entity.ForeignCall
import GHC.Entity.SrcLoc           ( noSrcSpan )
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
The RuntimeRep type (and friends) in GHC.Types has a bunch of
constructors, making it a pain to wire in. To ease the pain somewhat, we use
lists of the different bits, like Uniques, Names, DataCons. These lists must be
kept in sync with each other. The rule is this: use the order as declared in
GHC.Types. All places where such lists exist should contain a reference
...
* When looking up an OccName in the original-name cache
  (GHC.IR.Interface.Environment.lookupOrigNameCache), we spot the tuple OccName
  to make sure we get the right wired-in name.  This guy can't tell the
  difference between BoxedTuple and ConstraintTuple (same OccName!), so tuples
...
* Serialization to interface files works via the usual mechanism for known-key
  ...
  in GHC.Builtin.Uniques. See Note [Symbol table representation of names] for
  details.
...
GHC supports both boxed and unboxed one-tuples:
 ...
 - A boxed one-tuple is used by
   GHC.Compiler.HaskellToCore.Utils.mkSelectorBinds, when
...
-- Type synonyms; see Note [TYPE and RuntimeRep] in GHC.Builtin.Primitive.Types
