...
import {-# SOURCE #-} GHC.CoreTypes.ConLike
import GHC.CoreTypes.TyCon
import GHC.CoreTypes.Class            ( Class, mkClass )
import GHC.CoreTypes.RdrName
import GHC.CoreTypes.Name as Name
import GHC.CoreTypes.Name.Environment ( NameEnv, mkNameEnv, lookupNameEnv )
import GHC.CoreTypes.Name.Set         ( NameSet, mkNameSet, elemNameSet )
import GHC.CoreTypes.BasicTypes       ( Arity, Boxity(..), TupleSort(..), ConTagZ,
                                     SourceText(..) )
...
import GHC.Util
import GHC.Data.BoolFormula       ( mkAnd )
...
* When looking up an OccName in the original-name cache
  (GHC.Interface.Environment.lookupOrigNameCache), we spot the tuple OccName
...
mk_tuple Unboxed arity = (tycon, tuple_con)
  where
    ...
    -- See Note [Unboxed tuple RuntimeRep vars] in GHC.CoreTypes.TyCon
...
-- see Note [TYPE and RuntimeRep] in GHC.Builtin.Primitive.Types
runtimeRepTy :: Type
