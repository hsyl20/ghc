...
import GHC.IR.Haskell.TypeChecker  ( initTcInteractive, initIfaceTcRn )
import GHC.IR.Interface.Loader     ( loadPluginInterface )
import GHC.Entity.RdrName          ( RdrName, ImportSpec(..), ImpDeclSpec(..)
                                   , ImpItemSpec(..), mkGlobalRdrEnv
                                   , lookupGRE_RdrName , gre_name, mkRdrQual )
import GHC.Entity.OccName          ( OccName, mkVarOcc )
...
import GHC.Entity.Type             ( Type, eqType, mkTyConTy, pprTyThingCategory )
import GHC.Entity.TypeConstructor  ( TyCon )
import GHC.Entity.Name             ( Name, nameModule_maybe )
import GHC.Entity.Id               ( idType )
import GHC.Entity.Module           ( Module, ModuleName )
