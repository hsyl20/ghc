...
{-
    ...
    * The number of registers used for parameter passing and the exact set of
      nonvolatile registers differs (see GHC.CmmToAsm.PPC.Regs).
    ...
    * The SysV spec claims that FF32 is represented as FF64 on the stack. GCC on
    ...
    to hold the parameter lists for all called routines). The CmmToAsm compiler
    already uses the stack for register spilling, leaving 64 bytes free at the
    top.  If we need a larger parameter area than that, we just allocate a new
    stack frame just before ccalling.
-}
...
-- in form of:
--     bl puts+32768@plt
-- in this form it's not seen directly (by GHC CmmToAsm compiler)
