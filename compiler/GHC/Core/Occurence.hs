...
import GHC.CoreTypes.Name   ( localiseName )
...
import GHC.CoreTypes.Demand       ( argOneShots, argsOneShots )
...
See also Note [Inlining and hs-boot files] in GHC.CoreToInterface, which deals
...
recursive functions] in GHC.Core.Simplify
...
GHC.Core.Syntax), it can still be made a join point with some effort. This is
