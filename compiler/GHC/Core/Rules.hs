...
import GHC.Types.Type                 ( Type, mkTCvSubst )
import qualified GHC.Types.Type as Type
import GHC.Haskell.TypeCheck.Type ( tcSplitTyConApp_maybe )
...
import qualified GHC.Types.Type.Unify as Unify
import GHC.Types.BasicTypes           ( Activation, CompilerPhase, isActive
                                       , pprRuleName )
...
* GHC.Interface.Tidy strips off all the rules from local Ids
...
mkRule this_mod is_auto is_local name act fn bndrs args rhs
  ...
  where
        -- Compute orphanhood.
        -- See Note [Orphans] in GHC.Types.Instance
...
-- It's here rather than in GHC.Core.Printer because it calls tidyRules
...
but that isn't quite right:
     ...
     - The rules in GHC.Core.ConstantFold.builtinRules should
