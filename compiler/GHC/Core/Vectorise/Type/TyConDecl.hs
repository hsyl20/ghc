...
module GHC.Core.Vectorise.Type.TyConDecl (
  vectTyConDecls
) where
...
import GHC.Core.Vectorise.Env( GlobalEnv(global_fam_inst_env) )
import GHC.Interface.BuildTypeAndClass( TcMethInfo, buildClass, buildDataCon
                                         , newTyConRepName )
...
import GHC.CoreTypes.BasicTypes  ( DefMethSpec(..) )
