...
import GHC.CoreTypes.Literal( pprLiteral )
import GHC.CoreTypes.Name( pprInfixName, pprPrefixName )
...
import GHC.CoreTypes.SrcLoc      ( pprUserRealSpan )
...
ppIdInfo id info
  = sdocWithDynFlags $ \dflags ->
    ...
    , (not (null rules), text "RULES:" <+> vcat (map pprRule rules))
        ...
        -- see GHC.Core.Printer.pprIdBndr
  where
