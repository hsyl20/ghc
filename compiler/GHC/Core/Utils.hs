...
import GHC.Types.TypeAndCoercion( TyBinder(..) )
...
isExprLevPoly = go
  where
   ...
   -- this case can happen in GHC.Core.LevelSet
   go (Coercion {})                = False
...
-- "GHC.Core.Syntax#let_app_invariant"
...
because it is true of "CONLIKE" Ids: see Note [CONLIKE pragma] in
...
isDivOp :: PrimOp -> Bool
-- This function probably belongs in GHC.Builtin.Primitive.Operations, or even
-- in an automagically generated file.. but it's such a special case I thought
...
-- happen: see "GHC.Core.Syntax#let_app_invariant". This invariant states
...
* Note [Arity care]: we need to be careful if we just look at f's
  ...
  Note [Arity robustness] in GHC.Core.Simplify.Environment) so
  we must *not* trust the arity when checking that 'f' is a value.  Otherwise we
  will eta-reduce
      f = \x. f x
  to
      f = f
...
rhsIsStatic :: Platform
            ...
            -> CoreExpr -> Bool
-- It's called
--    (i) in GHC.Interface.Tidy.hasCafRefs to decide if the rhs
...
--
--
-- This is a bit like GHC.Core.Utils.exprIsHNF, with the following
-- differences:
