...
import GHC.CoreTypes.Name             ( Name, mkSystemVarName, isExternalName
                                  , getOccFS )
import GHC.CoreTypes.Coercion hiding  ( substCo, substCoVar )
import GHC.CoreTypes.Coercion.Optimise( optCoercion )
import GHC.CoreTypes.FamilyInstance   ( topNormaliseType_maybe )
import GHC.CoreTypes.DataCon          ( DataCon, dataConWorkId
                                  , dataConRepStrictness, dataConRepArgTys )
import GHC.Core.Monad             ( Tick(..), SimplMode(..) )
...
import GHC.CoreTypes.BasicTypes       ( TopLevelFlag(..), isNotTopLevel
                                  , isTopLevel, RecFlag(..), Arity )
import GHC.Util.Monad            ( mapAccumLM, liftIO )
...
import GHC.CoreTypes.Module             ( moduleName, pprModuleName )
    ...
    go n_val_args (Var fun)
        ...
        where
          is_exp = isExpandableApp fun n_val_args   -- The fun a constructor or PAP
                        -- See Note [CONLIKE pragma] in GHC.CoreTypes.BasicTypes
...
See Note [User-defined RULES for seq] in GHC.CoreTypes.Id.Make.
...
simplAlt env scrut' _ case_bndr' cont' (DataAlt con, vs, rhs)
  ...
  where
        ...
        -- See Note [Data-con worker strictness] in GHC.CoreTypes.Id.Make
    add_evals the_strs
        = go vs the_strs
        where
          go [] [] = []
