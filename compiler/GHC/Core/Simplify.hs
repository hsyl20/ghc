...
import GHC.Types.Type hiding      ( substTy, substTyVar, extendTvSubst
                                   , extendCvSubst )
import GHC.Core.Simplify.Environment
import GHC.Core.Simplify.Utils
import GHC.Core.Occurence  ( occurAnalyseExpr )
import GHC.Types.FamilyInstance   ( FamInstEnv )
import GHC.Types.Literal          ( litIsLifted )
import GHC.Types.Id
import GHC.Types.Id.Make          ( seqId )
import GHC.Core.Syntax.Make     ( mkImpossibleExpr, castBottomExpr )
import GHC.Types.Id.Info
import GHC.Types.Name             ( Name, mkSystemVarName, isExternalName
                                   , getOccFS )
import GHC.Types.Coercion hiding  ( substCo, substCoVar )
import GHC.Types.Coercion.Optimise( optCoercion )
import GHC.Types.FamilyInstance   ( topNormaliseType_maybe )
import GHC.Types.DataCon  ( DataCon, dataConWorkId
...
--import GHC.Types.TyCon       ( isEnumerationTyCon )
--import GHC.Builtin.Primitive.Types      ( intPrimTy )
--import GHC.Types.Unique                ( hasKey )
--import GHC.Types.Literal               ( mkMachInt )

import GHC.Core.Rules ( mkRuleInfo, lookupRule, getRules )
import GHC.Types.BasicTypes         ( TopLevelFlag(..), isNotTopLevel
                                     , isTopLevel, RecFlag(..) )
...
the simplifier is in GHC.Core.Monad
...
GHC.Core.Simplify.Utils.FloatsWith.
...
simplTopBinds env0 binds0
  = do  {       -- Put all the top-level binders into scope at the start
                ...
                -- See Note [Glomming] in GHC.Core.Occurence.
    ...
    go n_val_args env (Var fun)
        ...
        where
          is_exp = isExpandableApp fun n_val_args   -- The fun a constructor or PAP
                        -- See Note [CONLIKE pragma] in GHC.Types.BasicTypes
                        -- The definition of is_exp should match that in
                        -- GHC.Core.Occurence.occAnalApp
...
See Note [Core syntax top-level string literals] in GHC.Core.Syntax.
...
Similarly to CPR (see Note [Don't CPR join points] in
GHC.Core.WorkerWrap), a join point stands well to gain from
its outer binding's eta-expansion, and eta-expanding a join point is fraught
with issues like how to deal with a cast:
...
--
-- Precondition: rhs satisfies the let/app invariant
--               Note [Core syntax let/app invariant] in GHC.Core.Syntax
--
...
GHC.Core.Rules, but failed to occ-anal the OutExpr arguments,
...
See Note [Binder swap] in GHC.Core.Occurence.
...
see Note [Add scrutinee to ValueEnv too] in
...
NB: take great care when doing strictness analysis;
    see Note [Lambda-bound unfoldings] in GHC.Core.Demand.
  ...
  = is_bottoming `seq`  -- See Note [Force bottoming field]
    do { dflags <- getDynFlags
       ; return (mkUnfolding dflags InlineRhs is_top_lvl is_bottoming new_rhs) }
            ...
            --         (b) In GHC.Interface.Tidy we currently
...
See Note [Forming Rec groups] in GHC.Core.Occurence
-}
