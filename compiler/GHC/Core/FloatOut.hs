...
module GHC.Core.FloatOut ( floatOutwards ) where
...
import GHC.Util.Error             ( dumpIfSet_dyn )
import GHC.CoreTypes.Id               ( Id, idArity, idType, isBottomingId,
                                     isJoinId, isJoinId_maybe )
import GHC.CoreTypes.BasicTypes       ( TopLevelFlag(..), isTopLevel )
import GHC.Core.LevelSet
import GHC.Data.UniqueSupply    ( UniqSupply )
import GHC.Data.Bag
import GHC.Util
...
join points] in GHC.Core.Syntax), so we must be careful with how far we float
them. The mechanism for doing so is the *join ceiling*, detailed in Note [Join
ceiling] in GHC.Core.LevelSet. For us, the significance is
...
GHC.Core.LevelSet. The difference between "this *is at* level
...
floatExpr lam@(Lam (TB _ lam_spec) _)
  = let (bndrs_w_lvls, body) = collectBinders lam
        ...
        -- See GHC.Core.LevelSet.lvlLamBndrs
...
The trouble is that
  ...
  (b) GHC.Core.LevelSet introduces a new bindings site for every float
