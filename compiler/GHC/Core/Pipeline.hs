...
\section[GHC.Core.Pipeline]{Driver for simplifying @Core@ programs}
-}

{-# LANGUAGE CPP #-}

module GHC.Core.Pipeline ( core2core, simplifyExpr ) where
...
import GHC.Core.CommonSubExpr ( cseProgram )
import GHC.Core.Rules     ( mkRuleBase, unionRuleBase
                                         , extendRuleBaseList , ruleCheckProgram
                                         , addRuleInfo )
import GHC.Core.Printer               ( pprCoreBindings, pprCoreExpr )
...
import GHC.Util.Error        ( withTiming )
import GHC.CoreTypes.BasicTypes  ( CompilerPhase(..), isDefaultInlinePragma )
import GHC.CoreTypes.Var.Set
import GHC.CoreTypes.Var.Environment
import GHC.Core.LiberateCase     ( liberateCase )
import GHC.Core.StaticArgument   ( doStaticArgs )
import GHC.Core.Specialise      ( specProgram)
import GHC.Core.Specialise.Constructor ( specConstrProgram)
import GHC.Core.Demand              ( dmdAnalProgram )
import GHC.Core.CallArity           ( callArityAnalProgram )
import GHC.Core.WorkerWrap    ( wwTopBinds )
import GHC.Core.Vectorise       ( vectorise )
import GHC.CoreTypes.SrcLoc
import GHC.Util
import GHC.CoreTypes.Module

import GHC.Data.Maybe
import GHC.Data.UniqueSupply       ( UniqSupply, mkSplitUniqSupply
                                      , splitUniqSupply )
...
getCoreToDo dflags
  ...
  where
    ...
    -- after this before anything else
        ...
        -- has run at all.
        -- See Note [Final Demand Analyser run] in GHC.Core.Demand
