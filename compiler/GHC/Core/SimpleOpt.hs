...
import GHC.CoreTypes.Var                 ( varType )
...
import GHC.CoreTypes.Type         hiding ( substTy, extendTvSubst, extendCvSubst
                                      , extendTvSubstList , isInScope
                                      , substTyVarBndr, cloneTyVarBndr )
import GHC.CoreTypes.Coercion     hiding ( substCo, substCoVarBndr )
import GHC.CoreTypes.TyCon     ( tyConArity )
...
-- GHC.Core.Subst.substIdBndr carefully does not do)
...
unfolding. Also see Note [Desugaring coerce as cast] in
GHC.HaskellToCore.
...
GHC.Core.Syntax.Make.mkStringExprFS, or unpackCStringUtf8# when the literal
...
-- GHC.Core.Syntax.Make.mkStringExprFS turns those into [] automatically, but
...
-- C.f. simplCast in GHC.Core.Simplify
