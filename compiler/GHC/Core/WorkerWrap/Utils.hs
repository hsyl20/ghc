...
import GHC.Types.Id
import GHC.Types.Id.Info           ( JoinArity, vanillaIdInfo )
...
import GHC.Types.BasicTypes        ( Boxity(..) )
import GHC.Types.Literal           ( absentLiteralOf )
...
Note [CPR for thunks] in GHC.Core.Demand.
...
(see Note [Worker-wrapper for INLINABLE functions] in
GHC.Core.WorkerWrap), which can still be specialised by the
type-class specialiser, something like
   fw :: Ord a => [a] -> Int# -> a
