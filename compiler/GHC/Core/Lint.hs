...
import GHC.Haskell.TypeCheck.Type ( isFloatingTy )
...
import GHC.Types.TypeAndCoercion       -- checks validity of types/coercions
import GHC.Types.TyCon as TyCon
...
import GHC.Utils
import GHC.Types.Instance     ( instanceDFunId )
import GHC.Types.Coercion.Optimise ( checkAxInstCo )
import GHC.Data.UniqueSupply
import GHC.Core.Arity   ( typeArity )
import GHC.Types.Demand            ( splitStrictSig, isBotRes )
...
GHC.Core.Syntax. The only one that causes any difficulty is the first: All
  ...
  = addLoc (RhsOf binder) $
    ...
    do { ty <- lintRhs binder rhs
       ...
       ; checkL (isJoinId binder || not (isTypeLevPoly binder_ty))
        ...
        -- See Note [Core syntax let/app invariant] in GHC.Core.Syntax
      ...
      --                  in GHC.Core.Demand.  After eta-expansion
...
lintIdUnfolding  _ _ _
  = return ()       -- Do not Lint unstable unfoldings, because that leads
                    ...
                    -- c.f. GHC.Core.FreeVars.idUnfoldingVars
...
in, say, GHC.Core.Substitution.simpleOptPgm, which in some
circumstances can run immediately before Float Out.

All that said, currently GHC.Core.Substitution.simpleOptPgm is
...
lintTyApp fun_ty arg_ty
  ...
  = do  { lintTyKind tv arg_ty
        ...
        -- See Note [The substitution invariant] in GHC.Types.TypeAndCoercion
...
lint_app doc kfn kas
    = do { in_scope <- getInScope
         ...
         -- Note [The substitution invariant] in GHC.Types.TypeAndCoercion
...
'GHC.Core.Arity.etaExpandToJoinPointRule'), and in fact the
...
[Rules and join points] in GHC.Core.Analyse.Occurence for further discussion.
-}
       ...
       ; case (splitForAllTy_maybe s, splitForAllTy_maybe t) of
         ...
         { (Just (tc_s, tys_s), Just (tc_t, tys_t))
             ...
             , isInjectiveTyCon tc_s r
                 -- see Note [NthCo and newtypes] in GHC.Types.TypeAndCoercion
...
~~~~~~~~~~~~~~~~~~~~~~~~~~
See Note [Grand plan for static forms] in GHC.Utils.CodeGen.StaticPtrTable for
an overview.
...
Pointer Table from the top-level bindings. See GHC.Core.Monad
