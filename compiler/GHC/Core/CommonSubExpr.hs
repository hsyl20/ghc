...
module GHC.Core.CommonSubExpr (cseProgram, cseOneExpr) where

#include "HsVersions.h"

import GHC.Core.Substitution
import GHC.Types.Var             ( Var )
import GHC.Types.Var.Environment ( elemInScopeSet, mkInScopeSet )
import GHC.Types.Id              ( Id, idType, idInlineActivation, isDeadBinder
                                 , zapIdOccInfo, zapIdUsageInfo, idInlinePragma
                                 , isJoinId )
import GHC.Core.Utils            ( mkAltExpr, eqExpr , exprIsLiteralString
                                 , stripTicksE, stripTicksT, mkTicks )
import GHC.Core.FreeVars         ( exprFreeVars )
import GHC.Types.Type            ( tyConAppArgs )
...
GHC.Core.Syntax). This seems to come up quite seldom, but it happens (first
...
-- Note [Type-let] in GHC.Core.Syntax), in which case we can just substitute.
...
[Core syntax top-level string literals] in GHC.Core.Syntax.
