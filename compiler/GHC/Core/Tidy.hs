...
import GHC.CoreTypes.Demand         ( zapUsageEnvSig )
import GHC.CoreTypes.Type           ( tidyType, tidyTyCoVarBndr )
import GHC.CoreTypes.Coercion       ( tidyCo )
import GHC.CoreTypes.Var
import GHC.CoreTypes.Var.Environment
import GHC.Data.UniqueFM
import GHC.CoreTypes.Name hiding (tidyNameOcc)
...
tidyLetBndr rec_tidy_env env@(tidy_env, var_env) (id,rhs)
  = case tidyOccName tidy_env (getOccName id) of { (tidy_env', occ') ->
    let
        ...
        --      (See Note [Zapping DmdEnv after Demand Analyzer] in
        --      GHC.Core.WorkerWrap)
        --
...
tidyUnfolding tidy_env
  ...
  = seqIt $ unf { uf_tmpl = tidyExpr tidy_env unf_rhs }    -- Preserves OccInfo
    ...
    -- pre-tidied expression forever (GHC.CoreToInterface doesn't look
    -- at them)
...
must preserve this info in inlinings. See Note [The oneShot function] in
