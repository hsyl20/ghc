...
module GHC.Core.Demand ( dmdAnalProgram ) where
...
import GHC.CoreTypes.Name             ( getName, stableNameCmp )
...
see Note [Demand on case-alternative binders] in GHC.CoreTypes.Demand
...
GHC.Core.Arity), but its demand signature will be that of plusInt.
...
-- See Note [Trimming a demand to a type] in GHC.CoreTypes.Demand
