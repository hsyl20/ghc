...
import GHC.Core.Substitution as CoreSubst
...
import GHC.Types.OccName          ( occNameString )
import GHC.Types.Type             ( Type, mkLamTypes, splitTyConApp_maybe )
import GHC.Types.BasicTypes       ( Arity, RecFlag(..), isRec )
import GHC.Types.DataCon  ( dataConOrigResTy )
...
-- In GHC.Core.LevelSet we do set the context to (Level 0 0)
...
lvlExpr env expr@(_, AnnLam {})
  ...
  where
    ...
    (new_env, new_bndrs) = lvlLamBndrs env1 (le_ctxt_lvl env) bndrs1
        ...
        -- [See GHC.Core.LevelSet rev 1.50 for a version with
        -- this approach.]
...
lvlApp env orig_expr ((_,AnnVar fn), args)
  ...
  where
    ...
    left _ _ _                   = panic "GHC.Core.LevelSet.lvlExpr.left"
...
lvlMFE env strict_ctxt ann_expr
  ...
  where
    ...
    is_mk_static = isJust (collectMakeStaticArgs expr)
        -- Yuk: See Note [Grand plan for static forms] in
...
etaExpand during GHC.Core.LevelSet because it works over a
...
-- GHC.Core.Substitution.substBndrs
...
destLevel env fvs is_function is_bot is_join
  ...
  | is_join  -- Never float a join point past the join ceiling
             -- See Note [Join points] in GHC.Core.FloatOut
...
abstractVars :: Level -> LevelEnv -> DVarSet -> [OutVar]
        ...
        -- See Note [Unique Determinism] in GHC.Types.Unique for explanation of
        -- why Uniques are not deterministic.
