...
import GHC.CoreTypes.Literal          ( litIsTrivial )
...
import Data.Maybe
import GHC.Util.Monad       ( mapAccumLM )
...
lvlLamBndrs env lvl bndrs
  ...
  where
    ...
    is_major bndr = isId bndr && not (isProbablyOneShotLambda bndr)
       ...
       -- See Note [Computing one-shot info] in GHC.CoreTypes.Demand
...
abstractVars :: Level -> LevelEnv -> DVarSet -> [OutVar]
        ...
        -- See Note [Unique Determinism] in GHC.CoreTypes.Unique for explanation of
 ...
 = ASSERT( all (not . isCoVar) bndrs )   -- What would we add to the CoSubst in this case. No easy answer.
  ...
  where
    ...
    mk_poly_bndr bndr uniq = transferPolyIdInfo bndr abs_vars $
                             -- Note [transferPolyIdInfo] in GHC.CoreTypes.Id
