...
import qualified GHC.Core.Substitution as CoreSubst
...
import GHC.Types.Type     hiding( substTy )
import GHC.Types.Coercion hiding( substCo )
import GHC.Types.DataCon ( dataConWorkId, isNullaryRepDataCon )
...
-- See Note [RHS of lets] in GHC.Core.Inliner
...
contArgs cont
  ...
  | otherwise = go [] cont
  where
    -- See Note [Lone variables] in GHC.Core.Inliner
...
(see Note [Specialisation shape] in GHC.Core.Specialise), Trac
#9509.
       ...
       ; WARN( new_arity < old_id_arity,
               (text "Arity decrease:" <+> (ppr bndr <+> ppr old_id_arity
                <+> ppr old_arity <+> ppr new_arity) $$ ppr new_rhs) )
                        -- Note [Arity decrease] in GHC.Core.Simplify
...
* In dataToTag we might need to make up some fake binders;
  see Note [caseRules for dataToTag] in GHC.Core.ConstantFold
-}
