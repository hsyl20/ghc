...
import GHC.Core.Syntax.Make hiding ( FloatBind(..) )
                               -- We use our own FloatBind here
...
import GHC.CoreTypes.Name       ( NamedThing(..), nameSrcSpan )
import GHC.CoreTypes.SrcLoc     ( SrcSpan(..), realSrcLocSpan, mkRealSrcLoc )
import Data.Bits
...
9.  Replace (lazy e) by e.  See Note [lazyId magic] in GHC.CoreTypes.Id.Make
...
cpeApp top_env expr
  ...
  where
    ...
    rebuild_app (a : as) fun' fun_ty floats ss = case a of
      ...
      CpeApp arg -> do
        let (ss1, ss_rest)  -- See Note [lazyId magic] in GHC.CoreTypes.Id.Make
...
-- See Note [lazyId magic] in GHC.CoreTypes.Id.Make
...
-- The gory details are in Note [lazyId magic] in GHC.CoreTypes.Id.Make, but the
-- main reason is that lazy must appear in unfoldings (optimizer output) and it
-- must prevent call-by-value for catch# (which is implemented by CorePrep.)
--
