...
\section{GHC.Core.Transform.LevelSetting}
...
module GHC.Core.Transform.LevelSetting (
        setLevels,
...
-- In GHC.Core.Transform.LevelSetting we do set the context to (Level 0 0) when we get to an InlineMe
...
lvlExpr env (_, AnnType ty)     = return (Type (GHC.Core.Substitution.substTy (le_subst env) ty))
...
lvlExpr env expr@(_, AnnLam {})
  ...
  where
    ...
    (new_env, new_bndrs) = lvlLamBndrs env1 (le_ctxt_lvl env) bndrs1
        ...
        -- [See GHC.Core.Transform.LevelSetting rev 1.50 for a version with this approach.]
...
lvlApp env orig_expr ((_,AnnVar fn), args)
  ...
  where
    ...
    left _ _ _                   = panic "GHC.Core.Transform.LevelSetting.lvlExpr.left"
...
lvlMFE env _ (_, AnnType ty)
  = return (Type (GHC.Core.Substitution.substTy (le_subst env) ty))
...
etaExpand during GHC.Core.Transform.LevelSetting because it works over a decorated form of
...
-- So named only to avoid the name clash with GHC.Core.Substitution.substBndrs
  ...
  where
    ...
    mk_poly_bndr bndr uniq = transferPolyIdInfo bndr abs_vars $         -- Note [transferPolyIdInfo] in Id.hs
                           ...
                           where
                             ...
                             poly_ty = mkLamTypes abs_vars (GHC.Core.Substitution.substTy subst (idType bndr))
