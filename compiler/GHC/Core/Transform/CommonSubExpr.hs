...
module GHC.Core.Transform.CommonSubExpr (cseProgram, cseOneExpr) where
...
the binding instead?  Well, not at top level! See GHC.Core.Transform.Simplify.Utils
...
GHC.Core.Transform.Simplify.Utils Note [Stable unfoldings and postInlineUnconditionally].
...
GHC.Core.Syntax). This seems to come up quite seldom, but it happens (first seen
...
[GHC.Core.Syntax top-level string literals] in GHC.Core.Syntax.
...
combineIdenticalAlts in GHC.Core.Transform.Simplify.Utils.prepareAlts.  The basic idea is
to transform
