...
module GHC.Core.Transform.FloatOut ( floatOutwards ) where
...
import GHC.Core.Transform.LevelSetting
...
in GHC.Core.Transform.LevelSetting. For us, the significance is that a binder might be marked to be
...
floatBind (NonRec (TB var _) rhs)
  = case (floatRhs var rhs) of { (fs, rhs_floats, rhs') ->
        ...
        -- A tiresome hack:
        -- see Note [Bottoming floats: eta expansion] in GHC.Core.Transform.LevelSetting
...
TODO: This can probably be solved somehow in GHC.Core.Transform.LevelSetting. The difference between
...
floatExpr lam@(Lam (TB _ lam_spec) _)
  = let (bndrs_w_lvls, body) = collectBinders lam
        ...
        -- See GHC.Core.Transform.LevelSetting.lvlLamBndrs
...
The trouble is that
  ...
  (b) GHC.Core.Transform.LevelSetting introduces a new bindings site for every float
