...
{-# LANGUAGE CPP #-}
module GHC.Core.Transform.Simple (
...
simple_bind_pair env@(SOE { soe_inl = inl_env, soe_subst = subst })
  ...
  where
    ...
    pre_inline_unconditionally
       ...
       | isExportedId in_bndr     = False    --     in GHC.Core.Transform.Simplify.Utils
       | stable_unf               = False
...
In GHC.Core.Transform.Simplify.Utils, Note [Top level and postInlineUnconditionally], we
