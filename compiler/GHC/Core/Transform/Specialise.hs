...
{-# LANGUAGE CPP #-}
module GHC.Core.Transform.Specialise ( specProgram, specUnfolding ) where
...
import GHC.Core.Transform.Simple         ( collectBindersPushingCo )
           ...
           {    -- Figure out the type of the specialised function
             let body_ty = applyTypeToArgs rhs fn_type rule_args
                 (lam_args, app_args)           -- Add a dummy argument if body_ty is unlifted
                   | isUnliftedType body_ty     -- C.f. GHC.Core.Transform.WorkerWrapper.Utils.mkWorkerArgs
                   , not (isJoinId fn)
...
Thing, but it's what happens now; see GHC.Core.Transform.Simplify.Utils Note [Casts and
