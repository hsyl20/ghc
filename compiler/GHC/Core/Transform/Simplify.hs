...
module GHC.Core.Transform.Simplify
   ( simplTopBinds
   , simplExpr
   , simplRules
   )
...
import GHC.Core.Transform.Simplify.Monad
import GHC.Data.Type hiding      ( substTy, substTyVar, extendTvSubst, extendCvSubst )
import GHC.Core.Transform.Simplify.Environment
import GHC.Core.Transform.Simplify.Utils
...
import GHC.Core.Transform.Simple         ( pushCoTyArg, pushCoValArg
                        , joinPointBinding_maybe, joinPointBindings_maybe )
--import PrimOp           ( tagToEnumKey ) -- temporalily commented out. See #8326
import GHC.Core.Transform.Rules            ( mkRuleInfo, lookupRule, getRules )
...
the simplifier is in GHC.Core.Transform.Pipeline.hs.
...
expression. This is wrapped as a datatype GHC.Core.Transform.Simplify.Utils.FloatsWith.
  ...
  = ASSERT( isId bndr )
    ...
    do  { let   rhs_env     = rhs_se `setInScopeAndZapFloats` env
        ...
        ; (body_env, tvs') <- simplBinders rhs_env tvs
                -- See Note [Floating and type abstraction] in GHC.Core.Transform.Simplify.Utils
...
completeBind env top_lvl is_rec mb_cont old_bndr new_bndr new_rhs
 ...
 = ASSERT( isId new_bndr )
   do { let old_info = idInfo old_bndr
        ...
        -- See Note [Eta-expanding at let bindings] in GHC.Core.Transform.Simplify.Utils
...
--       }
  ...
  getDoneId (DoneEx e _) = getIdFromTrivialExpr e -- Note [substTickish] in GHC.Core.Substitution
...
rebuildCall env (ArgInfo { ai_fun = fun, ai_args = rev_args, ai_strs = [] }) cont
  ...
  -- that the call is definitely bottom; see GHC.Core.Transform.Simplify.Utils.mkArgInfo
...
The code in GHC.Core.Transform.Simplify.Utils.prepareAlts has the effect of generalise this
...
Note that GHC.Core.Transform.Simplify.Utils.mkCase combines identical RHSs.  So
...
simplUnfolding env top_lvl mb_cont id unf
                  ...
                  UnfWhen { ug_arity = arity, ug_unsat_ok = sat_ok }  -- Happens for INLINE things
                     -> let guide' = UnfWhen { ug_arity = arity, ug_unsat_ok = sat_ok
                        ...
                        in return (mkCoreUnfolding src is_top_lvl expr' guide')
                            -- See Note [Top-level flag on inline rules] in GHC.Core.Transform.Unfolding
  ...
  where
    ...
    rule_env     = updMode (updModeForStableUnfoldings act) env
         -- See Note [Simplifying inside stable unfoldings] in GHC.Core.Transform.Simplify.Utils
