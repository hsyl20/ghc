...
-}
  ...
  | StgConApp   DataCon
                ...
                [Type]          -- See Note [Types in StgConApp] in GHC.IR.Stg.Transform.Unarise
...
-- `CoreToStg`) to make sure CAF-ness predicted by `GHC.IR.Interface.Tidy` is consistent with
-- reality.
--
...
-- is that `GHC.IR.Interface.Tidy` computed the CAF info on the `Id` but some transformations
...
stgIdHasCafRefs id =
  ...
  -- have CAF refs. At this point (after GHC.IR.Interface.Tidy) top-level Ids (whether
