...
@Cmm@): basically, what we have is a stylised form of @GHC.IR.Core.Syntax@,
the style being one that happens to be ideally suited to spineless tagless code
generation.
-}

{-# LANGUAGE CPP #-}

module GHC.IR.Stg.Syntax (
        GenStgArg(..),
...
import GHC.Entity.CostCentre  ( CostCentreStack )
import Data.ByteString        ( ByteString )
import Data.List              ( intersperse )
import GHC.Entity.DataConstructor
import GHC.Config.Flags
import GHC.Data.FastString
import GHC.Entity.ForeignCall ( ForeignCall )
import GHC.Entity.Id
import GHC.Entity.Id.Info     ( mayHaveCafRefs )
import GHC.Entity.Literal     ( Literal, literalType )
import GHC.Entity.Module      ( Module )
import GHC.Utils.Outputable
import GHC.Packages           ( isDllName )
import GHC.Utils.Platform
import GHC.IR.Core.Printer    ( {- instances -} )
import GHC.Builtin.Primitive.Operations ( PrimOp, PrimCall )
import GHC.Entity.TypeConstructor       ( PrimRep(..), TyCon )
import GHC.Entity.Type        ( Type )
import GHC.Entity.RepType     ( typePrimRep1 )
import GHC.Entity.Unique      ( Unique )
import GHC.Utils
...
-}
  ...
  | StgConApp   DataCon
                ...
                [Type]          -- See Note [Types in StgConApp] in
                                -- GHC.IR.Stg.Transformer.Unariser
...
-- `CoreToStg`) to make sure CAF-ness predicted by
-- `GHC.IR.Interface.Transformer.Tidier` is consistent with reality.
--
...
-- is that `GHC.IR.Interface.Transformer.Tidier` computed the CAF info on the
-- `Id` but some transformations have taken place since then.
...
stgIdHasCafRefs id =
  ...
  -- have CAF refs. At this point (after GHC.IR.Interface.Transformer.Tidier)
  -- top-level Ids (whether imported or defined in this module) are GlobalIds,
  -- so the test is easy.
...
Very like in @GHC.IR.Core.Syntax@ (except no type-world stuff).
...
{- Many passes apply a substitution, and it's very handy to have type
   ...
   See GHC.IR.Core.Syntax for precedence in Core land
-}
