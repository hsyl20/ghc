...
\section[GHC.IR.Stg.Syntax]{Shared term graph (STG) syntax for spineless-tagless code generation}

This data type represents programs just before code generation (conversion to
@Cmm@): basically, what we have is a stylised form of @GHC.IR.Core.Syntaxtax@, the style
...
-}
  ...
  | StgConApp   DataCon
                ...
                [Type]          -- See Note [Types in StgConApp] in GHC.IR.Stg.Transformer.Unariser
...
-- `CoreToStg`) to make sure CAF-ness predicted by `GHC.IR.Interface.Transformer.Tidier` is consistent with
-- reality.
--
...
-- is that `GHC.IR.Interface.Transformer.Tidier` computed the CAF info on the `Id` but some transformations
...
stgIdHasCafRefs id =
  ...
  -- have CAF refs. At this point (after GHC.IR.Interface.Transformer.Tidier) top-level Ids (whether
...
Very like in @GHC.IR.Core.Syntaxtax@ (except no type-world stuff).
