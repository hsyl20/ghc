...
\section[GHC.IR.Stg.Transformer.Pipeline]{Driver for simplifying @STG@ programs}
-}

{-# LANGUAGE CPP #-}

module GHC.IR.Stg.Transformer.Pipeline ( stg2stg ) where

#include "HsVersions.h"

import GHC.IR.Stg.Syntax

import GHC.Entity.CostCentre       ( CollectedCCs )
import GHC.IR.Stg.Transformer.CostCentreCollecter         ( stgMassageForProfiling )
import GHC.IR.Stg.Analyser.Lint  ( lintStgTopBindings )
import GHC.IR.Stg.Analyser.Stats ( showStgStats )
import GHC.IR.Stg.Transformer.Unariser       ( unarise )
import GHC.IR.Stg.Transformer.CommonSubExpr ( stgCse )

import GHC.Config.Flags
import GHC.Entity.Module           ( Module )
import GHC.Utils.Error
import GHC.Entity.SrcLoc
import GHC.Entity.Unique.Supply       ( mkSplitUniqSupply, splitUniqSupply )
