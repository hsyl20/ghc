...
STG programs after unarisation have these invariants:
  ...
  * DataCon applications (StgRhsCon and StgConApp) don't have void arguments.
    ...
    `GHC.Compiler.StgToCmm.Environment.NonVoid`, for example.
...
module GHC.IR.Stg.Transformer.Unariser (unarise) where
...
import GHC.Entity.Literal (Literal (..))
import GHC.IR.Core.Syntax.Make (aBSENT_ERROR_ID)
import GHC.Entity.Id.Make (voidPrimId, voidArgId)
