...
-- See Note [Detecting forced eta expansion] in GHC.Compilers.HaskellToCore.Expression
...
-- information, see Note [Strict binds check] is GHC.Compilers.HaskellToCore.Binding.
...
{-
Note [Dictionary binders in ConPatOut] See also same Note in GHC.Compilers.HaskellToCore.Arrow
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Do *not* gather (a) dictionary and (b) dictionary bindings as binders
of a ConPatOut pattern.  For most calls it doesn't matter, because
it's pre-typechecker and there are no ConPatOuts.  But it does matter
more in the desugarer; for example, GHC.Compilers.HaskellToCore.Utils.mkSelectorBinds uses
collectPatBinders.  In a lazy pattern, for example f ~(C x y) = ...,
we want to generate bindings for x,y but not for dictionaries bound by
C.  (The type checker ensures they would not be used.)

Desugaring of arrow case expressions needs these bindings (see GHC.Compilers.HaskellToCore.Arrow
...
So in mkSelectorBinds in GHC.Compilers.HaskellToCore.Utils, we want just m,n as the variables bound.
-}
