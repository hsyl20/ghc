...
Here we collect a variety of helper functions that construct or analyse
GHC.IR.Haskell.  All these functions deal with generic GHC.IR.Haskell; functions
which deal with the instantiated versions are located elsewhere:

   Parameterised by     Module
   ----------------     -------------
   GhcPs/RdrName        GHC.IR.Haskell.Parser.Syntax
   GhcRn/Name           GHC.IR.Haskell.Renamer.Environment
   GhcTc/Id             GHC.IR.Haskell.TypeChecker.Syntax
-}
...
import GHC.Entity.Type   ( filterOutInvisibleTypes )
...
GHC.Compiler.HaskellToCore.Arrow
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Do *not* gather (a) dictionary and (b) dictionary bindings as binders of a
ConPatOut pattern.  For most calls it doesn't matter, because it's
pre-typechecker and there are no ConPatOuts.  But it does matter more in the
desugarer; for example, GHC.Compiler.HaskellToCore.Utils.mkSelectorBinds uses
collectPatBinders.  In a lazy pattern, for example f ~(C x y) = ..., we want to
generate bindings for x,y but not for dictionaries bound by C.  (The type
checker ensures they would not be used.)

Desugaring of arrow case expressions needs these bindings (see
GHC.Compiler.HaskellToCore.Arrow and arrowcase1), but SPJ (Jan 2007) says it's
...
So in mkSelectorBinds in GHC.Compiler.HaskellToCore.Utils, we want just m,n as
the variables bound.
-}
