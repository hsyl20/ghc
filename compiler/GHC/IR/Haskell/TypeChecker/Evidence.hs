...
module GHC.IR.Haskell.TypeChecker.Evidence (
...
import GHC.IR.Core.Printer ()   -- Instance OutputableBndr TyVar
...
data HsWrapper
  ...
  | WpFun HsWrapper HsWrapper TcType SDoc
       ...
       -- the desugarer. See Note [Levity polymorphism checking] in GHC.Compiler.HaskellToCore.Monad
...
and the constraint
    [G] g1 :: a~Bool
See Trac [7238] and Note [Bind new Givens immediately] in GHC.IR.Haskell.TypeChecker.Types
...
GHC.IR.Haskell.TypeChecker.Constraint.Interact) because we can't have infinitely
many instances.
...
2. In a function call, instead of simply passing the given IP, we first
   ...
   (see GHC.IR.Haskell.TypeChecker.Constraint.Interact.interactDict)
...
3. We default any insoluble CallStacks to the empty CallStack. Suppose
   ...
   (see GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.simpl_top and GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.defaultCallStacks)
...
- An EvCallStack term desugars to a CoreExpr of type `IP "some str" CallStack`.
  ...
  call-site onto a given stack (See GHC.Compiler.HaskellToCore.Binding.dsEvCallStack)
...
-- Note [Type-checking overloaded labels] in GHC.IR.Haskell.TypeChecker.Expression.
