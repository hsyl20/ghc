...
module GHC.IR.Haskell.TypeChecker.ModuleSignature (
    findExtraSigImports',
    findExtraSigImports,
...
import GHC.Entity.BasicTypes (defaultFixity)
import GHC.Packages
...
import GHC.Entity.NameShape
import GHC.IR.Haskell.TypeChecker.Error
import GHC.IR.Haskell.TypeChecker.Unify
import GHC.IR.Interface.Renamer
import GHC.Utils

import Control.Monad
import Data.List (find, foldl')

import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Module
...
checkHsigIface tcg_env gr sig_iface
  ModDetails { md_insts = sig_insts, md_fam_insts = sig_fam_insts,
    ...
    unless (null sig_fam_insts) $
        panic ("GHC.IR.Haskell.TypeChecker.Module.checkHsigIface: Cannot handle family " ++
...
-- for "GHC.Program.Make" and "GHC.IR.Haskell.TypeChecker.Module".
...
--    and the Note [Resolving never-exported Names in GHC.IR.Interface.TypeChecker].
--
...
--    up as needed.
--
--    The details are in Note [rnIfaceNeverExported] in 'GHC.IR.Interface.Renamer'.
--
...
mergeSignatures
  ...
  orig_tcg_env lcl_iface0 = setSrcSpan loc $ do
    ...
    -- the types match.
    --
    -- See also Note [rnIfaceNeverExported] in GHC.IR.Interface.Renamer
