...
checkHsigIface tcg_env gr sig_iface
  ModDetails { md_insts = sig_insts, md_fam_insts = sig_fam_insts,
    ...
    unless (null sig_fam_insts) $
        panic ("checkHsigIface: Cannot handle family " ++
...
--    and the Note [Resolving never-exported Names] in
...
--    up as needed.
--
--    The details are in Note [rnIfaceNeverExported] in
--    'GHC.IR.Interface.Renamer'.
--
