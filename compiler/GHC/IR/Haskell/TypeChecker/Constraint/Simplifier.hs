...
module GHC.IR.Haskell.TypeChecker.Constraint.Simplifier(
...
import GHC.Data.Bag
import GHC.Entity.Class      ( Class, classKey, classTyCon )
import GHC.Config.Flags      ( WarningFlag ( Opt_WarnMonomorphism )
                             , WarnReason ( Reason )
                             , DynFlags( solverIterations ) )
import GHC.IR.Haskell.TypeChecker.Instantiation
...
import GHC.IR.Haskell.TypeChecker.Error
import GHC.IR.Haskell.TypeChecker.Evidence
import GHC.IR.Haskell.TypeChecker.Constraint.Interact
import GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser   ( makeSuperClasses )
import GHC.IR.Haskell.TypeChecker.MutableType   as TcM
import GHC.IR.Haskell.TypeChecker as TcM
import GHC.IR.Haskell.TypeChecker.Constraint.Solver  as TcS
...
import GHC.Entity.BasicTypes  ( IntWithInf, intGtLimit )
...
captureTopConstraints thing_inside
  = do { static_wc_var <- TcM.newTcRef emptyWC ;
       ...
       -- See GHC.IR.Haskell.TypeChecker Note [Constraints and errors]
       ...
       ; case mb_res of
           ...
           Left {}   -> do { _ <- reportUnsolved lie; failM } }
                ...
                -- this function is here instead of GHC.IR.Haskell.TypeChecker
...
-- See Note [Overview of implicit CallStacks] in GHC.IR.Haskell.TypeChecker.Evidence
...
How is this implemented? It's complicated! So we'll step through it all:
 ...
 1) `GHC.Entity.ClassInstance.lookupInstEnv`
 ...
 2) `GHC.IR.Haskell.TypeChecker.Constraint.Interact.matchClassInst`
 ...
 3) `GHC.IR.Haskell.TypeChecker.Constraint.Interact.doTopReactDict`
 ...
 4) `GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.simplifyTop`:
       ...
       * For unresolved constraints (all types), call
         `GHC.IR.Haskell.TypeChecker.Error.reportUnsolved`, while for resolved
         but unsafe overlapping dictionary constraints, call
         `GHC.IR.Haskell.TypeChecker.Error.warnAllUnsolved`. Both functions
       ...
       * In the case of `warnAllUnsolved` for resolved, but unsafe
         ...
         it) and call `GHC.IR.Haskell.TypeChecker.recordUnsafeInfer` to mark the
         module we are compiling as unsafe, passing the warning message along as
         the reason.

 5) `GHC.IR.Haskell.TypeChecker.Error.*Unsolved`
 ...
 6) `GHC.IR.Haskell.TypeChecker.recordUnsafeInfer`
    Save the unsafe result and reason in an IORef called `tcg_safeInfer`.

 7) `GHC.Program.Main.tcRnModule'`
    Reads `tcg_safeInfer` after type-checking, calling
    `GHC.Program.Main.markUnsafeInfer` (passing the reason along) when
 ...
 * Gather the constraints for the RHS with ambient level *one more than*
   ...
   in GHC.IR.Haskell.TypeChecker.Binding.tcPolyInfer
...
data InferMode = ApplyMR          -- ^ Apply the monomorphism restriction,
               ...
               | EagerDefaulting  -- ^ See Note [TcRnExprMode] in
                                  -- GHC.IR.Haskell.TypeChecker.Module,
...
*  To find (a), when we have two Given constraints,
   ...
   GHC.IR.Haskell.TypeChecker.Constraint.Interact.solveOneFromTheOther
...
* GHC.IR.Haskell.TypeChecker.Error does the actual warning, in
  warnRedundantConstraints.

* We don't report redundant givens for *every* implication; only for those which
  reply True to
  GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.warnRedundantGivens:
   ...
   - More subtly, in a function definition
     ...
     other set False in
        - GHC.IR.Haskell.TypeChecker.Binding.tcSpecPrag
        - GHC.IR.Haskell.TypeChecker.Binding.tcTySig

  This decision is taken in setImplicationStatus, rather than
  GHC.IR.Haskell.TypeChecker.Error so that we can discard implication
...
GHC.IR.Haskell.TypeChecker.Constraint.Simplifier prior to Oct 2014).
