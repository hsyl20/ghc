...
How is this implemented? It's complicated! So we'll step through it all:
 ...
 2) `GHC.IR.Haskell.TypeChecker.Constraint.Interact.matchClassInst` -- This module drives the instance resolution
 ...
 3) `GHC.IR.Haskell.TypeChecker.Constraint.Interact.doTopReactDict` -- Takes a dictionary / class constraint and
 ...
 4) `GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.simplifyTop`:
       ...
       * For unresolved constraints (all types), call `GHC.IR.Haskell.TypeChecker.Error.reportUnsolved`,
         while for resolved but unsafe overlapping dictionary constraints, call
         `GHC.IR.Haskell.TypeChecker.Error.warnAllUnsolved`. Both functions convert constraints into a
         warning message for the user.

       * In the case of `warnAllUnsolved` for resolved, but unsafe
         dictionary constraints, we collect the generated warning
         message (pop it) and call `GHC.IR.Haskell.TypeChecker.recordUnsafeInfer` to
         mark the module we are compiling as unsafe, passing the
         warning message along as the reason.

 5) `GHC.IR.Haskell.TypeChecker.Error.*Unsolved` -- Generates error messages for constraints by
 ...
 6) `GHC.IR.Haskell.TypeChecker.recordUnsafeInfer` -- Save the unsafe result and reason in an
...
data InferMode = ApplyMR          -- ^ Apply the monomorphism restriction,
               ...
               | EagerDefaulting  -- ^ See Note [TcRnExprMode] in GHC.IR.Haskell.TypeChecker.Module,
...
*  To find (a), when we have two Given constraints,
   ...
   the Ord dictionary.  This is done by GHC.IR.Haskell.TypeChecker.Constraint.Interact.solveOneFromTheOther
...
* GHC.IR.Haskell.TypeChecker.Error does the actual warning, in warnRedundantConstraints.

* We don't report redundant givens for *every* implication; only
  for those which reply True to GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.warnRedundantGivens:
  ...
  This decision is taken in setImplicationStatus, rather than GHC.IR.Haskell.TypeChecker.Error
