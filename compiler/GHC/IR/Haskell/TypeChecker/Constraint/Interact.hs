...
module GHC.IR.Haskell.TypeChecker.Constraint.Interact (
     ...
     -- for use in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier
     solveCallStack,      
  ) where
...
import GHC.IR.Haskell.Syntax.Type   ( HsIPName(..) )
import GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser as Canonicaliser
...
import GHC.Entity.ClassInstance     ( DFunInstType, lookupInstEnv
                                    , instanceDFunId )
...
thePipeline = [ ("canonicalization",        Canonicaliser.canonicalize)
...
we keep?  More subtle than you might think!
  ...
  * Constraints coming from the same level (i.e. same implication)
       ...
       - Keep the one that has a non-trivial evidence binding.
            ...
            the Ord dictionary.
         Why? See Note [Tracking redundant constraints] in
         GHC.IR.Haskell.TypeChecker.Constraint.Simplifier again.
...
See also Note [No touchables as FunEq RHS] in
GHC.IR.Haskell.TypeChecker.Constraint.Solver; avoiding double unifications is
the main reason we disallow touchable unification variables as RHS of type
family equations: F xis ~ alpha.
...
* For type-nat stuff the derived constraints include type families;
  ...
  massively confused the unflattening step
  (GHC.IR.Haskell.TypeChecker.Flatten.unflatten).
...
doFunTy clas ty arg_ty ret_ty
  = do { let preds = map (mk_typeable_pred clas) [arg_ty, ret_ty]
             ...
             build_ev _ = panic "TypeChecker.Constraint.Interact.doFunTy"
