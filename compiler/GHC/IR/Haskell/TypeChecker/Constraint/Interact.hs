...
module GHC.IR.Haskell.TypeChecker.Constraint.Interact (
     solveSimpleGivens,   -- Solves [Ct]
     solveSimpleWanteds,  -- Solves Cts

     solveCallStack,      -- for use in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier
  ) where

#include "HsVersions.h"

import GHC.Entity.BasicTypes ( SwapFlag(..), isSwapped,
                    infinity, IntWithInf, intGtLimit )
import GHC.IR.Haskell.Type ( HsIPName(..) )
import GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser
import GHC.IR.Haskell.TypeChecker.Flatten
import GHC.IR.Haskell.TypeChecker.Unify( canSolveByUnification )
import GHC.Entity.Var.Set
import GHC.Entity.Type as Type
import GHC.Entity.Kind( isConstraintKind )
import GHC.Entity.ClassInstance( DFunInstType, lookupInstEnv, instanceDFunId )
import GHC.Entity.Coercion.Axiom( sfInteractTop, sfInteractInert )

import GHC.IR.Haskell.TypeChecker.MutableType (newMetaTyVars)

import GHC.Entity.Var
import GHC.IR.Haskell.TypeChecker.Type
import GHC.Entity.Name
import GHC.Entity.RdrName ( lookupGRE_FieldLabel )
...
import GHC.Builtin.Primitive.Types    ( eqPrimTyCon, eqReprPrimTyCon )
import GHC.Entity.Id( idType, isNaughtyRecordSelector )
import GHC.Entity.Coercion.Axiom ( TypeEqn, CoAxiom(..), CoAxBranch(..), fromBranches )
import GHC.Entity.Class
import GHC.Entity.TypeConstructor
import GHC.Entity.DataConstructor( dataConWrapId )
import GHC.Entity.FieldLabel
import GHC.IR.Haskell.TypeChecker.FunctionalDependency
...
import GHC.IR.Haskell.TypeChecker.Constraint.Solver
import GHC.Data.Bag
...
the unflattening that takes place in step (1). See GHC.IR.Haskell.TypeChecker.Flatten,
...
thePipeline = [ ("canonicalization",        GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.canonicalize)
...
we keep?  More subtle than you might think!
  ...
  * Constraints come from different levels (different_level_strategy)
      ...
      - For everything else, we want to keep the outermost one.  Reason: that
        ...
        in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.
  ...
  * Constraints coming from the same level (i.e. same implication)
       ...
       - Keep the one that has a non-trivial evidence binding.
            ...
            the Ord dictionary.
         Why? See Note [Tracking redundant constraints] in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier again.
...
the instance that we have in scope:
    ...
    f :: forall b. C Int b => b -> Int -> Int
    f = \ (@ b) _ _ (x :: Int) ->
        case x of { GHC.Entity.Types.I# x1 -> GHC.Entity.Types.I# (GHC.Prim.+# x1 1#) }
...
produces the following core:
    ...
    f = \ (@ b) ($dC :: C [Int] b) (x :: b) ->
        GHC.Classes.$fEq[]_$s$c==
          (m @ [Int] @ b $dC x) (GHC.Entity.Types.[] @ Int)
...
--     See GHC.IR.Haskell.TypeChecker.Types.Ct invariants.
--
...
See also Note [No touchables as FunEq RHS] in GHC.IR.Haskell.TypeChecker.Constraint.Solver; avoiding
...
To achieve this required some refactoring of
GHC.IR.Haskell.TypeChecker.FunctionalDependency (nicer now!).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
c.f. Note [The flattening story] in GHC.IR.Haskell.TypeChecker.Flatten
...
* For type-nat stuff the derived constraints include type families;
  ...
  massively confused the unflattening step (GHC.IR.Haskell.TypeChecker.Flatten.unflatten).
...
-- check.
--
-- See Note [Safe Haskell Overlapping Instances] in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier
-- See Note [Safe Haskell Overlapping Instances Implementation] in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier
...
see Note [Simplifiable given constraints] in GHC.IR.Haskell.TypeChecker.Validity.
-}
...
-- in GHC.IR.Haskell.TypeChecker.Evidence.  The coercion happens in 2 steps:
--
...
doFunTy clas ty arg_ty ret_ty
  = do { let preds = map (mk_typeable_pred clas) [arg_ty, ret_ty]
             ...
             build_ev _ = panic "GHC.IR.Haskell.TypeChecker.Constraint.Interact.doFunTy"
