...
module GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser(
     canonicalize,
     unifyDerived,
...
import GHC.IR.Haskell.TypeChecker.Unify( swapOverTyVars, metaTyVarUpdateOK )
...
import GHC.Entity.TypeAndCoercion   -- cleverly decomposes types,
                                    -- good for completeness checking
import GHC.Entity.Coercion
import GHC.Entity.FamilyInstance                 ( FamInstEnvs )
import GHC.IR.Haskell.TypeChecker.FamilyInstance (tcTopNormaliseNewTypeTF_maybe)
...
(see type Ct in GHC.IR.Haskell.TypeChecker.Types).
...
3. If we have any remaining unsolved wanteds
   (see Note [When superclasses help] in GHC.IR.Haskell.TypeChecker.Types)
   ...
   proof.  This is done in
   GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.expandSuperClasses.

4. Go round to (2) again.  This loop (2,3,4) is implemented
   in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.simpl_loop.
...
Note [Instance and Given overlap] in GHC.IR.Haskell.TypeChecker.Constraint.Interact.
...
Furthermore, as explained in Note [NthCo and newtypes] in
GHC.Entity.TypeAndCoercion, we can't use NthCo on representational coercions
over newtypes. NthCo comes into play only when decomposing givens.
...
constraints: see Note [Instance and Given overlap] in
...
See Note [Rewrite insolubles] in GHC.IR.Haskell.TypeChecker.Constraint.Solver.
And if we don't do this there is a bad danger that
GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.applyTyVarDefaulting will find
a variable that has in fact been substituted.
...
canEqTyVar ev eq_rel swapped tv1 ps_ty1 (TyVarTy tv2) _
  ...
  = do { traceTcS "canEqTyVar" (ppr tv1 $$ ppr tv2 $$ ppr swapped)
         -- FM_Avoid commented out: see Note [Lazy flattening] in
