...
module GHC.IR.Haskell.TypeChecker.Constraint.Solver (
    ...
    checkWellStagedDFun,
    pprEq                                    
    -- Smaller utils, re-exported from TcM
    -- TODO (DV): these are only really used in the instance matcher in
    -- GHC.IR.Haskell.TypeChecker.Constraint.Simplifier. I am wondering if the
    -- whole instance matcher simply belongs here
) where
...
import qualified GHC.IR.Haskell.TypeChecker.Instantiation as TcM
import GHC.Entity.ClassInstance
import GHC.IR.Haskell.TypeChecker.FamilyInstance as FamInst
import GHC.Entity.FamilyInstance

import qualified GHC.IR.Haskell.TypeChecker as TcM
import qualified GHC.IR.Haskell.TypeChecker.MutableType as TcM
import qualified GHC.IR.Haskell.TypeChecker.Environment as TcM
...
import GHC.IR.Haskell.TypeChecker.Error   ( solverDepthErrorTcS )

import GHC.Entity.Name
import GHC.Entity.RdrName ( GlobalRdrEnv, GlobalRdrElt )
import qualified GHC.IR.Haskell.Renamer.Environment as TcM
...
The InertCans represents a collection of constraints with the following properties:
  ...
  * CTyEqCan equalities: see Note [Applying the inert substitution]
                         in GHC.IR.Haskell.TypeChecker.Flatten
...
Conditions (K1-K3) are established by
GHC.IR.Haskell.TypeChecker.Constraint.Solver.kickOutRewritable
...
rewrite Wanteds] in GHC.IR.Haskell.TypeChecker.Types), we may miss some
opportunities for solving.
Here's a classic example (indexed-types/should_fail/T4093a)
...
-- in GHC.IR.Haskell.TypeChecker.Constraint.Interact.
...
panicTcS doc = pprPanic "GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser" doc
...
-- Getters and setters of GHC.IR.Haskell.TypeChecker.Environment fields
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
-- See Note [Bind new Givens immediately] in GHC.IR.Haskell.TypeChecker.Types
...
-- --------- Check done in GHC.IR.Haskell.TypeChecker.Constraint.Interact.selectNewWorkItem???? ---------
...
from which we get the implication
   (forall a. t1 ~ t2)
See GHC.IR.Haskell.TypeChecker.Constraint.Solver.deferTcSForAllEq
-}
...
deferTcSForAllEq role loc kind_cos (bndrs1,body1) (bndrs2,body2)
 = do { let tvs1'  = zipWithEqual "deferTcSForAllEq"
      ...
      ; ev_binds <- newTcEvBinds
           ...
           -- but GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.setImplicationStatus
