...
module GHC.IR.Haskell.TypeChecker.Type (
  --------------------------------
  -- Types
...
import GHC.Entity.Name as Name -- hiding (varName)
...
-- See Note [ExpType] in GHC.IR.Haskell.TypeChecker.MutableType, where you'll also find manipulators.
data ExpType = Check TcType
             | Infer !InferResult

data InferResult
  = IR { ir_uniq :: Unique  -- For debugging only
       , ir_lvl  :: TcLevel -- See Note [TcLevel of ExpType] in GHC.IR.Haskell.TypeChecker.MutableType
...
-- GHC.IR.Haskell.TypeChecker.Expression .hs-boot file.
data SyntaxOpType
  = SynAny     -- ^ Any type
...
smart-app checking code --- see GHC.IR.Haskell.TypeChecker.Expression.tcIdApp
...
-- See also Note [Scope-check inferred kinds] in GHC.IR.Haskell.TypeChecker.UserType
...
-- Called only in GHC.IR.Haskell.TypeChecker.Error; see Note [Runtime skolems] there
isRuntimeUnkSkol x
...
-- See Note [Type-checking inside the knot] in GHC.IR.Haskell.TypeChecker.UserType
...
-- decomposing tycon applications] in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser for details.
...
-- equality] (in GHC.Entity.TypeAndCoercion) as `eqType`, but Type.eqType believes (* ==
...
-- This function is here rather than in GHC.IR.Haskell.TypeChecker.Validity because it is
-- called from GHC.IR.Haskell.TypeChecker.Constraint.Simplifier, which itself is imported by GHC.IR.Haskell.TypeChecker.Validity
...
See also GHC.IR.Haskell.TypeChecker.TypeDeclaration.checkClassCycles.
...
-- GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser for the motivation for this function.
