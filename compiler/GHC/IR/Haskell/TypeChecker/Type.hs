...
-- See Note [ExpType] in GHC.IR.Haskell.TypeChecker.MutableType, where you'll
...
data InferResult
  = IR { ir_uniq :: Unique  -- For debugging only
       , ir_lvl  :: TcLevel -- See Note [TcLevel of ExpType] 
                            -- in GHC.IR.Haskell.TypeChecker.MutableType
...
-- Called only in GHC.IR.Haskell.TypeChecker.Error; see Note [Runtime skolems]
-- there
isRuntimeUnkSkol x
...
-- GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser for details.
...
-- equality] (in GHC.Entity.TypeAndCoercion) as `eqType`, but Type.eqType
...
-- This function is here rather than in GHC.IR.Haskell.TypeChecker.Validity
-- because it is called from GHC.IR.Haskell.TypeChecker.Constraint.Simplifier,
-- which itself is imported by GHC.IR.Haskell.TypeChecker.Validity
...
-- GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser for the motivation for
-- this function.
