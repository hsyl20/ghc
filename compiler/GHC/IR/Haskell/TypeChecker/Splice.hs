...
module GHC.IR.Haskell.TypeChecker.Splice(
...
import GHC.IR.Haskell.TypeChecker.Expression
...
import GHC.IR.Haskell.TypeChecker.Syntax
import GHC.IR.Haskell.TypeChecker.Constraint.Simplifier
import GHC.Entity.Type as Type
...
import GHC.IR.Interface.TypeChecker
import GHC.Entity.TypeAndCoercion
import GHC.IR.Haskell.TypeChecker.FamilyInstance
...
import GHC.Entity.OccName as OccName
import GHC.Config.Hooks
import GHC.Entity.Var
import GHC.Entity.Module
import GHC.IR.Interface.Loader
import GHC.Entity.Class
import GHC.Entity.TypeConstructor
import GHC.Entity.Coercion.Axiom
import GHC.Entity.PatternSynonym
import GHC.Entity.ConstructorLike
import GHC.Entity.DataConstructor as DataCon
import GHC.IR.Haskell.TypeChecker.Evidence( TcEvBinds(..) )
...
import GHC.Entity.Var.Set ( isEmptyVarSet, filterVarSet, mkVarSet, elemVarSet )
import Data.List          ( find )
import Data.Maybe
import GHC.Data.FastString
import GHC.Entity.BasicTypes hiding( SuccessFlag(..) )
import qualified GHC.Entity.BasicTypes as BasicTypes
...
In both cases, desugaring happens like this:
  ...
  * When GHC.Compiler.HaskellToCore.Splices (used to desugar the body of the
    bracket) comes across a splice, it looks up the splice's Name, n, in the
    ds_meta envt, to find an (HsExpr Id) that should be substituted for the
    splice; it just desugars it to get a CoreExpr
    (GHC.Compiler.HaskellToCore.Splices.repSplice).
...
tcSpliceExpr splice@(HsTypedSplice _ name expr) res_ty
  = addErrCtxt (spliceCtxtDoc splice) $
    ...
    ; case stage of
          ...
          RunSplice _          ->
            -- See Note [RunSplice ThLevel] in "GHC.IR.Haskell.TypeChecker.Types".
...
"GHC.IR.Haskell.Renamer.Splice"). Thus after executing the splice, we move the
finalizers to the finalizer list in the global environment and set them to use
the current local environment (with 'addModFinalizersWithLclEnv').
...
runMeta' show_code ppr_hs run_and_convert expr
  = do  { traceTc "About to run" (ppr expr)
        ...
        ; either_hval <- tryM $ liftIO $
                         GHC.Program.Main.hscCompileCoreExpr hsc_env src_span
                                                             ds_expr
        ; case either_hval of {
...
like that.  Here's how it's processed:
  ...
  * The TcM monad is an instance of Quasi (see
    GHC.IR.Haskell.TypeChecker.Splice), and it implements (qReport True s) by
    using addErr to add an error message to the bag of errors.  The 'fail' in
    TcM raises an IOEnvFailure exception
...
-- GHC.IR.Haskell.TypeChecker.Environment.tcLookup; specialised mainly in that
       ...
       ; main_con <-
           if | not (null fields) && not isGadtDataCon ->
              ...
              | not (null fields) -> do
                ...
                -- See Note [Infix GADT constructors] in
...
annotThType :: Bool   -- True <=> annotate
            -> GHC.Entity.TypeAndCoercion.Type -> TH.Type -> TcM TH.Type
      ...
      DataFamilyInst rep_tc ->
        do { let rep_tvs = tyConTyVars rep_tc
                 fam' = reifyName fam
                   ...
                   -- in GHC.IR.Haskell.TypeChecker.Instance
...
------------------------------
reifyType :: GHC.Entity.TypeAndCoercion.Type -> TcM TH.Type
...
reify_for_all :: GHC.Entity.TypeAndCoercion.Type -> TcM TH.Type
reify_for_all ty
  = do { cxt' <- reifyCxt cxt;
       ; tau' <- reifyType tau
...
reifyTyLit :: GHC.Entity.TypeAndCoercion.TyLit -> TcM TH.TyLit
...
reify_kc_app :: TyCon -> [GHC.Entity.TypeAndCoercion.Kind] -> TcM TH.Kind
reify_kc_app kc kis
...
reifyPred :: GHC.Entity.TypeAndCoercion.PredType -> TcM TH.Pred
reifyPred ty
