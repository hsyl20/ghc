...
module GHC.IR.Haskell.TypeChecker.Instance ( tcInstDecls1, tcInstDeclsDeriv, tcInstDecls2 ) where
...
import GHC.IR.Haskell.TypeChecker.ClassDeclaration( tcClassDecl2, tcATDefault,
                   HsSigFun, mkHsSigFun,
...
import GHC.IR.Haskell.TypeChecker.Syntax    ( zonkTyBndrsX, emptyZonkEnv
...
import GHC.IR.Core.Transformer.Inliner ( mkInlineUnfoldingWithArity, mkDFunUnfolding )
...
* If op1_i is marked INLINE by the user there's a danger that we won't
  ...
  unfoldings. See Note [RULEs enabled in SimplGently] in GHC.IR.Core.Transformer.Simplifier.Utils
 ...
 * We make GHC.IR.Core.Transformer.Inliner.exprIsConApp_maybe spot a DFunUnfolding and return
...
tcInstDecls2 tycl_decls inst_decls
  = do  { -- (a) Default methods from class decls
        ...
        ; let dm_ids = collectHsBindsBinders dm_binds
              ...
              -- (they were arready added in GHC.IR.Haskell.TypeChecker.TypeDeclaration.tcAddImplicits)
 ...
 * Overall instance implication. There is an overall enclosing
   ...
   and superclasses.  See GHC.IR.Haskell.TypeChecker.Constraint.Simplifier Note [Tracking redundant
   constraints]
...
the (type of) the dictionary, defined by GHC.IR.Haskell.TypeChecker.Validity.sizeTypes.
...
Answer:
  ...
  * (sc2) ScOrigin wanted constraints can't be solved from a
    ...
    implemented by GHC.IR.Haskell.TypeChecker.Constraint.Interact.prohibitedSuperClassSolve
  ...
  * When we make a superclass selection from InstSkol we use
    ...
    in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.newSCWorkFromFlavored
...
tcMethod
- Make the method bindings, as a [(NonRec, GHC.IR.Haskell.Syntax.Binding)], one per method
...
mk_meth_spec_prags :: Id -> [LTcSpecPrag] -> [LTcSpecPrag] -> TcSpecPrags
        ...
        --     the correct wrapper.
        -- See Note [Handling SPECIALISE pragmas] in GHC.IR.Haskell.TypeChecker.Binding
