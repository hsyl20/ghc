...
import GHC.Entity.TypeAndCoercion         ( Type(..), Coercion(..)
                                          , UnivCoProvenance(..) )
...
result type (modulo alpha conversion).
[Checked in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration.checkValidTyCon]
E.g.
        data T where
          T1 { f :: Maybe a } :: T [a]
