...
module GHC.IR.Haskell.TypeChecker.TypeDeclaration(
        RolesInfo,
        inferRoles,
        checkSynCycles,
        checkClassCycles,
...
import GHC.IR.Haskell.TypeChecker.Binding( tcRecSelBinds )
import GHC.Entity.TypeAndCoercion( Type(..), Coercion(..), UnivCoProvenance(..) )
...
result type (modulo alpha conversion).  [Checked in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration.checkValidTyCon]
E.g.
        data T where
          T1 { f :: Maybe a } :: T [a]
