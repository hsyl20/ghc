...
module GHC.IR.Haskell.TypeChecker.Unify (
...
-- question. (See GHC.IR.Haskell.TypeChecker.Expression.tcArgs.) This argument
-- is the reversed list of arguments already seen (that is, not part of the
-- TcSigmaType passed in elsewhere).
...
-- See also Note [ExpType] in GHC.IR.Haskell.TypeChecker.MutableType
...
* ir_inst = False: do not instantantiate
  ...
  We'll call GHC.IR.Haskell.TypeChecker.Expression.tcInferFun to infer the type
  of the (let .. in f) And we don't want to instantite the type of 'f' when we
  reach it, else the outer visible type application won't work
-}
...
buildImplication skol_info skol_tvs given thing_inside
  = do { tc_lvl <- getTcLevel
      ...
      -- in GHC.IR.Haskell.TypeChecker.Error.
         else
...
The same applies later on in the constraint interaction code; see
GHC.IR.Haskell.TypeChecker.Constraint.Interact, function @occ_check_ok@.

Note [Non-TcTyVars in GHC.IR.Haskell.TypeChecker.Unify]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
with "tc" in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration"). By this time,
all the kind-vars are proper TyVars, not TcTyVars, get some kind unification
must happen.
...
The two variants of the function are to support
GHC.IR.Haskell.TypeChecker.Unify.checkTauTvUpdate, which wants to prevent
unification with type families.
For more on this point, see Note [Prevent unification with type families] in
