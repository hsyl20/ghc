...
-- question. (See GHC.IR.Haskell.TypeChecker.Expression.tcArgs.) This argument is the reversed list of
...
* ir_inst = False: do not instantantiate
  ...
  We'll call GHC.IR.Haskell.TypeChecker.Expression.tcInferFun to infer the type of the (let .. in f)
...
The same applies later on in the constraint interaction code; see GHC.IR.Haskell.TypeChecker.Constraint.Interact,
...
with "tc" in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration"). By this time, all the kind-vars are proper
...
The two variants of the function are to support GHC.IR.Haskell.TypeChecker.Unify.checkTauTvUpdate,
which wants to prevent unification with type families. For more on this
point, see Note [Prevent unification with type families] in GHC.IR.Haskell.TypeChecker.Unify.
