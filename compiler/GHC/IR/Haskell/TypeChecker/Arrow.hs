...
module GHC.IR.Haskell.TypeChecker.Arrow ( tcProc ) where

import {-# SOURCE #-}   GHC.IR.Haskell.TypeChecker.Expression( tcMonoExpr, tcInferRho, tcSyntaxOp, tcCheckId, tcPolyExpr )

import GHC.IR.Haskell.Syntax
import GHC.IR.Haskell.TypeChecker.Matches
import GHC.IR.Haskell.TypeChecker.Syntax( hsLPatType )
...
import GHC.Entity.Id( mkLocalId )
...
tc_cmd env cmd@(HsCmdArrApp fun arg _ ho_app lr) (_, res_ty)
  ...
  where
       ...
       -- See Note [Escaping the arrow scope] in GHC.IR.Haskell.TypeChecker.Types
