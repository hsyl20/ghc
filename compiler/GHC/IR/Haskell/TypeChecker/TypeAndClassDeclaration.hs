...
module GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration (
        tcTyAndClassDecls, tcAddImplicits,

        -- Functions used by GHC.IR.Haskell.TypeChecker.Instance to check
...
import GHC.IR.Haskell.TypeChecker.Validity
import GHC.IR.Haskell.TypeChecker.Syntax
import GHC.IR.Haskell.TypeChecker.TypeDeclaration
import GHC.IR.Haskell.TypeChecker.ClassDeclaration
import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Instance( tcInstDecls1 )
import GHC.IR.Haskell.TypeChecker.Deriving   ( DerivInfo )
...
import GHC.Entity.TypeAndCoercion   -- for checkValidRoles
...
-- See Note [TyClGroups and dependency analysis] in
...
Then:
  ...
  * During GHC.IR.Haskell.TypeChecker.UserType.kcTyVar we look in the *local*
    env, to get the known kind for T.

  * But in GHC.IR.Haskell.TypeChecker.UserType.ds_type (and ds_var_app in
    particular) we look in the *global* env to get the TyCon. But we must be
    careful not to force the TyCon or we'll get a loop.
  ...
  = -- Closed type families are a little tricky, because they contain the definition
    ...
    do { traceTc "Closed type family:" (ppr tc_name)
       ...
       ; branches <- mapM (tcTyFamInstEqn fam_tc_shape Nothing) eqns
         ...
         -- for this afterwards, in
         -- GHC.IR.Haskell.TypeChecker.Validity.checkValidCoAxiom
         -- Example: tc265
         ...
         -- differentiate names.
         -- See [Zonking inside the knot] in GHC.IR.Haskell.TypeChecker.UserType
...
tcTyFamInstEqn :: FamTyConShape -> Maybe ClsInstInfo -> LTyFamInstEqn GhcRn
               -> TcM CoAxBranch
-- Needs to be here, not in GHC.IR.Haskell.TypeChecker.Instance, because closed
-- families (typechecked here) have TyFamInstEqns
  ...
  = do { -- Kind-check and quantify
         ...
         do { (insting_subst, _leftover_binders, args, leftovers, n)
            ...
            ; case leftovers of
                ...
                _       -> return ()
              -- don't worry about leftover_binders;
              -- GHC.IR.Haskell.TypeChecker.Validity catches them
...
checkValidTyCon tc
  ...
  where
    ...
    -- See Note [GADT record selectors] in GHC.IR.Haskell.TypeChecker.TypeDeclaration
  ...
  = do { -- strip off the duplicates and look for ill-scoped things
       ...
       ; checkValidTelescope (pprTyVars vis_tvs) stripped_tvs extra
         `and_if_that_doesn't_error`
           ...
           -- It reports errors around Note [Dependent LHsQTyVars] in GHC.IR.Haskell.TypeChecker.UserType
         when duplicate_vars (
    ...
    check_op constrained_class_methods (sel_id, dm)
      = setSrcSpan (getSrcSpan sel_id) $
        ...
        ; checkValidType ctxt op_ty
           ...
           -- GHC.Compiler.HaskellToCore.Monad
...
2. Internally, GHC uses TypeApplications to instantiate the default method in an
   instance.  See Note [Default methods in instances] in
   GHC.IR.Haskell.TypeChecker.Instance.  Thus, GHC needs to know exactly what
   the universally quantified type variables are, and when instantiated that
   way, the default method's type must match the expected type.
...
-- -dcore-lint is enabled. See Note [Role inference] in GHC.IR.Haskell.TypeChecker.TypeDeclaration
    ...
    check_dc_roles datacon
      = do { traceTc "check_dc_roles" (ppr datacon <+> ppr (tyConRoles tc))
           ; mapM_ (check_ty_roles role_env Representational) $
                    ...
                    -- See Note [Role-checking data constructor arguments] in
                    -- GHC.IR.Haskell.TypeChecker.TypeDeclaration
      where
