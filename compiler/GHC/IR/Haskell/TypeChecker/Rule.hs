...
GHC.IR.Haskell.TypeChecker.Rule: Typechecking transformation rules
-}

{-# LANGUAGE ViewPatterns #-}
{-# LANGUAGE TypeFamilies #-}

module GHC.IR.Haskell.TypeChecker.Rule ( tcRules ) where
...
import GHC.IR.Haskell.TypeChecker.Unify( buildImplicationFor )
import GHC.IR.Haskell.TypeChecker.Evidence( mkTcCoVarCo )
import GHC.Entity.Type
import GHC.Entity.Id
import GHC.Entity.Var( EvVar )
import GHC.Entity.BasicTypes       ( RuleName )
...
tcRule (HsRule name act hs_bndrs lhs fv_lhs rhs fv_rhs)
  = addErrCtxt (ruleCtxt $ snd $ unLoc name)  $
    do { traceTc "---- Rule ------" (pprFullRuleName name)
       ...
       -- c.f. GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.simplifyInfer
       ...
       -- during zonking (see GHC.IR.Haskell.TypeChecker.Syntax.zonkRule)
...
--              a::*, x :: a->a
  = do  { let ctxt = RuleSigCtxt name
        ...
        ; let id  = mkLocalIdOrCoVar name id_ty
                    -- See Note [Pattern signature binders] in GHC.IR.Haskell.TypeChecker.UserType
