...
module GHC.IR.Haskell.TypeChecker.Environment(
...
import GHC.Entity.DataConstructor ( DataCon )
import GHC.Entity.PatternSynonym  ( PatSyn )
...
--
-- The Id is never a DataCon. (Why does that matter? see
-- GHC.IR.Haskell.TypeChecker.Expression.tcId)
tcLookupId name = do
    thing <- tcLookup name
    case thing of
...
to the environment. Then GHC.IR.Haskell.TypeChecker.UserType.tcTyVar will find A
in the kind environment, and will give a 'wrongThingErr' as a result.  But the
...
-- See Note [Placeholder PatSyn kinds] in GHC.IR.Haskell.TypeChecker.Binding
