...
import GHC.IR.Haskell.Renamer.Splice      ( rnTopSpliceDecls, traceSplice
                                          , SpliceInfo(..) )
...
tcRnModuleTcRnM hsc_env hsc_src
 ...
 = setSrcSpan loc $
   do { let { explicit_mod_hdr = isJust maybe_mod } ;
        ...
        -- interface files, via
        -- GHC.IR.Haskell.TypeChecker.Environment.setGlobalTypeEnv
...
tcRnHsBootDecls hsc_src decls
   = do { (first_group, group_tail) <- findSplice decls
        ...
        -- See Note [Extra dependencies from .hs-boot files] in
        -- GHC.IR.Haskell.Renamer
...
checkHiBootIface'
  ...
  = do  { traceTc "checkHiBootIface" $ vcat
        ...
        ; unless (null boot_fam_insts) $
            panic ("checkHiBootIface: Cannot handle family " ++
...
tcGhciStmts stmts
 = do { ioTyCon <- tcLookupTyCon ioTyConName ;
        ...
        traceTc "tcGhciStmts: tc stmts" empty ;
        ...
        traceTc "tcGhciStmts: simplify ctxt" empty ;
        const_binds <- checkNoErrs (simplifyInteractive lie) ;
                -- checkNoErrs ensures that the plan fails if context redn fails

        traceTc "tcGhciStmts: done" empty ;
...
tcRnExpr hsc_env mode rdr_expr
  = runTcInteractive hsc_env $
    ...
    -- same as of a bound expression
    -- (GHC.IR.Haskell.TypeChecker.Binding.mkInferredPolyId). See Trac #10321
