...
\section[GHC.IR.Haskell.TypeChecker.Binding]{GHC.IR.Haskell.TypeChecker.Binding}
-}
...
module GHC.IR.Haskell.TypeChecker.Binding ( tcLocalBinds, tcTopBinds, tcRecSelBinds,
                 tcHsBootSigs, tcPolyCheck,
                 tcVectDecls, addTypecheckedBinds,
                 chooseInferredQuantifiers,
                 badBootDeclErr ) where

import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Matches        ( tcGRHSsPat
                                                               , tcMatchesFun )
import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Expression     ( tcMonoExpr )
import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.PatternSynonym
                           ( tcInferPatSynDecl, tcCheckPatSynDecl
                           , tcPatSynBuilderBind )
import GHC.IR.Core.Syntax (Tickish (..))
import GHC.Entity.CostCentre (mkUserCC)
import GHC.Config.Flags
import GHC.Data.FastString
import GHC.IR.Haskell.Syntax
import GHC.Entity.Types( isHsBootOrSig )
import GHC.IR.Haskell.TypeChecker.Signature
import GHC.IR.Haskell.TypeChecker
import GHC.IR.Haskell.TypeChecker.Environment
import GHC.IR.Haskell.TypeChecker.Unify
import GHC.IR.Haskell.TypeChecker.Constraint.Simplifier
import GHC.IR.Haskell.TypeChecker.Evidence
import GHC.IR.Haskell.TypeChecker.UserType
import GHC.IR.Haskell.TypeChecker.Pattern
import GHC.IR.Haskell.TypeChecker.MutableType
import GHC.Entity.FamilyInstance( normaliseType )
import GHC.IR.Haskell.TypeChecker.FamilyInstance( tcGetFamInstEnvs )
import GHC.Entity.TypeConstructor
import GHC.IR.Haskell.TypeChecker.Type
import GHC.Entity.Type( mkStrLitTy, tidyOpenType, mkTyVarBinder, splitTyConApp_maybe)
import GHC.Builtin.Primitive.Types
import GHC.Builtin.Types( cTupleTyConName )
import GHC.Entity.Id
import GHC.Entity.Var as Var
import GHC.Entity.Var.Set
import GHC.Entity.Var.Environment( TidyEnv )
...
import GHC.IR.Haskell.TypeChecker.Validity (checkValidType)
import GHC.Entity.Unique (getUnique)
import GHC.Entity.Unique.FiniteMap
import GHC.Entity.Unique.Set
import qualified GHC.LanguageExtensions as LangExt
import GHC.Entity.ConstructorLike
...
@tcBindsAndThen@ typechecks a @GHC.IR.Haskell.Syntax.Binding@.  The "and then" part is
because it needs to know something about the {\em usage} of the things bound, so
that it can create specialisations of them.  So @tcBindsAndThen@ takes a
function which, given an extended environment, E, typechecks the scope of the
bindings returning a typechecked thing and (most important) an LIE.  It is this
LIE which is then used as the basis for specialising the things bound.
        ...
        ; tcExtendLetEnvIds top_lvl [(idName id, id) | id <- poly_ids] $ do
            { (binds', (extra_binds', thing)) <- tcBindGroups top_lvl sig_fn prag_fn binds $ do
                   { thing <- thing_inside
                     ...
                     --     in GHC.IR.Haskell.Renamer.Binding
...
tc_group top_lvl sig_fn prag_fn (Recursive, binds) closed thing_inside
  =     -- To maximise polymorphism, we do a new
        ...
        -- See Note [Polymorphic recursion] in GHC.IR.Haskell.Syntax.Binding.
...
-- type environment; see the invariant on
-- GHC.IR.Haskell.TypeChecker.Environment.tcExtendIdEnv
  ...
  = checkNoErrs $  -- The checkNoErrs ensures that if the type is ambiguous
    ...
    do { fam_envs <- tcGetFamInstEnvs
       ; let (_co, mono_ty') = normaliseType fam_envs Nominal mono_ty
               ...
               -- (see Note [Lazy flattening] in
               -- GHC.IR.Haskell.TypeChecker.Flatten) so do it here to make it as
...
tcVect (HsVectTypeOut _ _ _)
  = panic "GHC.IR.Haskell.TypeChecker.Binding.tcVect: Unexpected 'HsVectTypeOut'"
...
tcVect (HsVectClassOut _)
  = panic "GHC.IR.Haskell.TypeChecker.Binding.tcVect: Unexpected 'HsVectClassOut'"
...
tcVect (HsVectInstOut _)
  = panic "GHC.IR.Haskell.TypeChecker.Binding.tcVect: Unexpected 'HsVectInstOut'"
...
@tcMonoBinds@ deals with a perhaps-recursive group of GHC.IR.Haskell.Syntax.Binding.
...
tcMonoBinds is_rec sig_fn no_gen
  ...
  =     -- Note [Single function non-recursive binding special-case]
    ...
    do  { ((co_fn, matches'), rhs_ty)
            <- tcInferInst $ \ exp_ty ->
                  -- tcInferInst: see GHC.IR.Haskell.TypeChecker.Unify,
...
We typecheck pattern bindings as follows.  First tcLhs does this:
  ...
  3. Invoke tcLetPat to typecheck the pattern.
     ...
     - We pass in the current TcLevel.  This is captured by
       GHC.IR.Haskell.TypeChecker.Pattern.tcLetPat, and put into the pc_lvl field
       of PatCtxt, in PatEnv.
     ...
     - When we come to a binder (GHC.IR.Haskell.TypeChecker.Pattern.tcPatBndr),
       it looks it up in the little environment (the pc_sig_fn field of
       PatCtxt).
...
isClosedBndrGroup binds = do
  ...
  where
    ...
    -- See Note [Bindings with closed types] in GHC.IR.Haskell.TypeChecker.Types
