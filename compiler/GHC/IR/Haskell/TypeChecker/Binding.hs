...
\section[TcBinds]{Typecheck bindings}
-}
...
module GHC.IR.Haskell.TypeChecker.Binding
   ( tcLocalBinds, tcTopBinds, tcRecSelBinds
...
import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Matches
                           ( tcGRHSsPat , tcMatchesFun )
import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Expression
                           ( tcMonoExpr )
...
import GHC.Entity.Type                             ( mkStrLitTy, tidyOpenType
                                                   , mkTyVarBinder
                                                   , splitTyConApp_maybe)
...
@tcBindsAndThen@ typechecks a @GHC.IR.Haskell.Syntax.Binding@.  The "and then"
part is because it needs to know something about the {\em usage} of the things
bound, so that it can create specialisations of them.  So @tcBindsAndThen@ takes
a function which, given an extended environment, E, typechecks the scope of the
  ...
  = checkNoErrs $  -- The checkNoErrs ensures that if the type is ambiguous
    ...
    do { fam_envs <- tcGetFamInstEnvs
       ; let (_co, mono_ty') = normaliseType fam_envs Nominal mono_ty
               ...
               -- GHC.IR.Haskell.TypeChecker.Flatten) so do it here to make it
...
@tcMonoBinds@ deals with a perhaps-recursive group of
