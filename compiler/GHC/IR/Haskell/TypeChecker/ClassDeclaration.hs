...
module GHC.IR.Haskell.TypeChecker.ClassDeclaration ( tcClassSigs, tcClassDecl2,
...
import GHC.IR.Haskell.TypeChecker.Evidence ( idHsWrapper )
...
import GHC.Entity.Type     ( getClassPredTys_maybe, piResultTys )
...
and so it is. See GHC.IR.Haskell.TypeChecker.Binding.tcMonoBinds (with type-sig case).
