...
module GHC.IR.Haskell.TypeChecker.Error(
...
import GHC.IR.Haskell.TypeChecker.Unify     ( occCheckForErrors
                                            , OccCheckResult(..) )
...
import GHC.Entity.FamilyInstance            ( flattenTys )
...
import GHC.IR.Haskell.Syntax.Expression  ( UnboundVar(..) )
import GHC.IR.Haskell.Syntax.Binding     ( PatSynBind(..) )
import GHC.Entity.Name
import GHC.Entity.RdrName
               ( lookupGlobalRdrEnv, lookupGRE_Name, GlobalRdrEnv
...
import GHC.Entity.ConstructorLike  ( ConLike(..), conLikeWrapId_maybe )
import GHC.Utils
import GHC.Entity.Types            (HscEnv, lookupTypeHscEnv, TypeEnv
                                   , lookupTypeEnv )
import GHC.Entity.Name.Environment (lookupNameEnv)
...
import GHC.Entity.Var.Free ( fvVarList, unionFV )
...
we run into a type mismatch in GHC.IR.Haskell.TypeChecker.Unify, we normally
just emit an error. But it is always safe to defer the mismatch to the main
constraint solver. If we do that, `a` will get transformed into
...
It does this by keeping track of which errors correspond to which coercion in
GHC.IR.Haskell.TypeChecker.Error.
GHC.IR.Haskell.TypeChecker.Error.reportTidyWanteds does not print the errors and
does not fail if -fdefer-type-errors is on, so that we can continue compilation.
The errors are turned into warnings in `reportUnsolved`.
-}
...
--      (see Note [Fail fast on kind errors] in
...
reportWanteds ctxt tc_lvl (WC { wc_simple = simples, wc_insol = insols, wc_impl = implics })
 ...
 where
    ...
    -- (see GHC.IR.Haskell.TypeChecker.Types.trulyInsoluble) is caught here, otherwise
...
a subtype of the id. This can be done using `tcSubType` from
`GHC.IR.Haskell.TypeChecker.Unify` and `tcCheckSatisfiability` in
`GHC.IR.Haskell.TypeChecker.Constraint.Simplifier`.

Unfortunately, `GHC.IR.Haskell.TypeChecker.Constraint.Simplifier` uses
`GHC.IR.Haskell.TypeChecker.Error` to report errors found during constraint
checking, so checking for subsumption in holes would involve shuffling some code
around in `GHC.IR.Haskell.TypeChecker.Constraint.Simplifier`, to make a
non-error reporting constraint satisfiability checker which could then be used
for checking whether a given id satisfies the constraints imposed by the hole.
...
mkTyVarEqErr' dflags ctxt report ct oriented tv1 ty2
  ...
  , isUserSkolem ctxt tv1
    -- ty2 won't be a meta-tyvar, or else the thing would be oriented the other
    -- way round; see
    -- GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.canEqTyVarTyVar
...
Re-flattening is pretty easy, because we don't need to keep track of evidence.
We don't re-use the code in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser
because that's in the TcS monad, and we are in TcM here.
...
are created by in GHC.Interactive.ClosureInspect.zonkRTTIType.
