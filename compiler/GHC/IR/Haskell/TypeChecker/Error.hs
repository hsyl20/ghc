...
import GHC.IR.Haskell.TypeChecker.Unify( occCheckForErrors, OccCheckResult(..) )
...
import GHC.Entity.RdrName ( lookupGlobalRdrEnv, lookupGRE_Name, GlobalRdrEnv
...
import GHC.Utils
import GHC.Entity.Types (HscEnv, lookupTypeHscEnv, TypeEnv, lookupTypeEnv )
...
we run into a type mismatch in GHC.IR.Haskell.TypeChecker.Unify, we normally just emit an error. But it
...
in GHC.IR.Haskell.TypeChecker.Error. GHC.IR.Haskell.TypeChecker.Error.reportTidyWanteds does not print the errors
...
--      (see Note [Fail fast on kind errors] in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier]
...
a subtype of the id. This can be done using `tcSubType` from `GHC.IR.Haskell.TypeChecker.Unify` and
`tcCheckSatisfiability` in `GHC.IR.Haskell.TypeChecker.Constraint.Simplifier`.  Unfortunately, `GHC.IR.Haskell.TypeChecker.Constraint.Simplifier` uses
`GHC.IR.Haskell.TypeChecker.Error` to report errors found during constraint checking, so checking for
subsumption in holes would involve shuffling some code around in `GHC.IR.Haskell.TypeChecker.Constraint.Simplifier`,
...
mkTyVarEqErr' dflags ctxt report ct oriented tv1 ty2
  ...
  , isUserSkolem ctxt tv1   -- ty2 won't be a meta-tyvar, or else the thing would
                            ...
                            -- see GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.canEqTyVarTyVar
  ...
  | OC_Occurs <- occ_check_expand
    ...
    -- See Note [Occurs check error] in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser
...
something like
    ...
    There are instances for similar types:
      instance Num GHC.Entity.Types.Int -- Defined in ‘GHC.Num’
...
evidence.  We don't re-use the code in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser because that's in
