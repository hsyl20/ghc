...
module GHC.IR.Haskell.TypeChecker.Validity (
...
-- friends:
import GHC.IR.Haskell.TypeChecker.Unify                 ( tcSubType_NC )
import GHC.IR.Haskell.TypeChecker.Constraint.Simplifier (simplifyAmbiguityCheck)
import GHC.Entity.TypeAndCoercion
import GHC.IR.Haskell.TypeChecker.Type hiding ( sizeType, sizeTypes )
...
import GHC.IR.Haskell.TypeChecker              -- TcType, amongst others
import GHC.IR.Haskell.TypeChecker.Environment  ( tcGetInstEnvs )
import GHC.IR.Haskell.TypeChecker.FunctionalDependency
import GHC.Entity.ClassInstance                 ( InstMatch, lookupInstEnv )
import GHC.Entity.FamilyInstance                ( isDominatedBy
                                                , injectiveBranches
                                                , InjectivityCheckResult(..) )
import GHC.IR.Haskell.TypeChecker.FamilyInstance( makeInjectivityErrors )
import GHC.Entity.Name
import GHC.Entity.Var.Environment
import GHC.Entity.Var.Set
import GHC.Entity.Unique.Set
import GHC.Entity.Var         ( TyVarBndr(..), mkTyVar )
...
import GHC.Entity.Unique      ( mkAlphaTyVarUnique )
...
unambiguous. See Note [Impedance matching] in
GHC.IR.Haskell.TypeChecker.Binding.
...
"tau-tvs" stuff.  This is done in the function
'GHC.IR.Haskell.TypeChecker.FunctionalDependency.grow'.
-}
...
-- See Note [Extended defaults]" in GHC.IR.Haskell.TypeChecker.Environment
...
A type signature like
   f :: Eq [(a,b)] => a -> b
is very fragile, for reasons described at length in
GHC.IR.Haskell.TypeChecker.Constraint.Interact Note [Instance and Given
overlap].  As that Note discusses, for the most part the clever stuff in
GHC.IR.Haskell.TypeChecker.Constraint.Interact means that we don't use a
top-level instance if a local Given might fire, so there is no fragility. But if
we /infer/ the type of a local let-binding, things can go wrong (Trac #11948 is
an example, discussed in the Note).
...
The warning only fires if the constraint in the signature matches the top-level
instances in only one way, and with no unifiers -- that is, under the same
circumstances that GHC.IR.Haskell.TypeChecker.Constraint.Interact.matchInstEnv
fires an interaction with the top level instances.
For example (Trac #13526), consider
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The HasField class has magic constraint solving behaviour (see Note [HasField
instances] in GHC.IR.Haskell.TypeChecker.Constraint.Interact).  However, we
permit users to declare their own instances, provided they do not clash with the
...
derived instance contexts] in GHC.IR.Haskell.Deriver.  However the predicate is
