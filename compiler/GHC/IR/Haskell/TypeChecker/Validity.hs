...
import GHC.Entity.FamilyInstance  ( isDominatedBy, injectiveBranches,
...
unambiguous. See Note [Impedance matching] in GHC.IR.Haskell.TypeChecker.Binding.
...
"tau-tvs" stuff.  This is done in the function 'GHC.IR.Haskell.TypeChecker.FunctionalDependency.grow'.
-}
...
A type signature like
   f :: Eq [(a,b)] => a -> b
is very fragile, for reasons described at length in GHC.IR.Haskell.TypeChecker.Constraint.Interact
Note [Instance and Given overlap].  As that Note discusses, for the
most part the clever stuff in GHC.IR.Haskell.TypeChecker.Constraint.Interact means that we don't use a
...
GHC.IR.Haskell.TypeChecker.Constraint.Interact.matchInstEnv fires an interaction with the top
...
[HasField instances] in GHC.IR.Haskell.TypeChecker.Constraint.Interact).  However, we permit users to
