...
\section[GHC.IR.Haskell.TypeChecker.Expression]{Typecheck an expression}
-}
...
module GHC.IR.Haskell.TypeChecker.Expression
   ( tcPolyExpr, tcMonoExpr, tcMonoExprNC
   , tcInferSigma, tcInferSigmaNC, tcInferRho, tcInferRhoNC
   , tcSyntaxOp, tcSyntaxOpGen, SyntaxOpType(..), synKnownType
   , tcCheckId, addExprErrCtxt, getFixedTyVars
   )
where

#include "HsVersions.h"

import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Splice
                           ( tcSpliceExpr, tcTypedBracket, tcUntypedBracket )
import qualified GHC.Builtin.Names.TemplateHaskell as THNames
...
import GHC.IR.Haskell.TypeChecker.Binding       ( chooseInferredQuantifiers
                                                , tcLocalBinds )
import GHC.IR.Haskell.TypeChecker.Signature     ( tcUserTypeSig, tcInstSig )
...
import GHC.Utils
import GHC.Entity.Var.Environment         ( emptyTidyEnv )
import GHC.Data.List.SetOps
import GHC.Data.Maybe
import GHC.Utils.Outputable as Outputable
import GHC.Data.FastString
import Control.Monad
import GHC.Entity.Class                   (classTyCon)
import GHC.Entity.Unique.Set              ( nonDetEltsUniqSet )
...
tcExpr (HsIf Nothing pred b1 b2) res_ty    -- Ordinary 'if'
  = do { pred' <- tcMonoExpr pred (mkCheckExpType boolTy)
       ; res_ty <- tauifyExpType res_ty
           ...
           -- in GHC.IR.Haskell.TypeChecker.Matches (See #10619)
...
tcExpr (HsMultiIf _ alts) res_ty
  = do { res_ty <- if isSingleton alts
             ...
             -- Just like GHC.IR.Haskell.TypeChecker.Matches
...
NB: this is not (quite) the same as being a "naughty" record selector (See Note
[Naughty record selectors]) in
GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration), at least in the case of
GADTs. Consider
  ...
  = ASSERT( notNull rbnds )
    do  { -- STEP -2: typecheck the record_expr, the record to be updated
        ...
        ; let fixed_tvs = getFixedTyVars upd_fld_occs con1_tvs relevant_cons
              ...
              --   c.f. GHC.IR.Haskell.TypeChecker.MutableType.newMetaTyVars
...
tcExpr (PArrSeq _ _) _
  = panic "GHC.IR.Haskell.TypeChecker.Expression.tcExpr: Infinite parallel array!"
...
-- HsSpliced is an annotation produced by
-- 'GHC.IR.Haskell.Renamer.Splice.rnSpliceExpr'.
...
tcInferFun fun
  = tcInferSigma fun
      -- NB: tcInferSigma; see GHC.IR.Haskell.TypeChecker.Unify
...
just like in GHC.IR.Haskell.TypeChecker.Binding.decideGeneralisationPlan
...
--
-- This is similar to checkCrossStageLifting in GHC.IR.Haskell.Renamer.Splice,
-- but this code is applied to *typed* brackets.
...
Note [Converting strings] in GHC.Compiler.TemplateToHaskell
...
mixedSelectors _ _ =
  panic "GHC.IR.Haskell.TypeChecker.Expression: mixedSelectors emptylists"
