...
module GHC.IR.Haskell.TypeChecker.Expression ( tcPolyExpr, tcMonoExpr, tcMonoExprNC,
...
import {-# SOURCE #-}   GHC.IR.Haskell.TypeChecker.Splice( tcSpliceExpr, tcTypedBracket, tcUntypedBracket )
...
import GHC.IR.Haskell.TypeChecker.Binding          ( chooseInferredQuantifiers, tcLocalBinds )
import GHC.IR.Haskell.TypeChecker.Signature           ( tcUserTypeSig, tcInstSig )
import GHC.IR.Haskell.TypeChecker.Constraint.Simplifier       ( simplifyInfer, InferMode(..) )
import GHC.IR.Haskell.TypeChecker.FamilyInstance          ( tcGetFamInstEnvs, tcLookupDataFamInst )
...
import GHC.IR.Haskell.TypeChecker.PatternSynonym( tcPatSynBuilderOcc, nonBidirectionalErr )
...
(See Note [Naughty record selectors]) in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration), at least
...
mixedSelectors _ _ = panic "GHC.IR.Haskell.TypeChecker.Expression: mixedSelectors emptylists"
