...
module GHC.IR.Haskell.TypeChecker.Deriving
   ( tcDeriving
   , DerivInfo(..)
   , mkDerivInfos
   )
...
data EarlyDerivSpec = InferTheta (DerivSpec [ThetaOrigin])
        ...
        -- See Note [Inferring the instance context] in
        -- GHC.IR.Haskell.Deriver.Constraints
  ...
  where
    ...
    extensions
      | isDerivSpecNewtype mechanism
        ...
        -- See Note [Newtype-deriving instances] in
        -- GHC.IR.Haskell.Deriver.BasicClasses
...
tcATDefault function from TcClsDcl (which is also used in
GHC.IR.Haskell.TypeChecker.Instance to handle the empty instance declaration
case).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Much of the code in GHC.IR.Haskell.TypeChecker.Deriving assumes that deriving
only works on data types.  But this assumption doesn't hold true for
DeriveAnyClass, since it's perfectly reasonable to do something like this:
