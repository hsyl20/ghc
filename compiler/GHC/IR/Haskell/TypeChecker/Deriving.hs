...
module GHC.IR.Haskell.TypeChecker.Deriving ( tcDeriving, DerivInfo(..), mkDerivInfos ) where
...
import GHC.IR.Haskell.Deriver.Constraints
import GHC.IR.Haskell.Deriver.Utils
import GHC.IR.Haskell.TypeChecker.Validity( allDistinctTyVars )
import GHC.IR.Haskell.TypeChecker.ClassDeclaration( tcATDefault, tcMkDeclCtxt )
import GHC.IR.Haskell.TypeChecker.Environment
import GHC.IR.Haskell.Deriver.BasicClasses                       -- Deriv stuff
...
import GHC.IR.Haskell.Renamer   ( addTcgDUs )
import GHC.Entity.Available

import GHC.Utils.Unify( tcUnifyTy )
import GHC.Entity.BasicTypes ( DerivStrategy(..) )
...
import GHC.Entity.Name.Set as NameSet
...
import qualified GHC.Entity.Var.Free as FV
...
data EarlyDerivSpec = InferTheta (DerivSpec [ThetaOrigin])
        ...
        -- See Note [Inferring the instance context] in GHC.IR.Haskell.Deriver.Constraints
...
when the dict is constructed in GHC.IR.Haskell.TypeChecker.Instance.tcInstDecl2
...
************************************************************************
*                                                                      *
\subsection[GHC.IR.Haskell.TypeChecker.Deriving-driver]{Top-level function for \tr{derivings}}
*                                                                      *
************************************************************************
-}
...
tcDeriving deriv_infos deriv_decls
  = recoverM (do { g <- getGblEnv
    ...
    do  {       -- Fish the "deriving"-related information out of the GHC.IR.Haskell.TypeChecker.Environment
    ...
    do { traceTc "Standalone deriving decl for" (ppr deriv_ty)
       ...
       ; traceTc "Standalone deriving;" $ vcat
              ...
              , text "tys:" <+> ppr inst_tys ]
                -- C.f. GHC.IR.Haskell.TypeChecker.Instance.tcLocalInstDecl1
...
************************************************************************
*                                                                      *
\subsection[GHC.IR.Haskell.TypeChecker.Deriving-normal-binds]{Bindings for the various classes}
*                                                                      *
************************************************************************
  ...
  where
    ...
    extensions
      | isDerivSpecNewtype mechanism
        ...
        -- See Note [Newtype-deriving instances] in GHC.IR.Haskell.Deriver.BasicClasses
...
tcATDefault function from TcClsDcl (which is also used in GHC.IR.Haskell.TypeChecker.Instance to handle
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Much of the code in GHC.IR.Haskell.TypeChecker.Deriving assumes that deriving only works on data types.
...
************************************************************************
*                                                                      *
\subsection[GHC.IR.Haskell.TypeChecker.Deriving-taggery-Names]{What con2tag/tag2con functions are available?}
*                                                                      *
************************************************************************
-}
