...
import GHC.IR.Haskell.TypeChecker.Instantiation   ( tcInstBinders, tcInstBindersX, tcInstBinderX )
...
tc_infer_hs_type mode (HsKindSig ty sig)
  = do { sig' <- tc_lhs_kind (kindLevel mode) sig
       ...
       ; return (ty', sig') }
-- HsSpliced is an annotation produced by 'GHC.IR.Haskell.Renamer.Splice.rnSpliceType' to communicate
...
-- HsSpliced is an annotation produced by 'GHC.IR.Haskell.Renamer.Splice.rnSpliceType'.
...
look at the TyCon or Class involved.
  ...
  * GHC.IR.Haskell.TypeChecker.Syntax.zonkTcTypeToType also can safely check/establish
    invariants.
...
-- pass in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration. (Never in getInitialKind, never in the
...
to be 'a', because when pretty printing we'll get
            class C a b where
               data D b a0
(NB: the tidying happens in the conversion to GHC.IR.Interface.Syntax, which happens
 ...
 * The pattern (T p1 p2) creates a *skolem* type variable 'a_sk',
   ...
   GHC.IR.Haskell.TypeChecker.Error.getSkolemInfo can thereby find the binding site for the skolem.
