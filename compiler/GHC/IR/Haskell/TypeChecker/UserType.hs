...
module GHC.IR.Haskell.TypeChecker.UserType (
        -- Type signatures
...
import GHC.IR.Haskell.TypeChecker.Constraint.Simplifier ( solveEqualities )
import GHC.IR.Haskell.TypeChecker.Type
import GHC.IR.Haskell.TypeChecker.Syntax        ( zonkSigType )
import GHC.IR.Haskell.TypeChecker.Instantiation ( tcInstBinders, tcInstBindersX
                                                , tcInstBinderX )
import GHC.Entity.Type
import GHC.Entity.Kind
import GHC.Entity.RdrName                       ( lookupLocalRdrOcc )
...
with GHC.IR.Haskell.TypeChecker.Validity.checkValidType.
...
-- 'GHC.IR.Haskell.Renamer.Splice.rnSpliceType' to communicate the splice
-- location to the typechecker. Here we skip over it in order to have the same
-- kind inferred for a given expression whether it was produced from splices or
-- not.
--
...
-- HsSpliced is an annotation produced by
-- 'GHC.IR.Haskell.Renamer.Splice.rnSpliceType'.
...
look at the TyCon or Class involved.
  ...
  * GHC.IR.Haskell.TypeChecker.Syntax.zonkTcTypeToType also can safely
  check/establish invariants.
  ...
  * It zonks any kinds.  The returned type should have no mutable kind
    or type variables (hence returning Type not TcType):
      - any unconstrained kind variables are defaulted to (Any *) just
        as in GHC.IR.Haskell.TypeChecker.Syntax.
...
tcExplicitTKBndrsX new_tv orig_hs_tvs thing_inside
  = go orig_hs_tvs $ \ tvs ->
    do { (result, bound_tvs) <- thing_inside tvs
         ...
         -- See Note [Bad telescopes] in GHC.IR.Haskell.TypeChecker.Validity.
...
-- pass in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration. (Never in
-- getInitialKind, never in the "type-checking"/desugaring pass.)
...
to be 'a', because when pretty printing we'll get
            class C a b where
               data D b a0
(NB: the tidying happens in the conversion to GHC.IR.Interface.Syntax, which
happens as part of pretty-printing a TyThing.)
 ...
 * The pattern (T p1 p2) creates a *skolem* type variable 'a_sk',
   ...
   GHC.IR.Haskell.TypeChecker.Error.getSkolemInfo can thereby find the binding
   site for the skolem.
