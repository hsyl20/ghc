...
GHC.IR.Haskell.TypeChecker.Pattern: Typechecking patterns
-}
...
module GHC.IR.Haskell.TypeChecker.Pattern ( tcLetPat, newLetBndr, LetBndrSpec(..)
             , tcPat, tcPat_O, tcPats
             , addDataConStupidTheta, badFieldCon, polyPatSig ) where

#include "HsVersions.h"

import {-# SOURCE #-}   GHC.IR.Haskell.TypeChecker.Expression( tcSyntaxOp, tcSyntaxOpGen, tcInferSigma )

import GHC.IR.Haskell.Syntax
import GHC.IR.Haskell.TypeChecker.Syntax
import GHC.IR.Haskell.TypeChecker.Signature( TcPragEnv, lookupPragEnv, addInlinePrags )
...
import GHC.IR.Haskell.TypeChecker.Validity( arityErr )
import GHC.Entity.Type ( pprTyVars )
...
tcPatBndr penv@(PE { pe_ctxt = LetPat { pc_lvl    = bind_lvl
  ...
  -- Note [Typechecking pattern bindings] in GHC.IR.Haskell.TypeChecker.Binding
...
-- See Note [Typechecking pattern bindings], item (4) in GHC.IR.Haskell.TypeChecker.Binding
--
...
like the situation in Note [Case branches must be taus] in GHC.IR.Haskell.TypeChecker.Matches.
...
-- HsSpliced is an annotation produced by 'GHC.IR.Haskell.Renamer.Splice.rnSplicePat'.
...
As explained by [Wrappers for data instance tycons] in GHC.Entity.Id.Make, the
...
See also Note [Typechecking pattern bindings] in GHC.IR.Haskell.TypeChecker.Binding
-}
