...
module GHC.IR.Haskell.TypeChecker.Pattern
   ( tcLetPat, newLetBndr, LetBndrSpec(..)
...
like the situation in Note [Case branches must be taus] in
GHC.IR.Haskell.TypeChecker.Matches.  So we tauify the pat_ty before proceeding.
...
-- 'GHC.IR.Haskell.Renamer.Splice.rnSplicePat'.
...
See also Note [Typechecking pattern bindings] in
