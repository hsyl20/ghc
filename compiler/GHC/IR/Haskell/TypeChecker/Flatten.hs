...
module GHC.IR.Haskell.TypeChecker.Flatten(
   FlattenMode(..),
...
import GHC.IR.Haskell.TypeChecker.Unify( occCheckExpand )
import GHC.IR.Haskell.TypeChecker.Evidence
import GHC.Entity.TypeConstructor
import GHC.Entity.TypeAndCoercion   -- performs delicate algorithm on types
...
import GHC.Entity.BasicTypes( SwapFlag(..) )
...
-- Note [No derived kind equalities] in
...
where it is *used*, namely in
GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.canEqTyVar.

See also Note [Conservative unification check] in
GHC.IR.Haskell.TypeChecker.Unify, which gives other examples where lazy
flattening caused problems.
...
GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.homogeniseRhsKind).
Invariant (F2) is needed solely to support (F1). It is relied on in one place:
...
(unlike TyConAppCo), see Note [TcCoercions] in GHC.IR.Haskell.TypeChecker.Evidence.
    ...
    do { -- First, flatten the arguments
       ...
       ; case mb_ct of
           Just (co, rhs_ty, flav)  -- co :: F xis ~ fsk
                -- flav is [G] or [WD]
                -- See Note [Type family equations] in
...
-- See Definition [Applying a generalised substitution] in
-- GHC.IR.Haskell.TypeChecker.Constraint.Solver
-- See Note [Stability of flattening] in
-- GHC.IR.Haskell.TypeChecker.Constraint.Solver
...
This avoids "saturating" the Givens, which can save a modest amount of work.  It
is easy to implement, in
GHC.IR.Haskell.TypeChecker.Constraint.Interact.kick_out, by only kicking out an
inert only if 
