...
-- Note [No derived kind equalities] in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser
...
where it is *used*, namely in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.canEqTyVar.

See also Note [Conservative unification check] in GHC.IR.Haskell.TypeChecker.Unify, which gives
...
GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.homogeniseRhsKind). Invariant (F2) is needed solely to support
    ...
    do { -- First, flatten the arguments
       ...
       ; case mb_ct of
           Just (co, rhs_ty, flav)  -- co :: F xis ~ fsk
                -- flav is [G] or [WD]
                -- See Note [Type family equations] in GHC.IR.Haskell.TypeChecker.Constraint.Solver
...
-- See Definition [Applying a generalised substitution] in GHC.IR.Haskell.TypeChecker.Constraint.Solver
-- See Note [Stability of flattening] in GHC.IR.Haskell.TypeChecker.Constraint.Solver
...
It is easy to implement, in GHC.IR.Haskell.TypeChecker.Constraint.Interact.kick_out, by only kicking out an inert
