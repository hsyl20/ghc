...
module GHC.IR.Haskell.TypeChecker.Signature(
       TcSigInfo(..),
...
import GHC.IR.Haskell.TypeChecker.Unify( tcSkolemise, unifyType, noThing )
import GHC.IR.Haskell.TypeChecker.Instantiation( topInstantiate )
import GHC.IR.Haskell.TypeChecker.Environment( tcLookupId )
import GHC.IR.Haskell.TypeChecker.Evidence( HsWrapper, (<.>) )
import GHC.Entity.Type( mkTyVarBinders )

import GHC.Config.Flags
import GHC.Entity.Var      ( TyVar, tyVarName, tyVarKind )
import GHC.Entity.Id       ( Id, idName, idType, idInlinePragma, setInlinePragma, mkLocalId )
import GHC.Builtin.Names( mkUnboundName )
import GHC.Entity.BasicTypes
import GHC.Data.Bag( foldrBag )
import GHC.Entity.Module( getModule )
...
* GHC.IR.Haskell: a signature in a binding starts of as a TypeSig, in
  type GHC.IR.Haskell.Syntax.Binding.Sig
...
* tcInstSig: TcIdSigInfo -> TcIdSigInst
  ...
  binder.  You can see this in GHC.IR.Haskell.TypeChecker.Binding.tcLhsId.
...
impedance-matching check in GHC.IR.Haskell.TypeChecker.Binding.mkExport

See Note [Signature skolems] in TcType

None of this applies to a function binding with a complete
signature, which doesn't use tcInstSig.  See GHC.IR.Haskell.TypeChecker.Binding.tcPolyCheck.
-}
...
From the TcSpecPrag, in GHC.Compiler.HaskellToCore.Binding we generate a binding for f_spec and a RULE:
...
3. Note that the HsWrapper can transform *any* function with the right
   ...
   well as the dict.  That's what goes on in GHC.IR.Haskell.TypeChecker.Instance.mk_meth_spec_prags
-}
