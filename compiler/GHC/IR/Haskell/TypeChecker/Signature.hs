...
import GHC.IR.Haskell.TypeChecker.Unify         ( tcSkolemise, unifyType
                                                , noThing )
...
signature, which doesn't use tcInstSig.
See GHC.IR.Haskell.TypeChecker.Binding.tcPolyCheck.
-}
...
From the TcSpecPrag, in GHC.Compiler.HaskellToCore.Binding we generate a binding
for f_spec and a RULE:
...
3. Note that the HsWrapper can transform *any* function with the right
   ...
   well as the dict.  That's what goes on in
   GHC.IR.Haskell.TypeChecker.Instance.mk_meth_spec_prags
-}
