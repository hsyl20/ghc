...
module GHC.IR.Haskell.TypeChecker.MutableType (
...
-- as it's used in GHC.IR.Haskell.TypeChecker.Syntax in the presence of knots.
...
Note [Case branches must never infer a non-tau type] of
GHC.IR.Haskell.TypeChecker.Matches.
...
instSkolTyCoVarX mk_tcv subst tycovar
  = do  { uniq <- newUnique  -- using a new unique is critical. See
                             -- Note [Skolems in zonkSyntaxExpr] in
                             -- GHC.IR.Haskell.TypeChecker.Syntax
...
-- the unifier; see GHC.IR.Haskell.TypeChecker.Unify.nicer_to_update_tv1, and
  ...
  = ASSERT2( isMetaTyVar tv, ppr tv )
    do  { when debugIsOn (check_empty tv)
        ...
        ; let uniq        = getUnique tv
                ...
                -- important for GHC.IR.Haskell.TypeChecker.UserType.splitTelescopeTvs to work properly
...
which is propagated up to the toplevel (see
GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.tcSimplifyInferCheck).
...
simplifier by GHC.IR.Haskell.TypeChecker.Module.tcRnSrcDecls calling
GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.tcSimplifyTop.
...
-- smart constructors. See Note [Zonking within the knot] in
-- GHC.IR.Haskell.TypeChecker.UserType
zonkTcTypeAndFV ty
...
zonkTyCoVar tv | isTcTyVar tv = zonkTcTyVar tv
   ...
   -- GHC.IR.Haskell.TypeChecker.UserType.tcTyClTyVars, but it seems
...
Why?, for example:
- For CDictCan, the
  @GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.expandSuperClasses@ step,
  which runs after the simple wanted and plugin loop, looks for @CDictCan@s. If
  a plugin is in use, constraints are zonked before being passed to the plugin.
  This means if we don't preserve a canonical form, @expandSuperClasses@ fails
  to expand superclasses. This is what happened in Trac #11525.
...
@GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser@ even if they come in as
CDictCan. Only canonical constraints that are actually in the inert set carry
all the guarantees. So it is okay if zonkCt creates e.g. a CDictCan where the
cc_tyars are /not/ function free.
-}
zonkCt :: Ct -> TcM Ct
...
-- See Note [SigSkol SkolemInfo] in GHC.IR.Haskell.TypeChecker.Types
