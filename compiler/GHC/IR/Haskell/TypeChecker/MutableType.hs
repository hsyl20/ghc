...
Note [Case branches must never infer a non-tau type] of GHC.IR.Haskell.TypeChecker.Matches.
...
instSkolTyCoVarX mk_tcv subst tycovar
  = do  { uniq <- newUnique  -- using a new unique is critical. See
                             -- Note [Skolems in zonkSyntaxExpr] in GHC.IR.Haskell.TypeChecker.Syntax
...
-- GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.canEqTyVarTyVar (nicer_to_update_tv2)
...
which is propagated up to the toplevel (see GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.tcSimplifyInferCheck).
...
simplifier by GHC.IR.Haskell.TypeChecker.Module.tcRnSrcDecls calling GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.tcSimplifyTop.
...
-- smart constructors. See Note [Zonking within the knot] in GHC.IR.Haskell.TypeChecker.UserType
zonkTcTypeAndFV ty
...
Why?, for example:
- For CDictCan, the @GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.expandSuperClasses@ step, which runs after the
...
@GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser@ even if they come in as CDictCan. Only canonical constraints that
