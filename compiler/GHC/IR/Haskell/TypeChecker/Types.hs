...
Various types used during typechecking, please see GHC.IR.Haskell.TypeChecker as well for
...
module GHC.IR.Haskell.TypeChecker.Types(
...
import GHC.IR.Haskell.TypeChecker.Evidence
import GHC.Entity.Type
import GHC.Entity.Class            ( Class )
import GHC.Entity.TypeConstructor ( TyCon, tyConKind )
import GHC.Entity.Coercion         ( Coercion, mkHoleCo )
import GHC.Entity.ConstructorLike  ( ConLike(..) )
import GHC.Entity.DataConstructor  ( DataCon, dataConUserType, dataConOrigArgTys )
import GHC.Entity.PatternSynonym   ( PatSyn, pprPatSynType )
import GHC.Entity.Id       ( idType, idName )
import GHC.Entity.FieldLabel ( FieldLabel )
import GHC.IR.Haskell.TypeChecker.Type
import GHC.Entity.Annotation
import GHC.Entity.ClassInstance
import GHC.Entity.FamilyInstance
...
import GHC.Entity.Var.Free as FV
...
-- (see 'GHC.IR.Interface.Renamer').  Specifically, a 'NameShape' for
...
*                                                                      *
    ...
    All the combinators for the monad can be found in GHC.IR.Haskell.TypeChecker
*                                                                      *
************************************************************************
...
data IfGblEnv
  = IfGblEnv {
        ...
        -- to look in this env.
        -- See Note [Tying the knot] in GHC.IR.Interface.TypeChecker
...
data IfLclEnv
  = IfLclEnv {
        ...
        -- of the time it's @Nothing@.  See Note [Resolving never-exported Names in GHC.IR.Interface.TypeChecker]
        -- in GHC.IR.Interface.TypeChecker.
...
--
-- This data type really should be in GHC.Entity.Types, but it needs
...
--        in GHC.IR.Interface.Utils, and tcLookupGlobal in GHC.IR.Haskell.TypeChecker.Environment
--
...
--        in the home library we are compiling.  (See GHC.IR.Interface.Loader.)
...
data TcGblEnv
  = TcGblEnv {
        ...
        tcg_field_env :: RecFieldEnv,   -- ^ Just for things in this module
                                        -- See Note [The interactive package] in GHC.Entity.Types
        ...
        tcg_type_env :: TypeEnv,
          ...
          -- Note [The interactive package] in GHC.Entity.Types
        ...
        -- For the latter, see Note [The interactive package] in GHC.Entity.Types
        tcg_tr_module :: Maybe Id,           -- Id for $trModule :: GHC.Entity.Types.Module
...
type ThBindEnv = NameEnv (TopLevelFlag, ThLevel)
   ...
   -- bound at top level!  See Note [Template Haskell levels] in GHC.IR.Haskell.TypeChecker.Splice
...
data ThStage    -- See Note [Template Haskell state diagram] in GHC.IR.Haskell.TypeChecker.Splice
  ...
  | RunSplice (TcRef [ForeignRef (TH.Q ())])
      ...
      -- [Delaying modFinalizers in untyped splices] in "GHC.IR.Haskell.Renamer.Splice".
      --
      ...
      --
      -- See Note [Collecting modFinalizers in typed splices] in "GHC.IR.Haskell.TypeChecker.Splice".
...
type ThLevel = Int
    -- NB: see Note [Template Haskell levels] in GHC.IR.Haskell.TypeChecker.Splice
...
-- See 'GHC.IR.Haskell.TypeChecker.Environment' for how to retrieve a 'TyThing' given a 'Name'.
data TcTyThing
...
data PromotionErr
  ...
  | FamDataConPE     -- Data constructor for a data family
                     -- See Note [AFamDataCon: not promoting data family constructors] in GHC.IR.Haskell.TypeChecker.Module
  | PatSynPE         -- Pattern synonyms
                     -- See Note [Don't promote pattern synonyms] in GHC.IR.Haskell.TypeChecker.Environment

  | RecDataConPE     -- Data constructor in a recursive loop
                     -- See Note [ARecDataCon: recusion and promoting data constructors] in GHC.IR.Haskell.TypeChecker.TypeAndClassDeclaration
...
--
-- a) for static forms in GHC.IR.Haskell.TypeChecker.Expression.checkClosedInStaticForm and
-- b) to figure out when a nested binding can be generalised (in
--    GHC.IR.Haskell.TypeChecker.Binding.decideGeneralisationPlan).
--
...
data ImportAvails
   = ImportAvails {
        imp_mods :: ImportedMods,
          ...
          --
          -- See the documentation on ImportedModsVal in GHC.Entity.Types for the
        ...
        imp_trust_pkgs :: Set InstalledUnitId,
          ...
          -- See Note [GHC.IR.Haskell.Renamer.ImportExport . Tracking Trust Transitively]

        imp_trust_own_pkg :: Bool,
          -- ^ Do we require that our own package is trusted?
          -- This is to handle efficiently the case where a Safe module imports
          -- a Trustworthy module that resides in the same package as it.
          -- See Note [GHC.IR.Haskell.Renamer.ImportExport . Trust Own Package]
...
data WhereFrom
  ...
  | ImportByPlugin                      -- Importing a plugin;
                                        -- See Note [Care with plugin imports] in GHC.IR.Interface.Loader
...
-- GHC.IR.Haskell.TypeChecker.Constraint.Simplifier uses them, and GHC.IR.Haskell.TypeChecker.Constraint.Simplifier is fairly
...
data TcIdSigInfo   -- See Note [Complete and partial type signatures]
  ...
  | PartialSig     -- A partial type signature (i.e. includes one or more
      ...
      , psig_hs_ty :: LHsSigWcType GhcRn -- The original partial signature in GHC.IR.Haskell form
...
data TcIdSigInst
  = TISI { sig_inst_sig :: TcIdSigInfo
         ...
         , sig_inst_skols :: [(Name, TcTyVar)]
               ...
               -- See Note [Binding scoped type variables] in GHC.IR.Haskell.TypeChecker.Signature
...
It happens, too!  See Note [Polymorphic methods] in GHC.IR.Haskell.TypeChecker.ClassDeclaration.
...
data TcPatSynInfo
  = TPSI {
        ...
        patsig_implicit_bndrs :: [TyVarBinder], -- Implicitly-bound kind vars (Inferred) and
          ...
          -- See Note [The pattern-synonym signature splitting rule] in GHC.IR.Haskell.TypeChecker.PatternSynonym
...
-- We enforce the structure of Xi types when we flatten (GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser)
...
data Ct
  ...
  = CDictCan {  -- e.g.  Num xi
      ...
      cc_pend_sc :: Bool   -- See Note [The superclass story] in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser
  ...
  | CTyEqCan {  -- tv ~ rhs
       -- Invariants:
       --   * See Note [Applying the inert substitution] in GHC.IR.Haskell.TypeChecker.Flatten
       ...
       --     equalities] in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser
  ...
  | CFunEqCan {  -- F xis ~ fsk
      ...
      cc_fsk    :: TcTyVar  -- [Given]  always a FlatSkolTv
        ...
        -- See Note [The flattening story] in GHC.IR.Haskell.TypeChecker.Flatten
    }

  | CNonCanonical {        -- See Note [NonCanonical Semantics] in GHC.IR.Haskell.TypeChecker.Constraint.Solver
      cc_ev  :: CtEvidence
    }
...
built (in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser).
...
See Note [The superclass story] in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.
-}
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
First read Note [The superclass story] in GHC.IR.Haskell.TypeChecker.Constraint.Canonicaliser.
...
*                                                                      *
                Wanted constraints
     These are forced to be in GHC.IR.Haskell.TypeChecker.Types because
...
data ImplicStatus
  = IC_Solved     -- All wanteds in the tree are solved, all the way down
       ...
       , ics_dead :: [EvVar] }  -- Subset of ic_given that are not needed
         -- See Note [Tracking redundant constraints] in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier
...
outside the implication in GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.floatEqualities or
GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.approximateImplications
...
a coercion that is used in a type. (See the comments in Note [Flattening] in GHC.IR.Haskell.TypeChecker.Flatten
...
data TcEvDest
  ...
  | HoleDest  CoercionHole  -- ^ fill in this hole with the evidence
              ...
              -- See Note [Coercion holes] in GHC.Entity.TypeAndCoercion
...
"shadow" [D] in GHC.IR.Haskell.TypeChecker.Constraint.Solver.maybeEmitShadow.

See Note [The improvement story and derived shadows] in GHC.IR.Haskell.TypeChecker.Constraint.Solver
-}
...
-- Note [Flavours with roles] in GHC.IR.Haskell.TypeChecker.Constraint.Solver
...
GHC.IR.Haskell.TypeChecker.Constraint.Solver.
...
improvement works; see Note [The improvement story] in GHC.IR.Haskell.TypeChecker.Constraint.Interact.

However, for now at least I'm only letting (Derived,NomEq) rewrite
(Derived,NomEq) and not doing anything for ReprEq.  If we have
    eqCanRewriteFR (Derived, NomEq) (Derived, _)  = True
then we lose property R2 of Definition [Can-rewrite relation]
in GHC.IR.Haskell.TypeChecker.Constraint.Solver
  R2.  If f1 >= f, and f2 >= f,
...
sense of Definition [Can-rewrite relation] in GHC.IR.Haskell.TypeChecker.Constraint.Solver.
...
data SkolemInfo
  ...
  | InstSC TypeSize     -- A "given" constraint obtained by superclass selection.
                        ...
                        -- See Note [Solving superclass constraints] in GHC.IR.Haskell.TypeChecker.Instance
  ...
  | ArrowSkol           -- An arrow form (see GHC.IR.Haskell.TypeChecker.Arrow)
...
* Then when tidying in GHC.IR.Haskell.TypeChecker.MutableType.tidySkolemInfo, we first tidy a' to
