...
import GHC.Entity.TypeConstructor ( TyCon, isTupleTyCon, tyConSingleDataCon_maybe )
import GHC.Entity.DataConstructor  ( DataCon, dataConTyCon )
import GHC.Entity.ConstructorLike  ( ConLike(..) )
import GHC.Entity.Coercion.Axiom   ( Role, fsFromRole )
...
-- from GHC.Compiler.TemplateToHaskell
...
{- **********************************************************************
  ...
  #cvBinds-etc# Converting to @GHC.IR.Haskell.Syntax.Binding@, etc.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
In GHC.Entity.Types, which declares (:), we have
  infixr 5 :
...
makeFunBind :: Located RdrName -> [LMatch GhcPs (LHsExpr GhcPs)]
            -> HsBind GhcPs
-- Like GHC.IR.Haskell.Utils.mkFunBind, but we need to be able to set the fixity
-- too
makeFunBind fn ms
  = FunBind { fun_id = fn,
