...
data LHsQTyVars pass   -- See Note [HsType binders]
  = HsQTvs { hsq_implicit :: PostRn pass [Name]
           ...
           , hsq_dependent :: PostRn pass NameSet
               ...
               -- See Note [Dependent LHsQTyVars] in GHC.IR.Haskell.TypeChecker.UserType
    }
...
data HsType pass
  ...
  | HsParTy             (LHsType pass)   -- See Note [Parens in GHC.IR.Haskell]
...
? Is it `(*) Foo Int` or `Foo GHC.Entity.Types.* Int`? There's no way to know until renaming.
So we just take type expressions like this and put each component in a list, so be
sorted out in the renamer. The sorting out is done by GHC.IR.Haskell.Renamer.Type.mkHsOpTyRn. This means
