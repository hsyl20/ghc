...
the type * to mean what we now call GHC.Entity.Types.Type. The problem is that
...
these are the [[LHsType Name]] and a bunch of operators
  ...
  [GHC.TypeLits.+, GHC.Entity.Types.*]

Note that the * is GHC.Entity.Types.*. So, we want to rearrange to have
