...
import GHC.Entity.RdrName         ( RdrName, rdrNameOcc )
import GHC.Entity.SrcLoc
import GHC.Data.List.SetOps       ( findDupsEq )
import GHC.Entity.BasicTypes      ( RecFlag(..), LexicalFixity(..) )
...
walking polymorphic bindings (GHC.IR.Haskell.Syntax.Binding) and qualifier lists
...
\subsubsection[dep-GHC.IR.Haskell.Syntax.Binding]{Polymorphic bindings}
...
2. Typecheck the builder definition, which needs the typechecked
   ...
   in GHC.IR.Haskell.TypeChecker.Binding.tcValBinds.
