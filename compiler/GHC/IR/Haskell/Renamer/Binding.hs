...
as those walking polymorphic bindings (GHC.IR.Haskell.Syntax.Binding) and qualifier lists in
  ...
  = do  { mod <- getModule
        ...
        ; let all_fvs = pat_fvs `plusFV` rhs_fvs
              fvs'    = filterNameSet (nameIsLocalOrFrom mod) all_fvs
                ...
                -- MonoLocalBinds test in GHC.IR.Haskell.TypeChecker.Binding.decideGeneralisationPlan
