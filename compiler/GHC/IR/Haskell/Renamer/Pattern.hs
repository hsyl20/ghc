...
import GHC.IR.Haskell.TypeChecker.Syntax             ( hsOverLitName )
...
import GHC.Entity.Literal             ( inCharRange )
...
newPatName (LetMk is_top fix_env) rdr_name
  = CpsRn (\ thing_inside ->
    ...
    --       before going on to the RHSes (see GHC.IR.Haskell.Renamer).
    ...
    rn_fld pun_ok overload_ok (L l (HsRecField { hsRecFieldLbl = L loc f
      ...
      = do { let lbl = rdrNameAmbiguousFieldOcc f
           ; sel <- setSrcSpan loc $
                      ...
                      -- See Note [Disambiguating record fields] in GHC.IR.Haskell.TypeChecker.Expression
                      if overload_ok
