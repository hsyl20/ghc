...
import GHC.IR.Haskell.Renamer               ( rnSrcDecls, findSplice )
import GHC.IR.Haskell.Renamer.Pattern       ( rnPat )
import GHC.Entity.BasicTypes                ( TopLevelFlag, isTopLevel
                                            , SourceText(..) )
...
import GHC.IR.Haskell.TypeChecker.Environment           ( checkWellStaged )
import GHC.Builtin.Names.TemplateHaskell                ( liftName )

import GHC.Config.Flags
import GHC.Data.FastString
import GHC.Utils.Error                        ( dumpIfSet_dyn_printer )
import GHC.IR.Haskell.TypeChecker.Environment ( tcMetaTy )
...
import {-# SOURCE #-} GHC.IR.Haskell.TypeChecker.Expression   ( tcPolyExpr )
...
[2] 'rnSpliceExpr'
[3] 'GHC.IR.Haskell.TypeChecker.Splice.qAddModFinalizer'
[4] 'GHC.IR.Haskell.TypeChecker.Expression.tcExpr' ('HsSpliceE' ('HsSpliced' ...))
[5] 'GHC.IR.Haskell.TypeChecker.UserType.tc_hs_type' ('HsSpliceTy' ('HsSpliced' ...))
[6] 'GHC.IR.Haskell.TypeChecker.Pattern.tc_pat' ('SplicePat' ('HsSpliced' ...))
...
GHC.IR.Haskell.Renamer.Type.rnHsSigWcType
...
GHC.IR.Haskell.Renamer.Pattern, through the CpsRn monad. We don't wish to be in
that monad here (it would create import cycles and generally conflict with
renaming other splices), so we really want to return a (Pat RdrName) -- the
result of running the splice -- which can then be further renamed in
GHC.IR.Haskell.Renamer.Pattern, in the CpsRn monad.
...
-- GHC.IR.Haskell.TypeChecker.Expression, but this is only run on *untyped*
-- brackets.
...
GHC.IR.Haskell.TypeChecker.Expression.thBrackId).  So, after incrementing the
