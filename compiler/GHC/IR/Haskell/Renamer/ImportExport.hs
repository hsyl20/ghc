...
import GHC.IR.Interface.Loader        ( loadSrcInterface )
...
import GHC.Entity.BasicTypes          ( TopLevelFlag(..), StringLiteral(..) )
import GHC.Utils
...
according to the rules outlined in the Note [Safe Haskell Trust Check] in
GHC.Program.Main we must also check that these rules hold transitively for all
dependent modules and packages. Doing this without caching any trust information
would be very slow as we would need to touch all packages and interface files a
module depends on. To avoid this we make use of the property that if a modules
Safe Haskell mode changes, this triggers a recompilation from that module in the
dependcy graph. So we can just worry mostly about direct imports.
...
rnImportDecl this_mod
  ...
  = setSrcSpan loc $ do
    ...
    -- at least not until GHC.IR.Interface.TypeChecker.tcHiBootIface, which is
    -- too late to avoid typechecker crashes.  (Indirect self imports are not
    -- caught until GHC.IR.Interface.TypeChecker, see #10337 tracking how to
  ...
  let imp_mod    = mi_module iface
      ...
      (dependent_mods, dependent_pkgs, pkg_trust_req)
         | pkg == thisPackage dflags =
            ...
            -- check.  See GHC.IR.Interface.Loader.loadHiBootInterface
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
See also: Note [Interactively-bound Ids in GHCi] in GHC.Entity.Types
...
*** See Note [The Naming story] in GHC.IR.Haskell.Syntax.Declaration ****
*                                                                      *
********************************************************************* -}
...
newRecordSelector overload_ok (dc:_) (L loc (FieldOcc (L _ fld) _))
  ...
  where
    ...
    field | isExact fld = fld
              ...
              -- selectors in Template Haskell.
              -- See Note [Binders in Template Haskell] in
              -- GHC.Compiler.TemplateToHaskell and
              -- Note [Looking up Exact RdrNames] in
              -- GHC.IR.Haskell.Renamer.Environment
...
Then M's export_avails are (recall the AvailTC invariant from
