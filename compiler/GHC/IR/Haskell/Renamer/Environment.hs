...
GHC.IR.Haskell.Renamer.Environment contains functions which convert RdrNames into Names.
  ...
  =     -- This is here to catch
    ...
    if isExternalName name then
      do { this_mod <- getModule
         ...
         ; return name }
    else   -- See Note [Binders in Template Haskell] in GHC.Compiler.TemplateToHaskell
      do { this_mod <- getModule
...
lookupFamInstName (Just cls) tc_rdr  -- Associated type; c.f GHC.IR.Haskell.Renamer.Binding.rnMethodBind
 ...
 * In GHC.IR.Haskell.Renamer.Environment.newTopSrcBinder we spot Exact RdrNames that wrap a
   non-External Name, and make an External name for it. This is
   the name that goes in the GlobalRdrEnv

 * When looking up an occurrence of an Exact name, done in
   GHC.IR.Haskell.Renamer.Environment.lookupExactOcc, we find the Name with the right unique in the
 ...
 * Exact names are also use for purely local binders generated
   ...
   gets looked up in the LocalRdrEnv by GHC.IR.Haskell.Renamer.Environment.lookupOccRn, and
...
-- See Note [Renaming pattern synonym variables] in GHC.IR.Haskell.Renamer.Binding
...
We store the relevant Name in the GHC.IR.Haskell tree, in
