...
Contains functions which convert RdrNames into Names.

-}

{-# LANGUAGE CPP, MultiWayIf, NamedFieldPuns #-}

module GHC.IR.Haskell.Renamer.Environment (
        newTopSrcBinder,
...
import GHC.IR.Interface.Loader ( loadInterfaceForName, loadSrcInterface_maybe )
...
import GHC.Entity.BasicTypes   ( pprWarningTxtForMsg, TopLevelFlag(..))
...
import GHC.IR.Haskell.Renamer.Utils
import Data.Functor (($>))
...
newTopSrcBinder (L loc rdr_name)
  ...
  =     -- This is here to catch
    ...
    if isExternalName name then
      do { this_mod <- getModule
         ...
         ; return name }
    else   -- See Note [Binders in Template Haskell] in
           -- GHC.Compiler.TemplateToHaskell
      do { this_mod <- getModule
  ...
  = do  { this_mod <- getModule
        ...
        --      data T = (,,,)
        -- the constructor is parsed as a type, and then GHC.IR.Haskell.Parser.Syntax.tyConToDataCon
...
Looking up a name in the GHC.IR.Haskell.Renamer.Environment.
...
lookupFamInstName (Just cls) tc_rdr
  -- Associated type; c.f GHC.IR.Haskell.Renamer.Binding.rnMethodBind
...
positions for constructors, TyCons etc.  For example
    [d| data T = MkT Int |]
when we splice in and Convert to GHC.IR.Haskell RdrName, we'll get
 ...
 * In GHC.IR.Haskell.Renamer.Environment.newTopSrcBinder we spot Exact RdrNames
   that wrap a non-External Name, and make an External name for it. This is the
   name that goes in the GlobalRdrEnv

 * When looking up an occurrence of an Exact name, done in
   GHC.IR.Haskell.Renamer.Environment.lookupExactOcc, we find the Name with the
   right unique in the GlobalRdrEnv, and use the one from the envt -- it will be
   an External Name in the case of the data type/constructor above.

 * Exact names are also use for purely local binders generated
   by TH, such as    \x_33. x_33
   Both binder and occurrence are Exact RdrNames.  The occurrence gets looked up
   in the LocalRdrEnv by GHC.IR.Haskell.Renamer.Environment.lookupOccRn, and
   misses, because lookupLocalRdrEnv always returns Nothing for an Exact Name.
   Now we fall through to lookupExactOcc, which will find the Name is not in the
   GlobalRdrEnv, so we just use the Exact supplied Name.
...
-- GHC.IR.Haskell.Renamer.Binding
...
We store the relevant Name in the GHC.IR.Haskell.Syntax tree, in
  * HsIntegral/HsFractional/HsIsString
  * NegApp
  * NPlusKPat
  * HsDo
respectively.  Initially, we just store the "standard" name
(GHC.Builtin.Names.fromIntegralName, fromRationalName etc), but the renamer
changes this to the appropriate user name if Opt_NoImplicitPrelude is on.  That
is what lookupSyntaxName does.
