...
\section[GHC.IR.Haskell.Syntax.Binding]{Abstract syntax: top-level bindings and signatures}
...
module GHC.IR.Haskell.Syntax.Binding where

import {-# SOURCE #-} GHC.IR.Haskell.Syntax.Expression ( pprExpr, LHsExpr,
                               MatchGroup, pprFunBind,
                               GRHSs, pprPatBind )
import {-# SOURCE #-} GHC.IR.Haskell.Syntax.Pattern  ( LPat )
...
data HsBindLR idL idR
  = -- | Function Binding
    ...
    -- and variables                          @f = \x -> e@
    --
    -- Reason 1: Special case for type inference: see 'GHC.IR.Haskell.TypeChecker.Binding.tcMonoBinds'.
    --
  ...
  | AbsBinds {                      -- Binds abstraction; TRANSLATION
       ...
       -- to have the right type
        ...
        -- | Evidence bindings
        -- Why a list? See GHC.IR.Haskell.TypeChecker.Instance
...
variables.  The action happens in GHC.IR.Haskell.TypeChecker.Binding.mkExport.
...
a) Dependency analysis prior to type checking
    (see GHC.IR.Haskell.TypeChecker.Binding.tc_group)

b) Deciding whether we can do generalisation of the binding
    (see GHC.IR.Haskell.TypeChecker.Binding.decideGeneralisationPlan)

c) Deciding whether the binding can be used in static forms
    (see GHC.IR.Haskell.TypeChecker.Expression.checkClosedInStaticForm for the HsStatic case and
     GHC.IR.Haskell.TypeChecker.Binding.isClosedBndrGroup).
...
plusHsValBinds _ _
  = panic "GHC.IR.Haskell.Syntax.Binding.plusHsValBinds"
