...
module GHC.IR.Haskell.Syntax.Type (
...
import {-# SOURCE #-} GHC.IR.Haskell.Syntax.Expression ( HsSplice, pprSplice )

import GHC.IR.Haskell.Syntax.PlaceHolder ( PlaceHolder(..) )
import GHC.IR.Haskell.Syntax.Extension

import GHC.Entity.Id                ( Id )
import GHC.Entity.Name              ( Name )
import GHC.Entity.RdrName           ( RdrName )
import GHC.Entity.Name.Set          ( NameSet, emptyNameSet )
import GHC.Entity.DataConstructor   ( HsSrcBang(..), HsImplBang(..)
                                    , SrcStrictness(..), SrcUnpackedness(..) )
...
See also Note [Kind and type-variable binders] in GHC.IR.Haskell.Renamer.Type
...
* The explicit presence of these wrappers specifies, in the
  GHC.IR.Haskell.Syntax, exactly where implicit quantification is allowed, and
...
data HsImplicitBndrs pass thing   -- See Note [HsType binders]
  = HsIB { hsib_vars :: PostRn pass [Name] -- Implicitly-bound kind & type vars
         ...
         , hsib_closed :: PostRn pass Bool 
          ...
          -- GHC.IR.Haskell.TypeChecker.UserType.decideKindGeneralisationPlan
    }
...
data HsType pass
  ...
  | HsParTy             (LHsType pass)   -- See Note [Parens in Haskell syntax]
                                         -- in GHC.IR.Haskell.Syntax.Expression
        -- Parenthesis preserved for the precedence re-arrangement in
        -- GHC.IR.Haskell.Renamer.Type
...
Note [Context quantification] in GHC.IR.Haskell.Renamer.Type, and Trac #4426.
...
GHC.IR.Haskell.Renamer.Type.mkHsOpTyRn. This means that the parser should never
...
-- Note [Disambiguating record fields] in GHC.IR.Haskell.TypeChecker.Expression.
-- See Note [Located RdrNames] in HsExpr
