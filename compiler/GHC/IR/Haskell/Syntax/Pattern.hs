...
data Pat p
  ...
  | NPlusKPat       (Located (IdP p))       -- n+k pattern
                    ...
                    (HsOverLit p)      -- See Note [NPlusK patterns] in
                                       -- GHC.IR.Haskell.TypeChecker.Pattern
                     -- NB: This could be (PostTc ...), but that induced a
                     -- a new hs-boot file. Not worth it.

                    (SyntaxExpr p)     -- (>=) function, of type t1->t2->Bool
                    (SyntaxExpr p)     -- Name of '-' (see lookupSyntaxName in
...
-- 'GHC.Compiler.HaskellToCore.Splices.repUpdFields'.
--
...
--
-- See also Note [Disambiguating record fields] in
-- GHC.IR.Haskell.TypeChecker.Expression.
...
pprPat (ConPatOut { pat_con = con, pat_tvs = tvs, pat_dicts = dicts,
  ...
  = sdocWithDynFlags $ \dflags ->
       -- Tiresome; in GHC.IR.Haskell.TypeChecker.Binding.tcRhs we print out a
       -- typechecked Pat in an error message, and we want to make sure it
       -- prints nicely
