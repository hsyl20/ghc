...
(equivalent to the code in GHC.IR.Haskell.Deriver.BasicClasses, for other
classes)
-}
...
module GHC.IR.Haskell.Deriver.Generic
   ( canDoGenerics
   , canDoGenerics1
   , GenericKind(..)
   , gen_Generic_binds
...
import GHC.Entity.FamilyInstance     ( FamInst, FamFlavor(..), mkSingleCoAxiom )
import GHC.IR.Haskell.TypeChecker.FamilyInstance
import GHC.Entity.Module             ( moduleName, moduleNameFS
...
import GHC.Entity.Var.Set  (elemVarSet)
...
-- called by GHC.IR.Haskell.Deriver.inferConstraints; generates a list of types,
-- each of which must be a Functor in order for the Generic1 instance to work.
...
See Note [Unify kinds in deriving] in GHC.IR.Haskell.Deriver.
