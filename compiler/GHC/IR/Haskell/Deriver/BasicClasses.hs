...
GHC.IR.Haskell.Deriver.BasicClasses: Generating derived instance declarations

This module is nominally ``subordinate'' to @GHC.IR.Haskell.TypeChecker.Deriving@, which is the
...
module GHC.IR.Haskell.Deriver.BasicClasses (
...
import GHC.Entity.Module ( moduleName, moduleNameString
              , moduleUnitId, unitIdString )
import GHC.Entity.Id.Make ( coerceId )
...
import GHC.IR.Haskell.TypeChecker.Validity ( checkValidTyFamEqn )
      ...
      | otherwise   =
        ...
        where
             ...
             show_arg b arg_ty
               ...
               -- See Note [Deriving and unboxed types] in GHC.IR.Haskell.TypeChecker.Deriving
...
so we have to switch that flag on locally in GHC.IR.Haskell.Deriver.genInst.
...
gen_Newtype_binds loc cls inst_tvs inst_tys rhs_ty
  ...
  where
    ...
    mk_atf_inst fam_tc = do
        ...
        -- Check (c) from Note [GND and associated type families] in GHC.IR.Haskell.TypeChecker.Deriving
