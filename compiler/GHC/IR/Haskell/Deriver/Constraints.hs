...
module GHC.IR.Haskell.Deriver.Constraints
   ( inferConstraints
...
-- import GHC.IR.Haskell.TypeChecker.Error (reportAllUnsolved)
...
import GHC.IR.Haskell.TypeChecker.Validity (validDerivPred)
...
Specifically, we use GHC.IR.Haskell.Deriver.BasicClasses.box to box the Int#
into an Int (which we know how to show), and append a '#'. Parenthesis are not
required for unboxed values (`MkT -3#` is a valid expression).
...
simplifyInstanceContexts infer_specs
  ...
  where
    ...
    iterate_deriv n current_solns
      ...
      -- (See GHC.IR.Haskell.TypeChecker.Constraint.Simplifier.tcSimplifyDeriv.)
    ...
    gen_soln (DS { ds_loc = loc, ds_tvs = tyvars
      ...
      = setSrcSpan loc  $
        ...
        do { theta <- simplifyDeriv the_pred tyvars deriv_rhs
           ...
           ; traceTc "GHC.IR.Haskell.TypeChecker.Deriving"
