...
2. Generate a binding for every data type declaration T in module M,
   ...
   recipe for computing the kind of an instantiation of the tycon (see Note
   [Representing TyCon kinds: KindRep] later in this file for details).
...
4. Solve Typeable constraints.  This is done by a custom Typeable solver,
   currently in GHC.IR.Haskell.TypeChecker.Constraint.Interact, that use M.$tcT
   so solve (Typeable T).
...
* GHC.Prim doesn't have any associated object code, so we need to put the
  ...
  place to be GHC.Types.
  GHC.IR.Haskell.Deriver.Typeable.mkPrimTypeableBinds is responsible for
