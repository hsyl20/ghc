...
module GHC.IR.Haskell.Deriver.Typeable(mkTypeableBinds) where


import GHC.Entity.BasicTypes ( Boxity(..), neverInlinePragma )
import GHC.IR.Haskell.TypeChecker.Binding( addTypecheckedBinds )
import GHC.IR.Interface.Environment( newGlobalBinder )
import GHC.Entity.TypeAndCoercion( Type(..), TyLit(..) )
import GHC.IR.Haskell.TypeChecker.Environment
import GHC.IR.Haskell.TypeChecker.Evidence ( mkWpTyApps )
import GHC.IR.Haskell.TypeChecker
import GHC.Entity.Types ( lookupId )
import GHC.Builtin.Names
import GHC.Builtin.Primitive.Types ( primTyCons )
import GHC.Builtin.Types ( tupleTyCon, sumTyCon, runtimeRepTyCon
                         , vecCountTyCon, vecElemTyCon
                         , nilDataCon, consDataCon )
import GHC.Entity.Name
import GHC.Entity.Id
import GHC.Entity.Type
import GHC.Entity.Kind ( isTYPEApp )
...
import GHC.Data.Bag
import GHC.Entity.Var ( TyVarBndr(..) )
...
1. Generate a binding for each module p:M
   (done in GHC.IR.Haskell.Deriver.Typeable by mkModIdBindings)
       M.$trModule :: GHC.Entity.Types.Module
       M.$trModule = Module "p" "M"
   ("tr" is short for "type representation"; see GHC.Entity.Types)
...
2. Generate a binding for every data type declaration T in module M,
   ...
   its kind. kind_rep is a value of type GHC.Entity.Types.KindRep, which gives a
   ...
   they get slightly special treatment in GHC.IR.Interface.Renamer.rnIfaceDecl.
...
4. Solve Typeable constraints.  This is done by a custom Typeable solver,
   currently in GHC.IR.Haskell.TypeChecker.Constraint.Interact, that use M.$tcT so solve (Typeable T).
...
* The timing of when we produce this bindings is rather important: they must be
  ...
  currently compiling GHC.Entity.Types (where they are defined).
...
* GHC.Prim doesn't have any associated object code, so we need to put the
  ...
  place to be GHC.Entity.Types. GHC.IR.Haskell.Deriver.Typeable.mkPrimTypeableBinds is responsible for
...
* To save space and reduce dependencies, we need use quite low-level
  representations for TyCon and Module.  See GHC.Entity.Types
...
* In general there are lots of things of kind *, * -> *, and * -> * -> *. To
  ...
  once in GHC.Entity.Types. These are referred to as "built-in" KindReps below.
...
mkTypeableBinds
  = do { -- Create a binding for $trModule.
       ...
       ; tcg_env <- mkModIdBindings
         ...
         -- First we handle the primitive TyCons if we are compiling GHC.Entity.Types
...
-- representation bindings for these primitive types into "GHC.Entity.Types"
--
...
-- in "GHC.Entity.Types". This should include all types defined in "GHC.Prim".
--
...
-- Note [Built-in syntax and the OrigNameCache] in GHC.IR.Interface.Environment for more.
...
-- | We generate @KindRep@s for a few common kinds in @GHC.Entity.Types@ so that they
...
-- | Performed while compiling "GHC.Entity.Types" to generate the built-in 'KindRep's.
...
The TypeRep encoding of `Proxy Type Int` looks like this:
    ...
    $tcProxy :: GHC.Entity.Types.TyCon
    $trInt   :: TypeRep Int
    $trType  :: TypeRep Type
    ...
    $tkProxy :: GHC.Entity.Types.KindRep
 ...
 * In $tcProxy, the GHC.Entity.Types.TyCon structure for Proxy, we store a
   GHC.Entity.Types.KindRep, which represents the polymorphic kind of Proxy
       Proxy :: forall k. k->Type
