...
module GHC.IR.Core.Transformer.FloatOut ( floatOutwards ) where
...
import GHC.Entity.Id               ( Id, idArity, idType, isBottomingId,
                                     isJoinId, isJoinId_maybe )
import GHC.Entity.BasicTypes       ( TopLevelFlag(..), isTopLevel )
import GHC.IR.Core.Transformer.LevelSetter
import GHC.Entity.Unique.Supply    ( UniqSupply )
import GHC.Data.Bag
import GHC.Utils
...
join points] in GHC.IR.Core.Syntax), so we must be careful with how far we float
them. The mechanism for doing so is the *join ceiling*, detailed in Note [Join
ceiling] in GHC.IR.Core.Transformer.LevelSetter. For us, the significance is
that a binder might be marked to be dropped at the nearest boundary between tail
calls and non-tail calls. For example:
...
GHC.IR.Core.Transformer.LevelSetter. The difference between "this *is at* level
<2,0>" and "this *depends on* level <2,0>" is very important.)
...
floatExpr lam@(Lam (TB _ lam_spec) _)
  = let (bndrs_w_lvls, body) = collectBinders lam
        ...
        -- See GHC.IR.Core.Transformer.LevelSetter.lvlLamBndrs
...
The trouble is that
  ...
  (b) GHC.IR.Core.Transformer.LevelSetter introduces a new bindings site for every float
...
wrapTick t (FB tops ceils defns)
  ...
  where
    ...
    maybe_tick toplvl e
        ...
        -- strings as this violated the Core invariants. See Note [GHC.IR.Core.Syntax
