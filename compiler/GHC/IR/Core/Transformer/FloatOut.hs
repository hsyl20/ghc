...
join points] in GHC.IR.Core.Syntax), so we must be careful with how far we float them. The
mechanism for doing so is the *join ceiling*, detailed in Note [Join ceiling]
in GHC.IR.Core.Transformer.LevelSetter. For us, the significance is that a binder might be marked to be
...
floatBind (NonRec (TB var _) rhs)
  = case (floatRhs var rhs) of { (fs, rhs_floats, rhs') ->
        ...
        -- A tiresome hack:
        -- see Note [Bottoming floats: eta expansion] in GHC.IR.Core.Transformer.LevelSetter
...
TODO: This can probably be solved somehow in GHC.IR.Core.Transformer.LevelSetter. The difference between
