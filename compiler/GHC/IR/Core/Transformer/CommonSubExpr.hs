...
module GHC.IR.Core.Transformer.CommonSubExpr (cseProgram, cseOneExpr) where

#include "HsVersions.h"

import GHC.IR.Core.Transformer.Substitution
import GHC.Entity.Var             ( Var )
import GHC.Entity.Var.Environment ( elemInScopeSet )
import GHC.Entity.Id              ( Id, idType, idInlineActivation, isDeadBinder
                                  , zapIdOccInfo, zapIdUsageInfo, idInlinePragma
                                  , isJoinId )
import GHC.IR.Core.Utils          ( mkAltExpr, eqExpr , exprIsLiteralString
                                  , stripTicksE, stripTicksT, mkTicks )
import GHC.Entity.Type            ( tyConAppArgs )
...
GHC.IR.Core.Transformer.Simplifier.Utils Note [Top level and
...
because the simplifier will do it next.
...
GHC.IR.Core.Transformer.Simplifier.Utils Note [Stable unfoldings and
...
GHC.IR.Core.Syntax). This seems to come up quite seldom, but it happens (first
seen compiling ppHtml in Haddock.Backends.Xhtml).
...
-- Note [Type-let] in GHC.IR.Core.Syntax), in which case we can just substitute.
...
[Core syntax top-level string literals] in GHC.IR.Core.Syntax.
...
combineIdenticalAlts in GHC.IR.Core.Transformer.Simplifier.Utils.prepareAlts.
