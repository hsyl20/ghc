...
import GHC.Entity.Id               ( Id, idType, idInlineActivation, isDeadBinder
                        ...
                        , isJoinId )
import GHC.IR.Core.Utils        ( mkAltExpr, eqExpr
                        , exprIsLiteralString
...
the binding instead?  Well, not at top level! See GHC.IR.Core.Transformer.Simplifier.Utils
...
GHC.IR.Core.Transformer.Simplifier.Utils Note [Stable unfoldings and postInlineUnconditionally].
...
GHC.IR.Core.Syntax). This seems to come up quite seldom, but it happens (first seen
...
[GHC.IR.Core.Syntax top-level string literals] in GHC.IR.Core.Syntax.
...
combineIdenticalAlts in GHC.IR.Core.Transformer.Simplifier.Utils.prepareAlts.  The basic idea is
to transform
