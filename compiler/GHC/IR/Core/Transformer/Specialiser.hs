...
{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transformer.Specialiser ( specProgram, specUnfolding ) where

#include "HsVersions.h"

import GHC.Entity.Id
import GHC.IR.Haskell.TypeChecker.Type hiding( substTy )
import GHC.Entity.Type   hiding( substTy, extendTvSubstList )
import GHC.Entity.Module( Module, HasModule(..) )
import GHC.Entity.Coercion( Coercion )
           ...
           {    -- Figure out the type of the specialised function
             let body_ty = applyTypeToArgs rhs fn_type rule_args
                 (lam_args, app_args)           -- Add a dummy argument if body_ty is unlifted
                   | isUnliftedType body_ty     -- C.f. GHC.IR.Core.Transformer.WorkerWrapper.Utils.mkWorkerArgs
                   , not (isJoinId fn)
...
Thing, but it's what happens now; see GHC.IR.Core.Transformer.Simplifier.Utils Note [Casts and
