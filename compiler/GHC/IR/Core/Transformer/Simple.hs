...
{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transformer.Simple (
...
import GHC.IR.Core.Analyser.Arity     ( joinRhsArity, etaExpandToJoinPoint )

import GHC.IR.Core.Syntax
import GHC.IR.Core.Transformer.Substitution
import GHC.IR.Core.Utils
import GHC.IR.Core.Analyser.FreeVars
import GHC.IR.Core.Printer            ( pprCoreBindings, pprRules )
import GHC.IR.Core.Analyser.Occurence ( occurAnalyseExpr, occurAnalysePgm )
import GHC.Entity.Literal             ( Literal(MachStr) )
import GHC.Entity.Id
import GHC.Entity.Var                 ( varType )
...
import GHC.Entity.Type         hiding ( substTy, extendTvSubst, extendCvSubst
                                      , extendTvSubstList , isInScope
                                      , substTyVarBndr, cloneTyVarBndr )
import GHC.Entity.Coercion     hiding ( substCo, substCoVarBndr )
import GHC.Entity.TypeConstructor     ( tyConArity )
...
simple_bind_pair env@(SOE { soe_inl = inl_env, soe_subst = subst })
  ...
  where
    ...
    pre_inline_unconditionally
         ...
         --     in GHC.IR.Core.Transformer.Simplifier.Utils
...
In GHC.IR.Core.Transformer.Simplifier.Utils, Note [Top level and postInlineUnconditionally], we
...
--
-- It's important to zap fragile OccInfo (which
-- GHC.IR.Core.Transformer.Substitution.substIdBndr carefully does not do)
-- because simplOptExpr invalidates it
...
GHC.IR.Haskell.TypeChecker.Instance) we mark recursive things as INLINE but the
...
they are represented as unpackCString# "abc"# by
GHC.IR.Core.Syntax.Make.mkStringExprFS, or unpackCStringUtf8# when the literal
contains multi-byte UTF8 characters.
...
exprIsConApp_maybe (in_scope, id_unf) expr
  ...
  where
    ...
    ----------------------------
    -- Operations on the (Either InScopeSet GHC.IR.Core.Transformer.Substitution)
    -- The Left case is wildly dominant
    subst_co (Left {}) co = co
    subst_co (Right s) co = GHC.IR.Core.Transformer.Substitution.substCo s co
...
-- GHC.IR.Core.Syntax.Make.mkStringExprFS turns those into [] automatically, but
-- just in case something else in GHC generates a string literal directly.
...
than the ordinary arity of the dfun: see Note [DFun unfoldings] in
GHC.IR.Core.Syntax
-}
...
-- C.f. simplCast in GHC.IR.Core.Transformer.Simplifier
...
pushCoTyArg co ty
  ...
  where
    ...
    co1 = mkNthCo 0 co
       ...
       -- See the NthCo case in GHC.IR.Core.Analyser.Lint.
