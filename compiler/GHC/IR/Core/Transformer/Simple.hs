...
import GHC.Entity.Type     hiding ( substTy, extendTvSubst, extendCvSubst, extendTvSubstList
...
simple_bind_pair env@(SOE { soe_inl = inl_env, soe_subst = subst })
  ...
  where
    ...
    pre_inline_unconditionally
       ...
       | isExportedId in_bndr     = False    --     in GHC.IR.Core.Transformer.Simplifier.Utils
       | stable_unf               = False
...
--
-- It's important to zap fragile OccInfo (which GHC.IR.Core.Transformer.Substitution.substIdBndr
...
GHC.IR.Haskell.TypeChecker.Instance) we mark recursive things as INLINE but the recursion
...
they are represented as unpackCString# "abc"# by GHC.IR.Core.Syntax.Make.mkStringExprFS, or
...
-- This is not possible with user-supplied empty literals, GHC.IR.Core.Syntax.Make.mkStringExprFS
...
than the ordinary arity of the dfun: see Note [DFun unfoldings] in GHC.IR.Core.Syntax
-}
