...
The code for *top-level* bindings is in GHC.IR.Interface.Transformer.Tidier.
-}

{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transformer.Tidier (
...
import GHC.IR.Core.Syntax.Force ( seqUnfolding )
import GHC.IR.Core.Analyser.Arity
import GHC.Entity.Id
import GHC.Entity.Id.Info
import GHC.Entity.Demand ( zapUsageEnvSig )
import GHC.Entity.Type( tidyType, tidyTyCoVarBndr )
import GHC.Entity.Coercion( tidyCo )
import GHC.Entity.Var
import GHC.Entity.Var.Environment
import GHC.Entity.Unique.FiniteMap
import GHC.Entity.Name hiding (tidyNameOcc)
...
tidyUnfolding tidy_env
  ...
  = seqIt $ unf { uf_tmpl = tidyExpr tidy_env unf_rhs }    -- Preserves OccInfo
    ...
    -- pre-tidied expression forever (GHC.Compiler.CoreToInterface doesn't look at them)
