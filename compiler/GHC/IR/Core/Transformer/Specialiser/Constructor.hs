...
ToDo [Oct 2013]
~~~~~~~~~~~~~~~
1. Nuke ForceSpecConstr for good (it is subsumed by GHC.Entity.Types.SPEC in ghc-prim)
2. Nuke NoSpecConstr
...
module GHC.IR.Core.Transformer.Specialiser.Constructor (
        specConstrProgram,
...
import GHC.IR.Core.Transformer.Inliner       ( couldBeSmallEnoughToInline )
import GHC.IR.Core.Analyser.FreeVars          ( exprsFreeVarsList )
import GHC.IR.Core.Pipeline
import GHC.Entity.Literal          ( litIsLifted )
import GHC.Entity.Types         ( ModGuts(..) )
import GHC.IR.Core.Transformer.WorkerWrapper.Utils            ( isWorkerSmallEnough, mkWorkerArgs )
...
import GHC.Entity.TypeConstructor            ( tyConName )
import GHC.Entity.Id
import GHC.IR.Core.Printer          ( pprParendExpr )
import GHC.IR.Core.Syntax.Make           ( mkImpossibleExpr )
...
deprecated):
  ...
  1) Add a parameter of type GHC.Entity.Types.SPEC (from ghc-prim) to the loop body.
...
(Note this is the exact same definition of GHC.Entity.Types.SPEC, just
        ...
        let {
          ds_sht =
            ...
            TSpecConstr_DoubleInline.recursive
              (GHC.Entity.Types.: @ GHC.Entity.Types.Int x_af7 wild_X6) (q'_afe, p'_afd)
            } } in
  ...
  = do  { mb_pats <- mapM (callToPats env bndr_occs) calls
        ...
        ; let have_boring_call = any isNothing mb_pats
              ...
              too_big (vars,_) = not (isWorkerSmallEnough (sc_dflags env) vars)
                  ...
                  -- See Note [Limit w/w arity] in GHC.IR.Core.Transformer.WorkerWrapper.Utils
...
argToPat env in_scope val_env (Let _ arg) arg_occ
  = argToPat env in_scope val_env arg arg_occ
        -- See Note [Matching lets] in GHC.IR.Core.Transformer.Rules
        -- Look through let expressions
        -- e.g.         f (let v = rhs in (v,w))
        -- Here we can specialise for f (v,w)
        -- because the rule-matcher will look through the let.

{- Disabled; see Note [Matching cases] in GHC.IR.Core.Transformer.Rules
