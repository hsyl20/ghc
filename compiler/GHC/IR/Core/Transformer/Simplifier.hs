...
module GHC.IR.Core.Transformer.Simplifier
   ( simplTopBinds
   , simplExpr
   , simplRules
   )
...
import GHC.IR.Core.Transformer.Simplifier.Monad
import GHC.Entity.Type hiding      ( substTy, substTyVar, extendTvSubst, extendCvSubst )
import GHC.IR.Core.Transformer.Simplifier.Environment
import GHC.IR.Core.Transformer.Simplifier.Utils
import GHC.IR.Core.Analyser.Occurence        ( occurAnalyseExpr )
import GHC.Entity.FamilyInstance       ( FamInstEnv )
import GHC.Entity.Literal          ( litIsLifted ) --, mkMachInt ) -- temporalily commented out. See #8326
import GHC.Entity.Id
import GHC.Entity.Id.Make             ( seqId )
import GHC.IR.Core.Syntax.Make           ( mkImpossibleExpr, castBottomExpr )
import GHC.Entity.Id.Info
import GHC.Entity.Name             ( Name, mkSystemVarName, isExternalName, getOccFS )
import GHC.Entity.Coercion hiding  ( substCo, substCoVar )
import GHC.Entity.Coercion.Optimise      ( optCoercion )
import GHC.Entity.FamilyInstance       ( topNormaliseType_maybe )
import GHC.Entity.DataConstructor          ( DataCon, dataConWorkId, dataConRepStrictness, dataConRepArgTys )
--import GHC.Entity.TypeConstructor            ( isEnumerationTyCon ) -- temporalily commented out. See #8326
import GHC.IR.Core.Pipeline        ( Tick(..), SimplifierMode(..) )
import GHC.IR.Core.Syntax
import GHC.Entity.Demand           ( StrictSig(..), dmdTypeDepth, isStrictDmd )
import GHC.IR.Core.Printer          ( pprCoreExpr )
import GHC.IR.Core.Transformer.Inliner
import GHC.IR.Core.Utils
import GHC.IR.Core.Analyser.Arity
import GHC.IR.Core.Transformer.Simple         ( pushCoTyArg, pushCoValArg
                        , joinPointBinding_maybe, joinPointBindings_maybe )
--import GHC.Builtin.Primitive.Operations           ( tagToEnumKey ) -- temporalily commented out. See #8326
import GHC.IR.Core.Transformer.Rules            ( mkRuleInfo, lookupRule, getRules )
--import GHC.Builtin.Primitive.Types          ( intPrimTy ) -- temporalily commented out. See #8326
import GHC.Entity.BasicTypes       ( TopLevelFlag(..), isNotTopLevel, isTopLevel,
                          RecFlag(..) )
import GHC.Utils.Monad       ( foldlM, mapAccumLM, liftIO )
import GHC.Data.Maybe           ( isJust, fromJust, orElse, catMaybes )
--import GHC.Entity.Unique           ( hasKey ) -- temporalily commented out. See #8326
...
import GHC.Entity.Module          ( moduleName, pprModuleName )

{-
The guts of the simplifier is in this module, but the driver loop for
the simplifier is in GHC.IR.Core.Transformer.Pipeline
...
expression. This is wrapped as a datatype GHC.IR.Core.Transformer.Simplifier.Utils.FloatsWith.
  ...
  = ASSERT( isId bndr )
    ...
    do  { let   rhs_env     = rhs_se `setInScopeAndZapFloats` env
        ...
        ; (body_env, tvs') <- simplBinders rhs_env tvs
                -- See Note [Floating and type abstraction] in GHC.IR.Core.Transformer.Simplifier.Utils
...
-- Precondition: rhs satisfies the let/app invariant
--               See Note [GHC.IR.Core.Syntax let/app invariant] in GHC.IR.Core.Syntax
    ...
    go n_val_args env (Tick t rhs)
        ...
        = do { (is_exp, env', rhs') <- go n_val_args (zapFloats env) rhs
             ...
             ; let tickIt (id, expr)
                       ...
                       -- strings. See Note [GHC.IR.Core.Syntax top-level string literals].
...
See Note [GHC.IR.Core.Syntax top-level string literals] in GHC.IR.Core.Syntax.
...
completeBind env top_lvl is_rec mb_cont old_bndr new_bndr new_rhs
 ...
 = ASSERT( isId new_bndr )
   do { let old_info = idInfo old_bndr
        ...
        -- See Note [Eta-expanding at let bindings] in GHC.IR.Core.Transformer.Simplifier.Utils
...
--       }
  ...
  getDoneId (DoneEx e _) = getIdFromTrivialExpr e -- Note [substTickish] in GHC.IR.Core.Transformer.Substitution
...
--         ww1_s12Z,
--         PTTrees.PT
--           @ GHC.Entity.Types.Char @ GHC.Entity.Types.Int wild2_Xj ww2_s130 r_ajf)
--        }
--
...
--
-- Precondition: rhs satisfies the let/app invariant
--               Note [GHC.IR.Core.Syntax let/app invariant] in GHC.IR.Core.Syntax
--
...
rebuildCall env (ArgInfo { ai_fun = fun, ai_args = rev_args, ai_strs = [] }) cont
  ...
  -- that the call is definitely bottom; see GHC.IR.Core.Transformer.Simplifier.Utils.mkArgInfo
...
Historical note: previously we did occ-anal the rules in
GHC.IR.Core.Transformer.Rules, but failed to occ-anal the OutExpr arguments,
which led to the nasty performance problem described above.
...
The code in GHC.IR.Core.Transformer.Simplifier.Utils.prepareAlts has the effect of generalise this
...
Note that GHC.IR.Core.Transformer.Simplifier.Utils.mkCase combines identical RHSs.  So
...
See also Note [Empty case alternatives] in GHC.IR.Core.Syntax.
...
an eval'd function] in GHC.IR.Core.Utils.)
...
simplAlt env scrut' _ case_bndr' cont' (DataAlt con, vs, rhs)
  ...
  where
        ...
        -- See Note [Data-con worker strictness] in GHC.Entity.Id.Make
    add_evals the_strs
        = go vs the_strs
        where
          go [] [] = []
...
NB: take great care when doing strictness analysis;
    see Note [Lambda-bound unfoldings] in GHC.IR.Core.Analyser.Demand.
  ...
  = is_bottoming `seq`  -- See Note [Force bottoming field]
    do { dflags <- getDynFlags
       ; return (mkUnfolding dflags InlineRhs is_top_lvl is_bottoming new_rhs) }
            ...
            --         (b) In GHC.IR.Interface.Transformer.Tidier we currently assume that, if we want to
...
simplUnfolding env top_lvl mb_cont id unf
              ...
              ; case guide of
                  UnfWhen { ug_arity = arity, ug_unsat_ok = sat_ok }  -- Happens for INLINE things
                     -> let guide' = UnfWhen { ug_arity = arity, ug_unsat_ok = sat_ok
                        ...
                        -- Note [Single-method classes] in GHC.IR.Haskell.TypeChecker.Instance.
                        -- A test case is Trac #4138
                        in return (mkCoreUnfolding src is_top_lvl expr' guide')
                            -- See Note [Top-level flag on inline rules] in GHC.IR.Core.Transformer.Inliner
  ...
  where
    ...
    rule_env     = updMode (updModeForStableUnfoldings act) env
         -- See Note [Simplifying inside stable unfoldings] in GHC.IR.Core.Transformer.Simplifier.Utils
