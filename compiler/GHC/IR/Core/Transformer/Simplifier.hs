...
import GHC.Entity.Type hiding      ( substTy, substTyVar, extendTvSubst
                                   , extendCvSubst )
...
import GHC.Entity.Literal          ( litIsLifted )
...
import GHC.Entity.Name             ( Name, mkSystemVarName, isExternalName
                                   , getOccFS )
...
import GHC.Entity.DataConstructor  ( DataCon, dataConWorkId
                                   , dataConRepStrictness, dataConRepArgTys )
...
-- temporalily commented out. See #8326
--import GHC.Builtin.Primitive.Operations ( tagToEnumKey )
--import GHC.Entity.TypeConstructor       ( isEnumerationTyCon )
--import GHC.Builtin.Primitive.Types      ( intPrimTy )
--import GHC.Entity.Unique                ( hasKey )
--import GHC.Entity.Literal               ( mkMachInt )

import GHC.IR.Core.Transformer.Rules ( mkRuleInfo, lookupRule, getRules )
import GHC.Entity.BasicTypes         ( TopLevelFlag(..), isNotTopLevel
                                     , isTopLevel, RecFlag(..) )
...
expression. This is wrapped as a datatype
GHC.IR.Core.Transformer.Simplifier.Utils.FloatsWith.
  ...
  = ASSERT( isId bndr )
    ...
    do  { let   rhs_env     = rhs_se `setInScopeAndZapFloats` env
        ...
        ; (body_env, tvs') <- simplBinders rhs_env tvs
                ...
                -- GHC.IR.Core.Transformer.Simplifier.Utils
...
-- Precondition: rhs satisfies the let/app invariant
--               See Note [Core syntax let/app invariant] in GHC.IR.Core.Syntax
    ...
    go n_val_args env (Tick t rhs)
        ...
        = do { (is_exp, env', rhs') <- go n_val_args (zapFloats env) rhs
             ...
             ; let tickIt (id, expr)
                       ...
                       -- strings. See Note [Core syntax top-level string
                       -- literals].
...
See Note [Core syntax top-level string literals] in GHC.IR.Core.Syntax.
...
completeBind env top_lvl is_rec mb_cont old_bndr new_bndr new_rhs
 ...
 = ASSERT( isId new_bndr )
   do { let old_info = idInfo old_bndr
        ...
        -- See Note [Eta-expanding at let bindings] in
        -- GHC.IR.Core.Transformer.Simplifier.Utils
...
--       }
  ...
  getDoneId (DoneEx e _) = getIdFromTrivialExpr e
  ...
  where
       ...
       addCoerce co cont@(ApplyToVal { sc_arg = arg, sc_env = arg_se
         ...
         , not (isTypeLevPoly new_ty)  -- without this check, we get a lev-poly arg
                                       ...
                                       -- invariants] in GHC.IR.Core.Syntax
...
--
-- Precondition: rhs satisfies the let/app invariant
--               Note [Core syntax let/app invariant] in GHC.IR.Core.Syntax
--
...
rebuildCall env (ArgInfo { ai_fun = fun, ai_args = rev_args, ai_strs = [] }) cont
  ...
  -- GHC.IR.Core.Transformer.Simplifier.Utils.mkArgInfo
  ...
  = is_bottoming `seq`  -- See Note [Force bottoming field]
    do { dflags <- getDynFlags
       ; return (mkUnfolding dflags InlineRhs is_top_lvl is_bottoming new_rhs) }
            ...
            --         (b) In GHC.IR.Interface.Transformer.Tidier we currently
            --             assume that, if we want to expose the unfolding then
            --             indeed we *have* an unfolding to expose.  (We could
