...
\section[GHC.IR.Core.Transformer.Pipeline]{Driver for simplifying @Core@ programs}
-}

{-# LANGUAGE CPP #-}

module GHC.IR.Core.Transformer.Pipeline ( core2core, simplifyExpr ) where
...
import GHC.IR.Core.Transformer.CommonSubExpr ( cseProgram )
import GHC.IR.Core.Transformer.Rules     ( mkRuleBase, unionRuleBase
                                         , extendRuleBaseList , ruleCheckProgram
                                         , addRuleInfo )
import GHC.IR.Core.Printer               ( pprCoreBindings, pprCoreExpr )
import GHC.IR.Core.Analyser.Occurence    ( occurAnalysePgm, occurAnalyseExpr )
import GHC.Entity.Id.Info
import GHC.IR.Core.Analyser.Stats        ( coreBindsSize, coreBindsStats, exprSize )
import GHC.IR.Core.Utils                 ( mkTicks, stripTicksTop )
import GHC.IR.Core.Analyser.Lint         ( endPass, lintPassResult, dumpPassResult,
                                           lintAnnots )
import GHC.IR.Core.Transformer.Simplifier         ( simplTopBinds, simplExpr, simplRules )
import GHC.IR.Core.Transformer.Simplifier.Utils   ( simplEnvForGHCi, activeRule )
...
import GHC.IR.Core.Transformer.FloatIn   ( floatInwards )
import GHC.IR.Core.Transformer.FloatOut  ( floatOutwards )
import GHC.Entity.FamilyInstance
import GHC.Entity.Id
import GHC.Utils.Error        ( withTiming )
import GHC.Entity.BasicTypes  ( CompilerPhase(..), isDefaultInlinePragma )
import GHC.Entity.Var.Set
import GHC.Entity.Var.Environment
import GHC.IR.Core.Transformer.LiberateCase     ( liberateCase )
import GHC.IR.Core.Transformer.StaticArgument   ( doStaticArgs )
import GHC.IR.Core.Transformer.Specialiser      ( specProgram)
import GHC.IR.Core.Transformer.Specialiser.Constructor ( specConstrProgram)
import GHC.IR.Core.Analyser.Demand              ( dmdAnalProgram )
import GHC.IR.Core.Analyser.CallArity           ( callArityAnalProgram )
import GHC.IR.Core.Transformer.WorkerWrapper    ( wwTopBinds )
import GHC.IR.Core.Transformer.Vectoriser       ( vectorise )
import GHC.Entity.SrcLoc
import GHC.Utils
import GHC.Entity.Module

import GHC.Data.Maybe
import GHC.Entity.Unique.Supply       ( UniqSupply, mkSplitUniqSupply
                                      , splitUniqSupply )
...
import GHC.Plugin.Types                 ( installCoreToDos )
#else
    ...
    -- after this before anything else
        ...
        runWhen liberate_case (CoreDoPasses [
            ...
            simpl_phase 0 ["post-liberate-case"] max_iter
                        ...
                        -- Reason: see Note [Shadowing] in
                        -- GHC.IR.Core.Transformer.Specialiser.Constructor
        ...
        -- has run at all.
        -- See Note [Final Demand Analyser run] in GHC.IR.Core.Analyser.Demand
