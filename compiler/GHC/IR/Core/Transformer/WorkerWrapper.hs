...
{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transformer.WorkerWrapper ( wwTopBinds ) where

import GHC.IR.Core.Syntax
import GHC.IR.Core.Transformer.Inliner       ( certainlyWillInline, mkWwInlineRule, mkWorkerUnfolding )
...
in work_fn! See GHC.IR.Core.Transformer.Inliner.mkWorkerUnfolding.
...
inline into the worker's unfolding: see GHC.IR.Core.Transformer.Simplifier.Utils
 ...
 * We want them to be still there at the end of GHC.IR.Core.Analyser.Demand, so that
...
Note [Final Demand Analyser run] in GHC.IR.Core.Analyser.Demand).
...
in w/w-ing because we'll simply add (\y:Void#), see GHC.IR.Core.Transformer.WorkerWrapper.Utils.mkWorerArgs.
