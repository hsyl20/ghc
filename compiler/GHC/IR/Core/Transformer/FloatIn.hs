...
************************************************************************
*                                                                      *
\section[GHC.IR.Core.Transformer.FloatIn]{Floating Inwards pass}
*                                                                      *
************************************************************************
...
module GHC.IR.Core.Transformer.FloatIn ( floatInwards ) where
...
import GHC.IR.Core.Pipeline        ( CoreM )
import GHC.Entity.Id               ( isOneShotBndr, idType, isJoinId, isJoinId_maybe )
...
import GHC.Entity.BasicTypes       ( RecFlag(..), isRec )
...
GHC.IR.Core.Transformer.LevelSetter module, which tags things with their level
numbers.
\item
...
* Which unlifted cases do we float? See GHC.Builtin.Primitive.Operations
  ...
  where
    ...
    go (bind_w_fvs@(FB bndrs bind_fvs bind) : binds) drop_boxes@(here_box : fork_boxes)
        ...
        where
          ...
          new_fork_boxes = zipWithEqual "GHC.IR.Core.Transformer.FloatIn.sepBinds" insert_maybe
