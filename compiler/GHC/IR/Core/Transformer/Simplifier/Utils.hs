...
\section[GHC.IR.Core.Transformer.Simplifier.Utils]{The simplifier utilities}
-}

{-# LANGUAGE CPP #-}

module GHC.IR.Core.Transformer.Simplifier.Utils (
        -- Rebuilding
...
import qualified GHC.IR.Core.Transformer.Substitution
...
import GHC.Entity.Demand
import GHC.IR.Core.Transformer.Simplifier.Monad
import GHC.Entity.Type     hiding( substTy )
import GHC.Entity.Coercion hiding( substCo )
import GHC.Entity.DataConstructor          ( dataConWorkId, isNullaryRepDataCon )
...
import GHC.IR.Core.Transformer.ConstantFolder
...
mkRhsStop :: OutType -> SimplCont       -- See Note [RHS of lets] in GHC.IR.Core.Transformer.Inliner
...
contArgs cont
  ...
  | otherwise = go [] cont
  where
    lone (ApplyToTy  {}) = False  -- See Note [Lone variables] in GHC.IR.Core.Transformer.Inliner
      ...
      = case splitStrictSig (idStrictness fun) of
          (demands, result_info)
                ...
                ->      -- Enough args, use the strictness given.
                        ...
                        -- inlining lone variables, so its ok (see GHC.IR.Core.Transformer.Simplifier.Utils.analyseCont)
...
interestingCallContext cont
  ...
  where
    ...
    interesting (ApplyToVal {}) = ValAppCtxt
        ...
        -- in GHC.IR.Core.Transformer.Inliner
...
(in GHC.IR.Core.Analyser.Arity.findRhsArity), which we can't do for free-floating lambdas
...
and treat them specially. The real work is done in GHC.IR.Core.Transformer.Simplifier.Utils.abstractFloats,
...
abstractFloats main_tvs body_env body
  = ASSERT( notNull body_floats )
    do  { (subst, float_binds) <- mapAccumLM abstract empty_subst body_floats
        ; return (float_binds, GHC.IR.Core.Transformer.Substitution.substExpr (text "abstract_floats1") subst body) }
  where
    main_tv_set = mkVarSet main_tvs
    body_floats = getFloatBinds body_env
    empty_subst = GHC.IR.Core.Transformer.Substitution.mkEmptySubst (seInScope body_env)

    abstract :: GHC.IR.Core.Transformer.Substitution.Subst -> OutBind -> SimplM (GHC.IR.Core.Transformer.Substitution.Subst, OutBind)
    abstract subst (NonRec id rhs)
      = do { (poly_id, poly_app) <- mk_poly tvs_here id
           ; let poly_rhs = mkLams tvs_here rhs'
                 subst'   = GHC.IR.Core.Transformer.Substitution.extendIdSubst subst id poly_app
           ; return (subst', (NonRec poly_id poly_rhs)) }
      where
        rhs' = GHC.IR.Core.Transformer.Substitution.substExpr (text "abstract_floats2") subst rhs
    ...
    abstract subst (Rec prs)
       = do { (poly_ids, poly_apps) <- mapAndUnzipM (mk_poly tvs_here) ids
            ; let subst' = GHC.IR.Core.Transformer.Substitution.extendSubstList subst (ids `zip` poly_apps)
                  poly_rhss = [mkLams tvs_here (GHC.IR.Core.Transformer.Substitution.substExpr (text "abstract_floats3") subst' rhs)
                              | rhs <- rhss]
      ...
      = do { uniq <- getUniqueM
           ; let  poly_name = setNameUnique (idName var) uniq           -- Keep same name
                  ...
                  poly_id   = transferPolyIdInfo var tvs_here $
                              -- Note [transferPolyIdInfo] in GHC.Entity.Id
...
* In dataToTag we might need to make up some fake binders;
  see Note [caseRules for dataToTag] in GHC.IR.Core.Transformer.ConstantFolder
-}
