...
module GHC.IR.Core.Transformer.Inliner (
        ...
        -- Reexport from GHC.IR.Core.Transformer.Substitution (it only live there so it can be used
...
import GHC.IR.Core.Transformer.Simple
import GHC.IR.Core.Analyser.Arity       ( manifestArity )
import GHC.IR.Core.Utils
import GHC.Entity.Id
import GHC.Entity.Demand          ( isBottomingSig )
...
import GHC.Entity.BasicTypes       ( Arity, InlineSpec(..), inlinePragmaSpec )
...
and similar friends.  See Note [Bottoming floats] in GHC.IR.Core.Transformer.LevelSetter.
...
(4) We must be much more cautious about arity-zero things. Consider
    ...
    but it doesn't fire for top-level things; see GHC.IR.Core.Transformer.Simplifier.Utils
...
litSize :: Literal -> Int
-- Used by GHC.IR.Core.Transformer.Inliner.sizeExpr
    ...
    {- Arity: 3, HasNoCafRefs, Strictness: SLL,
       Unfolding: (\ @ a $dOrd :: GHC.Classes.Ord a x :: a y :: a ->
                   case @ a GHC.Classes.<= @ a $dOrd x y of wild {
                     GHC.Entity.Types.False -> y GHC.Entity.Types.True -> x }) -}
