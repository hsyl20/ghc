...
{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transformer.StaticArgument ( doStaticArgs ) where
...
GHC.Base.until =
  \ (@ a_aiK)
    (p_a6T :: a_aiK -> GHC.Entity.Types.Bool)
    (f_a6V :: a_aiK -> a_aiK)
    (x_a6X :: a_aiK) ->
    letrec {
        ...
        \ (x_a6X :: a_aiK) ->
          let {
            sat_shadow_r17 :: forall a_a3O.
                              (a_a3O -> GHC.Entity.Types.Bool) -> (a_a3O -> a_a3O) -> a_a3O -> a_a3O
            []
            sat_shadow_r17 =
              \ (@ a_aiK)
                (p_a6T :: a_aiK -> GHC.Entity.Types.Bool)
          ...
          case p_a6T x_a6X of wild_X3y [ALWAYS Dead Nothing] {
            GHC.Entity.Types.False -> GHC.Base.until @ a_aiK p_a6T f_a6V (f_a6V x_a6X);
            GHC.Entity.Types.True -> x_a6X
          }; } in
