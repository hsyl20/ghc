...
import GHC.IR.Core.Syntax              -- All of it
import GHC.Entity.Module               ( Module, ModuleSet, elemModuleSet )
import GHC.IR.Core.Transformer.Substitution
import GHC.IR.Core.Transformer.Simple  ( exprIsLambda_maybe )
import GHC.IR.Core.Analyser.FreeVars   ( exprFreeVars, exprsFreeVars
                                       , bindFreeVars , rulesFreeVarsDSet
                                       , exprsOrphNames , exprFreeVarsList )
import GHC.IR.Core.Utils               ( exprType, eqExpr, mkTick, mkTicks
                                       , stripTicksTopT, stripTicksTopE
                                       , isJoinBind )
import GHC.IR.Core.Printer             ( pprRules )
import GHC.Entity.Type                 ( Type, mkTCvSubst )
import qualified GHC.Entity.Type as Type
import GHC.IR.Haskell.TypeChecker.Type ( tcSplitTyConApp_maybe )
import GHC.Builtin.Types               ( anyTypeOfKind )
import GHC.Entity.Coercion
import GHC.IR.Core.Transformer.Tidier  ( tidyRules )
import GHC.Entity.Id
import GHC.Entity.Id.Info              ( RuleInfo( RuleInfo ) )
...
import qualified GHC.Utils.Unify as Unify
import GHC.Entity.BasicTypes           ( Activation, CompilerPhase, isActive
                                       , pprRuleName )
...
* After the desugarer:
   ...
   - Locally-declared rules for locally-declared Ids are attached to
     ...
     GHC.Compiler.HaskellToCore.Binding

* GHC.IR.Interface.Transformer.Tidier strips off all the rules from local Ids
  and adds them to mg_rules, so that the ModGuts has *all* the locally-declared
  rules.
...
* At the moment (c) is carried in a reader-monad way by the GHC.IR.Core.Pipeline.
...
* So in the outer simplifier loop, we combine (b-d) into a single
  RuleBase, reading
     (b) from the ModGuts,
     (c) from the GHC.IR.Core.Pipeline, and
...
-- compiled. See also 'GHC.IR.Core.Syntax.CoreRule'
...
-- It's here rather than in GHC.IR.Core.Printer because it calls tidyRules
...
but that isn't quite right:
     ...
     - The rules in GHC.IR.Core.Transformer.ConstantFolder.builtinRules should
       be active even in the module defining the Id (when it's a LocalId), but
...
-- RuleBase itself is defined in GHC.IR.Core.Syntax, along with CoreRule
...
Our cunning plan is this:
  ...
  * We use GHC.IR.Core.Transformer.Substitution.substBind to freshen the binding, using an
