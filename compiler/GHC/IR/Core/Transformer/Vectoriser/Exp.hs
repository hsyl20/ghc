...
module GHC.IR.Core.Transformer.Vectoriser.Exp
...
import GHC.IR.Core.Transformer.Vectoriser.Type.Type
import GHC.IR.Core.Transformer.Vectoriser.Var
import GHC.IR.Core.Transformer.Vectoriser.Convert
import GHC.IR.Core.Transformer.Vectoriser.Vect
import GHC.IR.Core.Transformer.Vectoriser.Env
import GHC.IR.Core.Transformer.Vectoriser.Monad
import GHC.IR.Core.Transformer.Vectoriser.Builtins
import GHC.IR.Core.Transformer.Vectoriser.Utils
...
import GHC.Entity.BasicTypes( isStrongLoopBreaker )
...
vectExpr aexpr@(_, AnnLam _ _)
  ...
  -- SPECIAL CASE: Vectorise/lift 'patError @ ty err' by only
  --   vectorising/lifting the type 'ty'; its only purpose is to abort the
  --   program, but we need to adjust the type to keep GHC.IR.Core.Analyser.Lint
  --   happy.
