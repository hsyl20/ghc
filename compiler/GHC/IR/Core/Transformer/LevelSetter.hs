...
\section{GHC.IR.Core.Transformer.LevelSetter}
...
module GHC.IR.Core.Transformer.LevelSetter (
        setLevels,
...
import GHC.IR.Core.Analyser.Arity        ( exprBotStrictness_maybe )
import GHC.IR.Core.Analyser.FreeVars          -- all of it
import GHC.IR.Core.Transformer.Substitution
import GHC.IR.Core.Syntax.Make           ( sortQuantVars )
...
import GHC.Entity.Literal          ( litIsTrivial )
import GHC.Entity.Demand           ( StrictSig, Demand, isStrictDmd, splitStrictSig, increaseStrictSigArity )
import GHC.Entity.Name             ( getOccName, mkSystemVarName )
import GHC.Entity.OccName          ( occNameString )
import GHC.Entity.Type             ( Type, mkLamTypes, splitTyConApp_maybe )
import GHC.Entity.BasicTypes       ( Arity, RecFlag(..), isRec )
import GHC.Entity.DataConstructor          ( dataConOrigResTy )
...
-- In GHC.IR.Core.Transformer.LevelSetter we do set the context to (Level 0 0) when we get to an InlineMe
...
lvlExpr env (_, AnnType ty)     = return (Type (GHC.IR.Core.Transformer.Substitution.substTy (le_subst env) ty))
...
lvlExpr env expr@(_, AnnLam {})
  ...
  where
    ...
    (new_env, new_bndrs) = lvlLamBndrs env1 (le_ctxt_lvl env) bndrs1
        ...
        -- [See GHC.IR.Core.Transformer.LevelSetter rev 1.50 for a version with this approach.]
...
lvlApp env orig_expr ((_,AnnVar fn), args)
  ...
  where
    ...
    left _ _ _                   = panic "GHC.IR.Core.Transformer.LevelSetter.lvlExpr.left"
...
lvlMFE env _ (_, AnnType ty)
  = return (Type (GHC.IR.Core.Transformer.Substitution.substTy (le_subst env) ty))
...
etaExpand during GHC.IR.Core.Transformer.LevelSetter because it works over a decorated form of
...
-- So named only to avoid the name clash with GHC.IR.Core.Transformer.Substitution.substBndrs
...
lvlLamBndrs env lvl bndrs
  ...
  where
    ...
    is_major bndr = isId bndr && not (isProbablyOneShotLambda bndr)
       ...
       -- See Note [Computing one-shot info] in GHC.Entity.Demand
  ...
  where
    ...
    mk_poly_bndr bndr uniq = transferPolyIdInfo bndr abs_vars $
                           ...
                           where
                             ...
                             poly_ty = mkLamTypes abs_vars (GHC.IR.Core.Transformer.Substitution.substTy subst (idType bndr))
