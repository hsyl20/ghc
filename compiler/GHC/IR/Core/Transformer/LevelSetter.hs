...
import GHC.IR.Core.Transformer.Substitution as CoreSubst
...
-- In GHC.IR.Core.Transformer.LevelSetter we do set the context to (Level 0 0)
-- when we get to an InlineMe which discourages floating out.
...
lvlExpr env expr@(_, AnnLam {})
  ...
  where
    ...
    (new_env, new_bndrs) = lvlLamBndrs env1 (le_ctxt_lvl env) bndrs1
        ...
        -- [See GHC.IR.Core.Transformer.LevelSetter rev 1.50 for a version with
        -- this approach.]
...
etaExpand during GHC.IR.Core.Transformer.LevelSetter because it works over a
decorated form of CoreExpr.  So we do the eta expansion later, in FloatOut.
...
-- So named only to avoid the name clash with
-- GHC.IR.Core.Transformer.Substitution.substBndrs
