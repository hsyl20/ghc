...
-- | Arity and eta expansion
module GHC.IR.Core.Analyser.Arity (
...
import GHC.Entity.TypeConstructor    ( initRecTc, checkRecTc )
...
Why is this important?  Because
  - In GHC.IR.Interface.Transformer.Tidier we use exprArity to fix the *final arity* of
    each top-level Id, and in
  - In CorePrep we use etaExpand on each rhs, so that the visible lambdas
    actually match that arity, which in turn means
    that the StgRhs has the right number of lambdas

An alternative would be to do the eta-expansion in GHC.IR.Interface.Transformer.Tidier, at least
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    at all.  See Note [Single-method classes] in GHC.IR.Haskell.TypeChecker.Instance. SLPJ May 2013
...
See Note [Dictionary-like types] in GHC.IR.Haskell.TypeChecker.Type for why we
...
etaInfoApp subst (Lam v1 e) (EtaVar v2 : eis)
  = etaInfoApp (GHC.IR.Core.Transformer.Substitution.extendSubstWithVar subst v1 v2) e eis

etaInfoApp subst (Cast e co1) eis
  = etaInfoApp subst e (pushCoercion co' eis)
  where
    co' = GHC.IR.Core.Transformer.Substitution.substCo subst co1
...
etaInfoApp subst (Case e b ty alts) eis
  ...
  where
    ...
    ty'   = etaInfoAppTy (GHC.IR.Core.Transformer.Substitution.substTy subst ty) eis
...
subst_expr = substExpr (text "GHC.IR.Core.Analyser.Arity:substExpr")
