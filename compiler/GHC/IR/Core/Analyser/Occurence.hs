...
module GHC.IR.Core.Analyser.Occurence (
...
import GHC.IR.Core.Utils        ( exprIsTrivial, isDefaultAlt, isExpandableApp,
                          stripTicksTopE, mkTicks )
import GHC.Entity.Id
import GHC.Entity.Id.Info
import GHC.Entity.Name( localiseName )
...
import GHC.Entity.Demand           ( argOneShots, argsOneShots )
...
recursive functions] in GHC.IR.Core.Transformer.Simplifier
...
GHC.IR.Core.Syntax), it can still be made a join point with some effort. This is because
...
Note [How instance declarations are translated] in GHC.IR.Haskell.TypeChecker.Instance
...
Annoyingly, we have to approximate GHC.IR.Core.Transformer.Simplifier.Utils.preInlineUnconditionally.
...
--
-- See Note [Invariants for join points] in GHC.IR.Core.Syntax.
