...
Note [Join points]
~~~~~~~~~~~~~~~~~~
We check the rules listed in Note [Invariants on join points] in GHC.IR.Core.Syntax. The
...
-- These are Local things (see Note [Interactively-bound Ids in GHCi] in GHC.Entity.Types).
...
interactiveInScope hsc_env
  = tyvars ++ ids
  where
    -- C.f. GHC.IR.Haskell.TypeChecker.Module.setInteractiveContext, Desugar.deSugarExpr
  ...
  = addLoc (RhsOf binder) $
    ...
    do { ty <- lintRhs binder rhs
       ...
       ; checkL ( isJoinId binder
        ...
        -- computation to perform, see Note [GHC.IR.Core.Syntax top-level string literals].
      ...
      --                  in GHC.IR.Core.Analyser.Demand.  After eta-expansion in CorePrep the rhs is no longer trivial.
...
lintIdUnfolding  _ _ _
  = return ()       -- Do not Lint unstable unfoldings, because that leads
                    -- to exponential behaviour; c.f. GHC.IR.Core.Analyser.FreeVars.idUnfoldingVars
...
lintCoreExpr e@(Case scrut var alt_ty alts) =
  ...
  do { let scrut_diverges = exprIsBottom scrut
     ...
     -- See Note [Rules for floating-point comparisons] in GHC.IR.Core.Transformer.ConstantFolder
...
in, say, GHC.IR.Core.Transformer.Substitution.simpleOptPgm, which in some circumstances can run immediately
before Float Out.

All that said, currently GHC.IR.Core.Transformer.Substitution.simpleOptPgm is the only thing using this
...
"undersaturated" rule (see 'GHC.IR.Core.Analyser.Arity.etaExpandToJoinPointRule'), and in fact
...
Pointer Table from the top-level bindings. See GHC.IR.Core.Transformer.Pipeline Note [Grand
