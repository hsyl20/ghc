...
module GHC.IR.Core.Analyser.Lint (
    ...
    endPass, endPassIO,
    dumpPassResult,
    GHC.IR.Core.Analyser.Lint.dumpIfSet,
 ) where
...
import GHC.IR.Core.Analyser.Stats      ( coreBindsStats )
...
import GHC.IR.Haskell.TypeChecker.Type ( isFloatingTy )
...
import GHC.IR.Core.Printer
import GHC.Utils.Error
import GHC.Entity.Coercion
import GHC.Entity.SrcLoc
import GHC.Entity.Kind
import GHC.Entity.Type as Type
import GHC.Entity.RepType
import GHC.Entity.TypeAndCoercion       -- checks validity of types/coercions
import GHC.Entity.TypeConstructor as TyCon
...
import GHC.Utils
import GHC.Entity.ClassInstance     ( instanceDFunId )
import GHC.Entity.Coercion.Optimise ( checkAxInstCo )
import GHC.Entity.Unique.Supply
import GHC.IR.Core.Analyser.Arity   ( typeArity )
import GHC.Entity.Demand            ( splitStrictSig, isBotRes )
...
That is, use a type let.   See Note [Type let] in GHC.IR.Core.Syntax.
...
GHC.IR.Core.Syntax. The only one that causes any difficulty is the first: All
occurrences must be tail calls. To this end, along with the in-scope set, we
remember in le_joins the subset of in-scope Ids that are valid join ids. For
example:
...
-- These are Local things (see Note [Interactively-bound Ids in GHCi] in
-- GHC.Entity.Types).
...
interactiveInScope hsc_env
  = tyvars ++ ids
  where
    -- C.f. GHC.IR.Haskell.TypeChecker.Module.setInteractiveContext,
    -- GHC.Compiler.HaskellToCore.deSugarExpr
  ...
  = addLoc (RhsOf binder) $
    ...
    do { ty <- lintRhs binder rhs
       ...
       ; checkL ( isJoinId binder
        ...
        -- computation to perform, see Note [Core syntax top-level string
        -- literals].
      ...
      --                  in GHC.IR.Core.Analyser.Demand.  After eta-expansion
      --                  in CorePrep the rhs is no longer trivial.
...
lintIdUnfolding  _ _ _
  = return ()       -- Do not Lint unstable unfoldings, because that leads
                    -- to exponential behaviour;
                    -- c.f. GHC.IR.Core.Analyser.FreeVars.idUnfoldingVars
...
[Invariants on join points] in GHC.IR.Core.Syntax). However, strictly from a
...
in, say, GHC.IR.Core.Transformer.Substitution.simpleOptPgm, which in some
circumstances can run immediately before Float Out.

All that said, currently GHC.IR.Core.Transformer.Substitution.simpleOptPgm is
the only thing using this loophole, doing so to avoid re-traversing large
functions (beta-reducing a type lambda without introducing a type let requires a
substitution). TODO: Improve simpleOptPgm so that we can forget all this ever
happened.
...
lintTyApp fun_ty arg_ty
  ...
  = do  { lintTyKind tv arg_ty
        ...
        -- See Note [The substitution invariant] in GHC.Entity.TypeAndCoercion
...
checkCaseAlts e ty alts =
  do { checkL (all non_deflt con_alts) (mkNonDefltMsg e)
     ; checkL (increasing_tag con_alts) (mkNonIncreasingAltsMsg e)
          ...
          -- But (see Note [Empty case alternatives] in GHC.IR.Core.Syntax) it's ok to
...
lint_app doc kfn kas
    = do { in_scope <- getInScope
         ...
         -- Note [The substitution invariant] in GHC.Entity.TypeAndCoercion
...
'GHC.IR.Core.Analyser.Arity.etaExpandToJoinPointRule'), and in fact the
simplifier would have to in order to deal with the RHS. So we take a
conservative view and don't allow undersaturated rules for join points. See Note
[Rules and join points] in GHC.IR.Core.Analyse.Occurence for further discussion.
-}
       ...
       ; case (splitForAllTy_maybe s, splitForAllTy_maybe t) of
         ...
         { (Just (tc_s, tys_s), Just (tc_t, tys_t))
             ...
             , isInjectiveTyCon tc_s r
                 -- see Note [NthCo and newtypes] in GHC.Entity.TypeAndCoercion
...
Pointer Table from the top-level bindings. See GHC.IR.Core.Transformer.Pipeline
...
lintAnnots pname pass guts = do
  ...
  when (gopt Opt_DoAnnotationLinting dflags) $ do
    ...
    when (not (null diffs)) $ GHC.IR.Core.Pipeline.putMsg $ vcat
