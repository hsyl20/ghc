...
module GHC.IR.Core.Analyser.Demand ( dmdAnalProgram ) where

#include "HsVersions.h"

import GHC.Config.Flags
import GHC.IR.Core.Transformer.WorkerWrapper.Utils 
                                   ( findTypeShape, deepSplitProductType_maybe )
import GHC.Entity.Demand   -- All of it
...
import GHC.IR.Core.Utils           ( exprIsHNF, exprType, exprIsTrivial )
import GHC.Entity.TypeConstructor
import GHC.Entity.Type
import GHC.Entity.Coercion         ( Coercion, coVarsOfCo )
...
import GHC.Entity.Name             ( getName, stableNameCmp )
...
see Note [Demand on case-alternative binders] in GHC.Entity.Demand
...
foo's arity will be zero (see Note [exprArity invariant] in
GHC.IR.Core.Analyser.Arity), but its demand signature will be that of plusInt.
A small example is the test case of Trac #8963.
...
-- See Note [Trimming a demand to a type] in GHC.Entity.Demand
 ...
 * Just before TidyCore, we add a pass of the demand analyser,
   ...
   right before TidyCore.  See GHC.IR.Core.Transformer.Pipeline.getCoreToDo.
