...
Why is this important?  Because
  - In GHC.IR.Interface.Tidy we use exprArity to fix the *final arity* of
    each top-level Id, and in
  - In CorePrep we use etaExpand on each rhs, so that the visible lambdas
    actually match that arity, which in turn means
    that the StgRhs has the right number of lambdas

An alternative would be to do the eta-expansion in GHC.IR.Interface.Tidy, at least
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    at all.  See Note [Single-method classes] in GHC.IR.Haskell.TypeSystem.Instance. SLPJ May 2013
...
etaInfoApp subst (Lam v1 e) (EtaVar v2 : eis)
  = etaInfoApp (GHC.IR.Core.Substitution.extendSubstWithVar subst v1 v2) e eis

etaInfoApp subst (Cast e co1) eis
  = etaInfoApp subst e (pushCoercion co' eis)
  where
    co' = GHC.IR.Core.Substitution.substCo subst co1
...
etaInfoApp subst (Case e b ty alts) eis
  ...
  where
    ...
    ty'   = etaInfoAppTy (GHC.IR.Core.Substitution.substTy subst ty) eis
...
subst_expr = substExpr (text "GHC.IR.Core.Arity:substExpr")
