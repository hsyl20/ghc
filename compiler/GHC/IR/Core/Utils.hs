...
isExprLevPoly = go
  where
   ...
   -- this case can happen in GHC.IR.Core.Transformer.LevelSetter
   go (Coercion {})                = False
...
-- expression as per the invariants of 'CoreExpr': see
-- "GHC.IR.Core.Syntax#let_app_invariant"
...
exprOkForSideEffects = expr_ok primOpOkForSideEffects
  ...
  -- There is one call at a non-Id binder type, in
  -- GHC.IR.Core.Transformer.LevelSetter
...
See also Note [dataToTag#] in GHC/Builtin/Primitive/primops.txt.pp.
...
-- happen: see "GHC.IR.Core.Syntax#let_app_invariant". This invariant states
-- that arguments of unboxed type must be ok-for-speculation (or trivial).
...
-- See Note [Core syntax top-level string literals]
...
rhsIsStatic :: Platform
            ...
            -> CoreExpr -> Bool
-- It's called
--    (i) in GHC.IR.Interface.Transformer.Tidier.hasCafRefs to decide if the rhs
--        is, or refers to, CAFs;
--    (ii) in CoreToStg to decide whether to put an
--        update flag on it and 
--    (iii) in GHC.Compiler.HaskellToCore.Expression to decide how to expand
--        list literals
--
...
--
--
-- This is a bit like GHC.IR.Core.Utils.exprIsHNF, with the following
-- differences:
