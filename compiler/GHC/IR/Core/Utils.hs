...
isExprLevPoly = go
  where
   ...
   go (Coercion {})                = False  -- this case can happen in GHC.IR.Core.Transform.LevelSetting
...
also GHC.IR.Core.Arity.exprBotStrictness_maybe, but that's a bit more
expensive.
-}
...
~~~~~~~~~~~~~~~~~~   in GHC.IR.Core.Transform.Unfolding.hs
...
exprOkForSideEffects = expr_ok primOpOkForSideEffects
  ...
  -- There is one call at a non-Id binder type, in GHC.IR.Core.Transform.LevelSetting
...
-- update flag on it and (iii) in GHC.Compilers.SyntaxToCore.Expression to decide how to expand
-- list literals
--
