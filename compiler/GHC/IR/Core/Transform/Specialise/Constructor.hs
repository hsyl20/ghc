...
module GHC.IR.Core.Transform.Specialise.Constructor (
        specConstrProgram,
...
import GHC.Data.Id
import GHC.IR.Core.PrettyPrint          ( pprParendExpr )
  ...
  = do  { mb_pats <- mapM (callToPats env bndr_occs) calls
        ...
        ; let have_boring_call = any isNothing mb_pats
              ...
              too_big (vars,_) = not (isWorkerSmallEnough (sc_dflags env) vars)
                  ...
                  -- See Note [Limit w/w arity] in GHC.IR.Core.Transform.WorkerWrapper.Utils
