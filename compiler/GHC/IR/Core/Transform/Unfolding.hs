...
module GHC.IR.Core.Transform.Unfolding (
        ...
        -- Reexport from GHC.IR.Core.Substitution (it only live there so it can be used
...
import GHC.IR.Core.Transform.Simple
...
and similar friends.  See Note [Bottoming floats] in GHC.IR.Core.Transform.LevelSetting.
...
(4) We must be much more cautious about arity-zero things. Consider
    ...
    but it doesn't fire for top-level things; see GHC.IR.Core.Transform.Simplify.Utils
...
litSize :: Literal -> Int
-- Used by GHC.IR.Core.Transform.Unfolding.sizeExpr
