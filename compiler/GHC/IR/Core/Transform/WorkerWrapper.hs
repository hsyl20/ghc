...
{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transform.WorkerWrapper ( wwTopBinds ) where

import GHC.IR.Core.Syntax
import GHC.IR.Core.Transform.Unfolding       ( certainlyWillInline, mkWwInlineRule, mkWorkerUnfolding )
...
in work_fn! See GHC.IR.Core.Transform.Unfolding.mkWorkerUnfolding.
...
inline into the worker's unfolding: see GHC.IR.Core.Transform.Simplify.Utils
 ...
 * We want them to be still there at the end of GHC.IR.Core.Analyse.Demand, so that
...
Note [Final Demand Analyser run] in GHC.IR.Core.Analyse.Demand).
...
in w/w-ing because we'll simply add (\y:Void#), see GHC.IR.Core.Transform.WorkerWrapper.Utils.mkWorerArgs.
