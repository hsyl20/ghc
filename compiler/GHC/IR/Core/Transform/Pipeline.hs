...
\section[GHC.IR.Core.Transform.Pipeline]{Driver for simplifying @Core@ programs}
-}

{-# LANGUAGE CPP #-}

module GHC.IR.Core.Transform.Pipeline ( core2core, simplifyExpr ) where
...
import GHC.Types
import GHC.IR.Core.Transform.CommonSubExpr ( cseProgram )
import GHC.IR.Core.Transform.Rules            ( mkRuleBase, unionRuleBase,
                          extendRuleBaseList, ruleCheckProgram, addRuleInfo, )
import GHC.IR.Core.PrettyPrint          ( pprCoreBindings, pprCoreExpr )
import GHC.IR.Core.Analyse.Occurence        ( occurAnalysePgm, occurAnalyseExpr )
import GHC.Data.Id.Info
import GHC.IR.Core.Analyse.Stats        ( coreBindsSize, coreBindsStats, exprSize )
import GHC.IR.Core.Utils        ( mkTicks, stripTicksTop )
import GHC.IR.Core.Analyse.Lint         ( endPass, lintPassResult, dumpPassResult,
                          lintAnnots )
import GHC.IR.Core.Transform.Simplify         ( simplTopBinds, simplExpr, simplRules )
...
import GHC.IR.Core.Transform.LiberateCase     ( liberateCase )
import GHC.IR.Core.Transform.StaticArgument   ( doStaticArgs )
import GHC.IR.Core.Transform.Specialise       ( specProgram)
import GHC.IR.Core.Transform.Specialise.Constructor       ( specConstrProgram)
import GHC.IR.Core.Analyse.Demand          ( dmdAnalProgram )
import GHC.IR.Core.Analyse.CallArity        ( callArityAnalProgram )
import GHC.IR.Core.Transform.WorkerWrapper   ( wwTopBinds )
import GHC.IR.Core.Transform.Vectorise        ( vectorise )
...
getCoreToDo dflags
  ...
  where
    ...
    -- after this before anything else
        ...
        -- has run at all. See Note [Final Demand Analyser run] in GHC.IR.Core.Analyse.Demand
