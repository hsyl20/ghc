...
module GHC.IR.Core.Transform.Vectorise.Exp
...
import GHC.IR.Core.Transform.Vectorise.Var
import GHC.IR.Core.Transform.Vectorise.Convert
import GHC.IR.Core.Transform.Vectorise.Vect
...
vectExpr aexpr@(_, AnnLam _ _)
  ...
  --   its only purpose is to abort the program, but we need to adjust the type to keep GHC.IR.Core.Analyse.Lint
  --   happy.
