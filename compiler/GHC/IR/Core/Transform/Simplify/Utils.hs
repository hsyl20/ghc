...
\section[GHC.IR.Core.Transform.Simplify.Utils]{The simplifier utilities}
-}

{-# LANGUAGE CPP #-}

module GHC.IR.Core.Transform.Simplify.Utils (
        -- Rebuilding
...
mkRhsStop :: OutType -> SimplCont       -- See Note [RHS of lets] in GHC.IR.Core.Transform.Unfolding
...
contArgs cont
  ...
  | otherwise = go [] cont
  where
    lone (ApplyToTy  {}) = False  -- See Note [Lone variables] in GHC.IR.Core.Transform.Unfolding
      ...
      = case splitStrictSig (idStrictness fun) of
          (demands, result_info)
                ...
                ->      -- Enough args, use the strictness given.
                        ...
                        -- inlining lone variables, so its ok (see GHC.IR.Core.Transform.Simplify.Utils.analyseCont)
...
interestingCallContext cont
  ...
  where
    ...
    interesting (ApplyToVal {}) = ValAppCtxt
        ...
        -- in GHC.IR.Core.Transform.Unfolding
...
(in GHC.IR.Core.Arity.findRhsArity), which we can't do for free-floating lambdas
...
and treat them specially. The real work is done in GHC.IR.Core.Transform.Simplify.Utils.abstractFloats,
...
abstractFloats main_tvs body_env body
  = ASSERT( notNull body_floats )
    do  { (subst, float_binds) <- mapAccumLM abstract empty_subst body_floats
        ; return (float_binds, GHC.IR.Core.Substitution.substExpr (text "abstract_floats1") subst body) }
  where
    main_tv_set = mkVarSet main_tvs
    body_floats = getFloatBinds body_env
    empty_subst = GHC.IR.Core.Substitution.mkEmptySubst (seInScope body_env)

    abstract :: GHC.IR.Core.Substitution.Subst -> OutBind -> SimplM (GHC.IR.Core.Substitution.Subst, OutBind)
    abstract subst (NonRec id rhs)
      = do { (poly_id, poly_app) <- mk_poly tvs_here id
           ; let poly_rhs = mkLams tvs_here rhs'
                 subst'   = GHC.IR.Core.Substitution.extendIdSubst subst id poly_app
           ; return (subst', (NonRec poly_id poly_rhs)) }
      where
        rhs' = GHC.IR.Core.Substitution.substExpr (text "abstract_floats2") subst rhs
    ...
    abstract subst (Rec prs)
       = do { (poly_ids, poly_apps) <- mapAndUnzipM (mk_poly tvs_here) ids
            ; let subst' = GHC.IR.Core.Substitution.extendSubstList subst (ids `zip` poly_apps)
                  poly_rhss = [mkLams tvs_here (GHC.IR.Core.Substitution.substExpr (text "abstract_floats3") subst' rhs)
                              | rhs <- rhss]
