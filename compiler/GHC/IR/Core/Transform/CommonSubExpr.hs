...
module GHC.IR.Core.Transform.CommonSubExpr (cseProgram, cseOneExpr) where
...
the binding instead?  Well, not at top level! See GHC.IR.Core.Transform.Simplify.Utils
...
GHC.IR.Core.Transform.Simplify.Utils Note [Stable unfoldings and postInlineUnconditionally].
...
combineIdenticalAlts in GHC.IR.Core.Transform.Simplify.Utils.prepareAlts.  The basic idea is
to transform
