...
import GHC.IR.Core.Transform.Simple         ( exprIsLambda_maybe )
import GHC.IR.Core.FreeVars          ( exprFreeVars, exprsFreeVars, bindFreeVars
...
import import GHC.IR.Haskell.TypeSystem.Type           ( tcSplitTyConApp_maybe )
...
* At the moment (c) is carried in a reader-monad way by the GHC.IR.Core.Monad.
...
* So in the outer simplifier loop, we combine (b-d) into a single
  RuleBase, reading
     (b) from the ModGuts,
     (c) from the GHC.IR.Core.Monad, and
...
-- It's here rather than in GHC.IR.Core.PrettyPrint because it calls tidyRules
...
Our cunning plan is this:
  ...
  * We use GHC.IR.Core.Substitution.substBind to freshen the binding, using an
