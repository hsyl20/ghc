...
module GHC.IR.Core.Transform.Simplify
   ( simplTopBinds
   , simplExpr
   , simplRules
   )
...
import GHC.IR.Core.PrettyPrint          ( pprCoreExpr )
...
--import GHC.Builtin.Primitive.Operationss           ( tagToEnumKey ) -- temporalily commented out. See #8326
import GHC.IR.Core.Transform.Rules            ( mkRuleInfo, lookupRule, getRules )
--import GHC.Builtin.Primitive.Typess          ( intPrimTy ) -- temporalily commented out. See #8326
...
the simplifier is in GHC.IR.Core.Transform.Pipeline.hs.
...
expression. This is wrapped as a datatype GHC.IR.Core.Transform.Simplify.Utils.FloatsWith.
  ...
  = ASSERT( isId bndr )
    ...
    do  { let   rhs_env     = rhs_se `setInScopeAndZapFloats` env
        ...
        ; (body_env, tvs') <- simplBinders rhs_env tvs
                -- See Note [Floating and type abstraction] in GHC.IR.Core.Transform.Simplify.Utils
...
completeBind env top_lvl is_rec mb_cont old_bndr new_bndr new_rhs
 ...
 = ASSERT( isId new_bndr )
   do { let old_info = idInfo old_bndr
        ...
        -- See Note [Eta-expanding at let bindings] in GHC.IR.Core.Transform.Simplify.Utils
...
--       }
  ...
  getDoneId (DoneEx e _) = getIdFromTrivialExpr e -- Note [substTickish] in GHC.IR.Core.Substitution
...
rebuildCall env (ArgInfo { ai_fun = fun, ai_args = rev_args, ai_strs = [] }) cont
  ...
  -- that the call is definitely bottom; see GHC.IR.Core.Transform.Simplify.Utils.mkArgInfo
...
The code in GHC.IR.Core.Transform.Simplify.Utils.prepareAlts has the effect of generalise this
...
Note that GHC.IR.Core.Transform.Simplify.Utils.mkCase combines identical RHSs.  So
...
NB: take great care when doing strictness analysis;
    see Note [Lambda-bound unfoldings] in GHC.IR.Core.Analyse.Demand.
  ...
  = is_bottoming `seq`  -- See Note [Force bottoming field]
    do { dflags <- getDynFlags
       ; return (mkUnfolding dflags InlineRhs is_top_lvl is_bottoming new_rhs) }
            ...
            --         (b) In GHC.IR.Interface.Tidy we currently assume that, if we want to
...
simplUnfolding env top_lvl mb_cont id unf
  ...
  where
    ...
    rule_env     = updMode (updModeForStableUnfoldings act) env
         -- See Note [Simplifying inside stable unfoldings] in GHC.IR.Core.Transform.Simplify.Utils
