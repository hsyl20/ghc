...
{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transform.Specialise ( specProgram, specUnfolding ) where

#include "HsVersions.h"

import GHC.Data.Id
import GHC.IR.Haskell.TypeSystem.Type hiding( substTy )
...
import GHC.IR.Core.Transform.Simple         ( collectBindersPushingCo )
import GHC.IR.Core.Utils        ( exprIsTrivial, applyTypeToArgs, mkCast )
import GHC.IR.Core.FreeVars          ( exprFreeVars, exprsFreeVars, idFreeVars, exprsFreeIdsList )
import GHC.IR.Core.Arity        ( etaExpandToJoinPointRule )
...
specProgram guts@(ModGuts { mg_module = this_mod
  ...
  where
    ...
    top_env = SE { se_subst = GHC.IR.Core.Substitution.mkEmptySubst $ mkInScopeSet $ mkVarSet $
...
data SpecEnv
  = SE { se_subst :: GHC.IR.Core.Substitution.Subst
...
specVar env v = GHC.IR.Core.Substitution.lookupIdSubst (text "specVar") (se_subst env) v
...
specCase env scrut' case_bndr [(con, args, rhs)]
  ...
  = do { (case_bndr_flt : sc_args_flt) <- mapM clone_me (case_bndr' : sc_args')
       ...
       ; let sc_rhss = [ Case (Var case_bndr_flt) case_bndr' (idType sc_arg')
             ...
             env_rhs' = env_rhs { se_subst = GHC.IR.Core.Substitution.extendIdSubstList (se_subst env_rhs) subst_prs
  ...
  where
    ...
    already_covered dflags args      -- Note [Specialisations already covered]
       = isJust (lookupRule dflags
                            (GHC.IR.Core.Substitution.substInScope (se_subst env), realIdUnfolding)
                            (const True)
           ...
           {    -- Figure out the type of the specialised function
             let body_ty = applyTypeToArgs rhs fn_type rule_args
                 (lam_args, app_args)           -- Add a dummy argument if body_ty is unlifted
                   | isUnliftedType body_ty     -- C.f. GHC.IR.Core.Transform.WorkerWrapper.Utils.mkWorkerArgs
                   , not (isJoinId fn)
...
Thing, but it's what happens now; see GHC.IR.Core.Transform.Simplify.Utils Note [Casts and
...
bindAuxiliaryDicts env@(SE { se_subst = subst, se_interesting = interesting })
  ...
  where
    ...
    env' = env { se_subst = subst `GHC.IR.Core.Substitution.extendSubstList`
                                     (orig_dict_ids `zip` spec_dict_args)
                                  `GHC.IR.Core.Substitution.extendInScopeList` dx_ids
...
extendTvSubstList env tv_binds
  = env { se_subst = GHC.IR.Core.Substitution.extendTvSubstList (se_subst env) tv_binds }

substTy :: SpecEnv -> Type -> Type
substTy env ty = GHC.IR.Core.Substitution.substTy (se_subst env) ty

substCo :: SpecEnv -> Coercion -> Coercion
substCo env co = GHC.IR.Core.Substitution.substCo (se_subst env) co

substBndr :: SpecEnv -> CoreBndr -> (SpecEnv, CoreBndr)
substBndr env bs = case GHC.IR.Core.Substitution.substBndr (se_subst env) bs of
                      (subst', bs') -> (env { se_subst = subst' }, bs')

substBndrs :: SpecEnv -> [CoreBndr] -> (SpecEnv, [CoreBndr])
substBndrs env bs = case GHC.IR.Core.Substitution.substBndrs (se_subst env) bs of
                      (subst', bs') -> (env { se_subst = subst' }, bs')
...
cloneBindSM env@(SE { se_subst = subst, se_interesting = interesting }) (NonRec bndr rhs)
  = do { us <- getUniqueSupplyM
       ; let (subst', bndr') = GHC.IR.Core.Substitution.cloneIdBndr subst us bndr
...
cloneBindSM env@(SE { se_subst = subst, se_interesting = interesting }) (Rec pairs)
  = do { us <- getUniqueSupplyM
       ; let (subst', bndrs') = GHC.IR.Core.Substitution.cloneRecIdBndrs subst us (map fst pairs)
