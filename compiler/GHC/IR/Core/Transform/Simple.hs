...
{-# LANGUAGE CPP #-}
module GHC.IR.Core.Transform.Simple (
...
import GHC.IR.Core.Arity( joinRhsArity, etaExpandToJoinPoint )

import GHC.IR.Core.Syntax
import GHC.IR.Core.Substitution
import GHC.IR.Core.Utils
import GHC.IR.Core.FreeVars
import GHC.IR.Core.PrettyPrint  ( pprCoreBindings, pprRules )
import GHC.IR.Core.Analyse.Occurence( occurAnalyseExpr, occurAnalysePgm )
...
simple_bind_pair env@(SOE { soe_inl = inl_env, soe_subst = subst })
  ...
  where
    ...
    pre_inline_unconditionally
       ...
       | isExportedId in_bndr     = False    --     in GHC.IR.Core.Transform.Simplify.Utils
       | stable_unf               = False
...
In GHC.IR.Core.Transform.Simplify.Utils, Note [Top level and postInlineUnconditionally], we
...
--
-- It's important to zap fragile OccInfo (which GHC.IR.Core.Substitution.substIdBndr
...
GHC.IR.Haskell.TypeSystem.Instance) we mark recursive things as INLINE but the recursion
...
exprIsConApp_maybe (in_scope, id_unf) expr
  ...
  where
    ...
    ----------------------------
    -- Operations on the (Either InScopeSet GHC.IR.Core.Substitution)
    -- The Left case is wildly dominant
    subst_co (Left {}) co = co
    subst_co (Right s) co = GHC.IR.Core.Substitution.substCo s co
...
pushCoTyArg co ty
  ...
  where
    ...
    co1 = mkNthCo 0 co
       ...
       -- See the NthCo case in GHC.IR.Core.Analyse.Lint.
