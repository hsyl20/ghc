...
import GHC.IR.Core.Analyser.Stats (exprStats)
import GHC.Entity.Literal( pprLiteral )
import GHC.Entity.Name( pprInfixName, pprPrefixName )
...
import GHC.Entity.SrcLoc      ( pprUserRealSpan )
...
ppIdInfo id info
  = sdocWithDynFlags $ \dflags ->
    ...
    , (not (null rules), text "RULES:" <+> vcat (map pprRule rules))
        ...
        -- see GHC.IR.Core.Printer.pprIdBndr
  where
