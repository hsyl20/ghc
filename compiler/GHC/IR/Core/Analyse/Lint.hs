...
module GHC.IR.Core.Analyse.Lint (
    ...
    endPass, endPassIO,
    dumpPassResult,
    GHC.IR.Core.Analyse.Lint.dumpIfSet,
 ) where
...
import GHC.IR.Core.Analyse.Stats   ( coreBindsStats )
...
import GHC.Utils
import GHC.Data.ClassInstance     ( instanceDFunId )
import GHC.Data.Coercion.Optimise ( checkAxInstCo )
import GHC.Data.Unique.Supply
import GHC.IR.Core.Arity ( typeArity )
...
lintSingleBinding top_lvl_flag rec_flag (binder,rhs)
  = addLoc (RhsOf binder) $
    ...
    do { ty <- lintRhs binder rhs
      ...
      --                  in GHC.IR.Core.Analyse.Demand.  After eta-expansion in CorePrep the rhs is no longer trivial.
...
lintIdUnfolding  _ _ _
  = return ()       -- Do not Lint unstable unfoldings, because that leads
                    -- to exponential behaviour; c.f. GHC.IR.Core.FreeVars.idUnfoldingVars
...
lintCoreExpr e@(Case scrut var alt_ty alts) =
  ...
  do { let scrut_diverges = exprIsBottom scrut
     ...
     -- See Note [Rules for floating-point comparisons] in GHC.IR.Core.Transform.ConstantFolding
...
in, say, GHC.IR.Core.Substitution.simpleOptPgm, which in some circumstances can run immediately
before Float Out.

All that said, currently GHC.IR.Core.Substitution.simpleOptPgm is the only thing using this
...
"undersaturated" rule (see 'GHC.IR.Core.Arity.etaExpandToJoinPointRule'), and in fact
...
Pointer Table from the top-level bindings. See GHC.IR.Core.Transform.Pipeline Note [Grand
