...
module GHC.IR.Core.Analyse.Demand ( dmdAnalProgram ) where

#include "HsVersions.h"

import GHC.Config.Flags
import GHC.IR.Core.Transform.WorkerWrapper.Utils            ( findTypeShape, deepSplitProductType_maybe )
...
foo's arity will be zero (see Note [exprArity invariant] in GHC.IR.Core.Arity),
 ...
 * Just before TidyCore, we add a pass of the demand analyser,
   ...
   right before TidyCore.  See GHC.IR.Core.Transform.Pipeline.getCoreToDo.
