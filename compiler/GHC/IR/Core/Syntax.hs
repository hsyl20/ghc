...
-- | GHC.IR.Core.Syntax holds all the main data types for use by for the Glasgow Haskell Compiler midsection
...
--    See Note [GHC.IR.Core.Syntax let/app invariant]
...
--    string literals, see Note [GHC.IR.Core.Syntax top-level string literals].
--
--    See Note [GHC.IR.Core.Syntax let/app invariant]
--    See Note [Levity polymorphism invariants]
--
...
string bindings; anything else and things break. GHC.IR.Core.Analyser.Lint checks this invariant.
...
* In Core, we have a special rule that permits top-level Addr# bindings. See
  Note [GHC.IR.Core.Syntax top-level string literals]. Core-to-core passes may introduce
...
'GHC.IR.Core.Utils.needsCaseBinding' can help you determine which to generate, or
alternatively use 'GHC.IR.Core.Syntax.Make.mkCoreLet' rather than this constructor directly,
...
that 'GHC.IR.Core.Utils.exprType' can still return a type for any expression, including
a jump.
...
data Tickish id =
  ...
  | Breakpoint
    ...
    , breakpointFVs    :: [id]  -- ^ the order of this list is important:
                                ...
                                -- Note [substTickish] in GHC.IR.Core.Transformer.Substitution.
    }
...
but GHC.IR.Core.Analyser.FreeVars, Subst, GHC.IR.Core.Printer, CoreTidy also inspect the representation.
-}
...
data CoreRule
  = Rule {
        ...
        ru_auto :: Bool,   -- ^ @True@  <=> this rule is auto-generated
                           ...
                           -- See Note [Trimming auto-rules] in GHC.IR.Interface.Transformer.Tidier
...
-- This type should be treated as abstract everywhere except in "GHC.IR.Core.Transformer.Inliner"
data Unfolding
...
data UnfoldingGuidance
  = UnfWhen {   -- Inline without thinking about the *size* of the uf_tmpl
                ...
                -- See Note [INLINE for small functions] in GHC.IR.Core.Transformer.Inliner
...
-- If you want an expression of type @Int@ use 'GHC.IR.Core.Syntax.Make.mkIntExpr'
...
-- If you want an expression of type @Word@ use 'GHC.IR.Core.Syntax.Make.mkWordExpr'
...
-- If you want an expression of type @Char@ use 'GHC.IR.Core.Syntax.Make.mkCharExpr'
mkCharLit :: Char -> Expr b
-- | Create a machine string literal expression of type @Addr#@.
-- If you want an expression of type @String@ use 'GHC.IR.Core.Syntax.Make.mkStringExpr'
...
-- If you want an expression of type @Float@ use 'GHC.IR.Core.Syntax.Make.mkFloatExpr'
...
-- If you want an expression of type @Double@ use 'GHC.IR.Core.Syntax.Make.mkDoubleExpr'
...
-- that the rhs satisfies the let/app invariant.  Prefer to use 'GHC.IR.Core.Syntax.Make.mkCoreLets' if
-- possible, which does guarantee the invariant
mkLets        :: [Bind b] -> Expr b -> Expr b
-- | Bind all supplied binders over an expression in a nested lambda expression. Prefer to
-- use 'GHC.IR.Core.Syntax.Make.mkCoreLams' if possible
...
-- | Create a binding group where a type variable is bound to a type. Per "GHC.IR.Core.Syntax#type_let",
...
These are defined here to avoid a module loop between GHC.IR.Core.Utils and GHC.IR.Core.Analyser.FreeVars
