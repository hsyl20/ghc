...
-- | GHC.IR.Core.Syntax holds all the main data types for use by for the Glasgow
...
import GHC.Entity.CostCentre
import GHC.Entity.Var.Environment   ( InScopeSet )
import GHC.Entity.Var
import GHC.Entity.Type
import GHC.Entity.Coercion
import GHC.Entity.Name
import GHC.Entity.Name.Set
import GHC.Entity.Name.Environment  ( NameEnv, emptyNameEnv )
...
import GHC.Entity.SrcLoc            ( RealSrcSpan, containsSpan )
...
--    See Note [Core syntax let/app invariant]
...
--    string literals, see Note [Core syntax top-level string literals].
--
--    See Note [Core syntax let/app invariant]
--    See Note [Levity polymorphism invariants]
--
...
--       in GHC.IR.Core.Transformer.ConstantFolder for rationale.
--
...
-- See Note [GHC Formalism] in GHC.IR.Core.Analyser.Lint
data Expr b
  = Var   Id
  | Lit   Literal
  | App   (Expr b) (Arg b)
...
GHC.IR.Core.Transformer.ConstantFolder for the rationale for this restriction.

-------------------------- GHC.IR.Core.Syntax INVARIANTS ---------------------------

Note [Core syntax top-level invariant]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
See #toplevel_invariant#

Note [Core syntax letrec invariant]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
See #letrec_invariant#

Note [Core syntax top-level string literals]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
string bindings; anything else and things break. GHC.IR.Core.Analyser.Lint
...
* In Core, we have a special rule that permits top-level Addr# bindings. See
  Note [Core syntax top-level string literals]. Core-to-core passes may
  introduce new top-level string literals.
...
Note [Core syntax let/app invariant]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
'GHC.IR.Core.Utils.needsCaseBinding' can help you determine which to generate,
or alternatively use 'GHC.IR.Core.Syntax.Make.mkCoreLet' rather than this
...
See Note [Levity polymorphism checking] in GHC.Compiler.HaskellToCore.Monad to
see where these invariants are established for user-written code.

Note [Core syntax let goal]
~~~~~~~~~~~~~~~~~~~~~~~
...
* A case expression can have empty alternatives if (and only if) the
  ...
  this?  See Note [Bottoming expressions] in GHC.IR.Core.Utils.
...
* We treat a case expression with empty alternatives as trivial iff
  its scrutinee is (see GHC.IR.Core.Utils.exprIsTrivial).  This is actually
  important; see Note [Empty case is trivial] in GHC.IR.Core.Utils
...
Join points must follow these invariants:
  ...
  2. For join arity n, the right-hand side must begin with at least n lambdas.
     No ticks, no casts, just lambdas!  C.f. GHC.IR.Core.Utils.joinRhsArity.
...
that 'GHC.IR.Core.Utils.exprType' can still return a type for any expression,
including a jump.
...
data Tickish id =
  ...
  | Breakpoint
    ...
    , breakpointFVs    :: [id]  -- ^ the order of this list is important:
                                ...
                                -- appropriate entry in GHC.Entity.Types.ModBreaks.
                                --
                                -- Careful about substitution!  See
                                -- Note [substTickish] in
...
GHC.IR.Core.Transformer.Rules, but GHC.IR.Core.Analyser.FreeVars,
GHC.IR.Core.Transformer.Substitution, GHC.IR.Core.Printer,
GHC.IR.Core.Transformer.Tidier also inspect the representation.
-}
...
data CoreRule
  = Rule {
        ...
        ru_auto :: Bool,   -- ^ @True@  <=> this rule is auto-generated
                           ...
                           -- See Note [Trimming auto-rules] in
                           -- GHC.IR.Interface.Transformer.Tidier for the sole
  ...
  | BuiltinRule {
        ...
        ru_try   :: RuleFun
                ...
                -- See Note [Extra args in rule matching] in
...
-- This type should be treated as abstract everywhere except in
...
data Unfolding
  ...
  | BootUnfolding      -- ^ We have no information about the unfolding, because
                       ...
                       -- See Note [Inlining and hs-boot files] in
                       -- GHC.Compiler.CoreToInterface for what this is used
                       -- for.
...
data UnfoldingSource
  ...
  | InlineCompulsory   -- Something that *has* no binding, so you *must* inline it
                       ...
                       -- (see GHC.Entity.Id.Make, calls to mkCompulsoryUnfolding).
...
data UnfoldingGuidance
  = UnfWhen {   -- Inline without thinking about the *size* of the uf_tmpl
                ...
                -- See Note [INLINE for small functions] in
                -- GHC.IR.Core.Transformer.Inliner
...
--   See Note [Expanding variables] in GHC.IR.Core.Transformer.Rules
...
See also Note [Inlining an InlineRule] in GHC.IR.Core.Transformer.Inliner.
...
-- use 'GHC.IR.Core.Syntax.Make.mkCoreApps' if possible
...
-- use 'GHC.IR.Core.Syntax.Make.mkCoreConApps' if possible
...
-- @Integer@.  If you want an expression of type @Int@ use
-- 'GHC.IR.Core.Syntax.Make.mkIntExpr'
mkIntLit      :: DynFlags -> Integer -> Expr b
-- | Create a machine integer literal expression of type @Int#@ from an @Int@.
-- If you want an expression of type @Int@ use
-- 'GHC.IR.Core.Syntax.Make.mkIntExpr'
...
-- If you want an expression of type @Word@ use
-- 'GHC.IR.Core.Syntax.Make.mkWordExpr'
...
-- If you want an expression of type @Char@ use
-- 'GHC.IR.Core.Syntax.Make.mkCharExpr'
mkCharLit :: Char -> Expr b
-- | Create a machine string literal expression of type @Addr#@.
-- If you want an expression of type @String@ use
-- 'GHC.IR.Core.Syntax.Make.mkStringExpr'
...
-- If you want an expression of type @Float@ use
-- 'GHC.IR.Core.Syntax.Make.mkFloatExpr'
...
-- If you want an expression of type @Double@ use
-- 'GHC.IR.Core.Syntax.Make.mkDoubleExpr'
...
-- expression. Assumes that the rhs satisfies the let/app invariant.  Prefer to
-- use 'GHC.IR.Core.Syntax.Make.mkCoreLets' if possible, which does guarantee
-- the invariant
mkLets        :: [Bind b] -> Expr b -> Expr b
-- | Bind all supplied binders over an expression in a nested lambda expression.
-- Prefer to use 'GHC.IR.Core.Syntax.Make.mkCoreLams' if possible
...
-- "GHC.IR.Core.Syntax#type_let", this can only be used to bind something in a
...
These are defined here to avoid a module loop between GHC.IR.Core.Utils and
