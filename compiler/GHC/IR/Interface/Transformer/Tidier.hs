...
module GHC.IR.Interface.Transformer.Tidier (
...
import GHC.IR.Core.Transformer.Tidier
import GHC.IR.Core.Pipeline
import GHC.Compiler.CoreToStg.Prepare
import GHC.IR.Core.Utils                 (rhsIsStatic)
import GHC.IR.Core.Analyser.Stats        (coreBindsStats, CoreStats(..))
import GHC.IR.Core.Syntax.Force          (seqBinds)
import GHC.IR.Core.Analyser.Lint
...
import GHC.IR.Core.Analyser.Arity        ( exprArity, exprBotStrictness_maybe )
...
import GHC.Entity.Id.Make                ( mkDictSelRhs )
import GHC.Entity.Id.Info
import GHC.Entity.ClassInstance
import GHC.Entity.FamilyInstance
import GHC.Entity.Type                   ( tidyTopType )
import GHC.Entity.Demand                 (appIsBottom, isTopSig, isBottomingSig)
import GHC.Entity.BasicTypes
import GHC.Entity.Name hiding (varName)
import GHC.Entity.Name.Set
import GHC.Entity.Name.Environment
import GHC.Entity.Name.Cache
...
import GHC.Entity.Unique.DeterFiniteMap
...
************************************************************************
*                                                                      *
                Plan A: simpleGHC.IR.Interface.Transformer.Tidier
*                                                                      *
************************************************************************
...
This generates bad code unless it's first simplified a bit.  That is why
GHC.IR.Core.Transformer.Inliner.mkImplicitUnfolding uses simpleOptExpr to do a
bit of optimisation first.  (Only matters when the selector is used curried; eg
map x ys.)  See Trac #2070.
...
At one time I tried injecting the implicit bindings *early*, at the beginning of
GHC.IR.Core.Transformer.Pipeline.  But that gave rise to real difficulty,
because GlobalIds are supposed to have *fixed* IdInfo, but the simplifier and
other core-to-core passes mess with IdInfo all the time.  The straw that broke
the camels back was when a class selector got the wrong arity -- ie the
simplifier gave it arity 2, whereas importing modules were expecting it to have
arity 1 (Trac #2844).  It's much safer just to inject them right at the end,
after tidying.

Oh: two other reasons for injecting them late:

  - If implicit Ids are already in the bindings when we start
    GHC.IR.Interface.Transformer.Tidier, we'd have to be careful not to treat
    them as external Ids (in the sense of chooseExternalIds); else the Ids
    mentioned in *their* RHSs will be treated as external and you get an
    interface file saying      a18 = <blah>
...
tidyTopIdInfo dflags rhs_tidy_env name orig_rhs tidy_rhs idinfo show_unfold caf_info
  ...
  where
    ...
    -- GHC.IR.Core.Transformer.WorkerWrapper
...
GHC.IR.Interface.Transformer.Tidier.  But CorePrep does some transformations
that affect CAF-hood.  So we have to *predict* the result here, which is
revolting.
