...
mkNewTyConRhs tycon_name tycon con
  ...
  where
    ...
    etad_rhs   :: Type     -- See Note [Tricky iface loop] in GHC.IR.Interface.Loader
...
in GHC.Entity.TypeAndCoercion), so we change it to Specified when making MkT's TyBinders
...
-- For iface ones, the GHC.IR.Interface.Loader has already allocated a suitable name in the cache
