...
import GHC.Entity.FamilyInstance   ( FamInstEnvs, mkNewTypeCoAxiom )
...
import GHC.Entity.SrcLoc( SrcSpan, noSrcSpan )
...
mkNewTyConRhs tycon_name tycon con
  ...
  where
    ...
    etad_rhs   :: Type     -- See Note [Tricky iface loop]
...
in GHC.Entity.TypeAndCoercion), so we change it to Specified when making MkT's
TyBinders
...
-- Called in GHC.IR.Interface.BuildTypeAndClass to allocate the implicit binders
-- of type/class decls
-- For source type/class decls, this is the first occurrence
-- For iface ones, the GHC.IR.Interface.Loader has already allocated a suitable
-- name in the cache
