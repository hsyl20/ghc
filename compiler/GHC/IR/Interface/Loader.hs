...
* BUT, if the TyCon is a wired-in TyCon, we don't really need its interface;
  ...
  rules.  That is what GHC.IR.Interface.Loader.loadWiredInHomeIface does.  It's called
  from GHC.IR.Interface.TypeChecker.{tcImportDecl, checkWiredInTyCon, ifCheckWiredInThing}
...
-- It might not be a wired-in tycon (see the calls in GHC.IR.Haskell.TypeChecker.Unify),
