...
module GHC.IR.Interface.Loader (
        -- Importing one thing
        ...
        loadDecls, -- Should move to GHC.IR.Interface.TypeChecker and be renamed
...
import {-# SOURCE #-}   GHC.IR.Interface.TypeChecker( tcIfaceDecl, tcIfaceRules, tcIfaceInst,
...
* BUT, if the TyCon is a wired-in TyCon, we don't really need its interface; but
  we must make sure we read its interface in case it has instances or rules.
  That is what GHC.IR.Interface.Loader.loadWiredInHomeIface does. It's called
  from GHC.IR.Interface.TypeChecker.{tcImportDecl, checkWiredInTyCon,
  ifCheckWiredInThing}
...
-- GHC.IR.Haskell.TypeChecker.Unify), in which case this is a no-op.
...
in one-shot mode; see notes with hsc_HPT decl in GHC.Entity.Types).
