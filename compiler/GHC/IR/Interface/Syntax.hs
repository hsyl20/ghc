...
import GHC.Entity.TypeConstructor ( Role (..), Injectivity(..) )
import GHC.Utils                  ( filterOut, filterByList )
import GHC.Entity.DataConstructor (SrcStrictness(..), SrcUnpackedness(..))
...
type IfaceTopBndr = Name
  -- It's convenient to have an Name in the GHC.IR.Interface.Syntax, although in
  -- each case the namespace is implied by the context. However, having an Name
  -- makes things like ifaceDeclImplicitBndrs and ifaceDeclFingerprints very
  -- convenient. Moreover, having the key of the binder means that we can encode
  -- known-key things cleverly in the symbol table.
...
-- See Note [Implicit TyThings] in GHC.Entity.Types

-- N.B. the set of names returned here *must* match the set of
-- TyThings returned by GHC.Entity.Types.implicitTyThings, in the sense that
-- TyThing.getOccName should define a bijection between the two lists.
-- This invariant is used in GHC.IR.Interface.Loader.loadDecl (see note [Tricky
-- iface loop])
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
In GHC.IR.Interface.Syntax an IfaceCase does not record the types of the
alternatives, unlike Core syntax Case.  But we need this type if the
alternatives are empty.  Hence IfaceECase.  See Note [Empty case alternatives]
in GHC.IR.Core.Syntax.
...
(see GHC.Utils.PrettyPrint.TyThing), the TyThing may come from some other module
so we really need the module qualifier.  We solve this by passing in a
pretty-printer for the binders.
...
When we read an interface file, we extend the PTE, a mapping of Names to
TyThings, with the declarations we have read. The extension of the PTE is strict
in the Names, but not in the TyThings themselves.
GHC.IR.Interface.Loader.loadDecl calculates the list of (Name, TyThing) bindings
to add to the PTE. For an IfaceId, there's just one binding to add; and the ty,
details, and idinfo fields of an IfaceId are used only in the TyThing. So by
reading those fields lazily we may be able to save the work of ever having to
deserialize them (into IfaceType, etc.).
