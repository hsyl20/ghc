...
module GHC.IR.Interface.TypeChecker (
...
import GHC.Entity.TypeAndCoercion    -- needs to build types & coercions in a knot
...
'TyCon', in this case?  There are three cases:
    ...
    1) tcHiBootIface in GHC.IR.Interface.TypeChecker: We're typechecking an
    hi-boot file in preparation of checking if the hs file we're building is
    compatible.  In this case, we want all of the internal TyCons to MATCH the
    ones that we just constructed during typechecking: the knot is thus tied
    through if_rec_types.

    2) retypecheckLoop in GHC.Program.Make: We are retypechecking a
    ...
    3) genModDetails in GHC.Program.Main: We are typechecking an
...
typecheckIfacesForMerging mod ifaces tc_env_var =
  ...
  initIfaceLcl mod (text "typecheckIfacesForMerging") False $ do
    ...
    -- serialize them out.
    -- See Note [rnIfaceNeverExported] in GHC.IR.Interface.Renamer
...
tcIfaceVectInfo mod typeEnv (IfaceVectInfo
  ...
  where
    vectVarMapping name
      ...
      --          }
      --
      --   notAnIdErr = pprPanic "tcIfaceVectInfo: not an id" (ppr name)
    ...
    vectTyConMapping vars name vName
      ...
      where
        ...
        notATyConErr = pprPanic "tcIfaceVectInfo: not a tycon" (ppr name)
