...
'TyCon', in this case?  There are three cases:
    ...
    1) tcHiBootIface in GHC.IR.Interface.TypeChecker: We're typechecking an hi-boot file in
...
typecheckIfacesForMerging mod ifaces tc_env_var =
  ...
  initIfaceLcl mod (text "typecheckIfacesForMerging") False $ do
    ...
    -- serialize them out.  See Note [rnIfaceNeverExported] in GHC.IR.Interface.Renamer
    ...
    details <- forM ifaces $ \iface -> do
        -- See Note [Resolving never-exported Names in GHC.IR.Interface.TypeChecker]
...
-- Note [Resolving never-exported Names in GHC.IR.Interface.TypeChecker]
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
tcIfaceVectInfo mod typeEnv (IfaceVectInfo
  ...
  where
    vectVarMapping name
      ...
      --          }
      --
      --   notAnIdErr = pprPanic "GHC.IR.Interface.TypeChecker.tcIfaceVectInfo: not an id" (ppr name)
    ...
    vectTyConMapping vars name vName
      ...
      where
        ...
        notATyConErr = pprPanic "GHC.IR.Interface.TypeChecker.tcIfaceVectInfo: not a tycon" (ppr name)
