...
import GHC.Data.UniqueFM      ( disjointUFM, pluralUFM, pprUFM )
import GHC.Data.UniqueDFM ( disjointUDFM, udfmToUfm
                                        , anyUDFM, allUDFM )
...
-- See Note [Deterministic UniqFM] in GHC.Data.UniqueDFM for
-- explanation why it's not deterministic and why it matters. Use DVarSet if the
-- set eventually gets converted into a list or folded over in a way where the
-- order changes the generated code, for example when abstracting variables.
...
-- See Note [Deterministic UniqFM] in GHC.Data.UniqueDFM for
-- explanation why we need DVarSet.
