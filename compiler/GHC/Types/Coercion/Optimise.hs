...
module GHC.Types.Coercion.Optimise ( optCoercion, checkAxInstCo ) where
...
import GHC.Types.Type hiding( substTyVarBndr, substTy )
import qualified GHC.Types.Type as Type
import GHC.Haskell.TypeCheck.Type ( exactTyCoVarsOfType )
...
checkAxInstCo (AxiomInstCo ax ind cos)
  ...
  where
    ...
    check_no_conflict flat (b@CoAxBranch { cab_lhs = lhs_incomp } : rest)
         -- See Note [Apartness] in GHC.Types.FamilyInstance
...
etaTyConAppCo_maybe tc co
  ...
  , let n = length tys1    -- in GHC.Types.TypeAndCoercion
  = ASSERT( tc == tc1 )
