...
import GHC.Types.BasicTypes        ( Boxity(..), neverInlinePragma )
import GHC.Haskell.TypeCheck.Binding( addTypecheckedBinds )
import GHC.Interface.Environment ( newGlobalBinder )
import GHC.Types.TypeAndCoercion   ( Type(..), TyLit(..) )
import GHC.Haskell.TypeCheck.Environment
import GHC.Haskell.TypeCheck.Evidence ( mkWpTyApps )
import GHC.Haskell.TypeCheck
import GHC.Types.Base ( lookupId )
...
import GHC.Types.Kind   ( isTYPEApp )
...
4. Solve Typeable constraints.  This is done by a custom Typeable solver,
   currently in GHC.Haskell.TypeCheck.Constraint.Interact, that use M.$tcT
   so solve (Typeable T).
