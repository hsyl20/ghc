...
import GHC.CoreTypes.Kind   ( isTYPEApp )
...
import GHC.Util.Fingerprint  (Fingerprint(..), fingerprintString
...
1. Generate a binding for each module p:M
   (done in GHC.Haskell.Derive.Typeable by mkModIdBindings)
       M.$trModule :: GHC.CoreTypes.Module
...
2. Generate a binding for every data type declaration T in module M,
   ...
   its kind. kind_rep is a value of type GHC.CoreTypes.KindRep, which gives a
   ...
   they get slightly special treatment in GHC.Interface.Rename.rnIfaceDecl.
...
* GHC.Prim doesn't have any associated object code, so we need to put the
  ...
  GHC.Haskell.Derive.Typeable.mkPrimTypeableBinds is responsible for
 ...
 * In $tcProxy, the GHC.CoreTypes.TyCon structure for Proxy, we store a
   GHC.CoreTypes.KindRep, which represents the polymorphic kind of Proxy
       Proxy :: forall k. k->Type
