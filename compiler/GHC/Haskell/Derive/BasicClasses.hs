...
module GHC.Haskell.Derive.BasicClasses (
...
import GHC.CoreTypes.Id.Make                  ( coerceId )
...
so we have to switch that flag on locally in GHC.Haskell.Derive.genInst.
...
mkAuxBinderName dflags parent occ_fun
  ...
  where
    ...
    parent_stable_hash =
      ...
      -- See Note [Base 62 encoding 128-bit integers] in GHC.Data.Char.Encoding
