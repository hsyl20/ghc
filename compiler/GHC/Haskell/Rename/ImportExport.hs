...
module GHC.Haskell.Rename.ImportExport (
...
import GHC.Types.BasicTypes          ( TopLevelFlag(..), StringLiteral(..) )
import GHC.Utils
...
GHC.Driver.Main we must also check that these rules hold transitively for all
...
rnImportDecl this_mod
  ...
  = setSrcSpan loc $ do
    ...
    -- at least not until GHC.Interface.TypeCheck.tcHiBootIface, which is
    -- too late to avoid typechecker crashes.  (Indirect self imports are not
    -- caught until GHC.Interface.TypeCheck, see #10337 tracking how to
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
See also: Note [Interactively-bound Ids in GHCi] in GHC.Types.Base
          Note [Looking up Exact RdrNames] in GHC.Haskell.Rename.Environment
...
1.  We give them an Internal Name, not (as usual) an External one.
    This is done by GHC.Haskell.Rename.Environment.newTopSrcBinder.
