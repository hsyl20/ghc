...
module GHC.Haskell.Rename.Environment (
        newTopSrcBinder,
...
import GHC.Types.BasicTypes   ( pprWarningTxtForMsg, TopLevelFlag(..))
...
newTopSrcBinder (L loc rdr_name)
  ...
  = do  { this_mod <- getModule
        ...
        --      data T = (,,,)
        -- the constructor is parsed as a type, and then GHC.Haskell.Parser.Syntax.tyConToDataCon
...
Looking up a name in the GHC.Haskell.Rename.Environment.
...
lookupFamInstName (Just cls) tc_rdr
  -- Associated type; c.f GHC.Haskell.Rename.Binding.rnMethodBind
...
positions for constructors, TyCons etc.  For example
    [d| data T = MkT Int |]
when we splice in and Convert to GHC.Haskell RdrName, we'll get
 ...
 * In GHC.Haskell.Rename.Environment.newTopSrcBinder we spot Exact RdrNames
   that wrap a non-External Name, and make an External name for it. This is the
   name that goes in the GlobalRdrEnv

 * When looking up an occurrence of an Exact name, done in
   GHC.Haskell.Rename.Environment.lookupExactOcc, we find the Name with the
 ...
 * Exact names are also use for purely local binders generated
   ...
   in the LocalRdrEnv by GHC.Haskell.Rename.Environment.lookupOccRn, and
