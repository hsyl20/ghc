...
\section[GHC.Haskell.Rename.Binding]{Renaming and dependency analysis of bindings}
...
module GHC.Haskell.Rename.Binding (
...
import GHC.Types.RdrName         ( RdrName, rdrNameOcc )
import GHC.Types.SrcLoc
import GHC.Data.List.SetOps       ( findDupsEq )
import GHC.Types.BasicTypes      ( RecFlag(..), LexicalFixity(..) )
...
walking polymorphic bindings (GHC.Haskell.Syntax.Binding) and qualifier lists
...
\subsubsection[dep-GHC.Haskell.Syntax.Binding]{Polymorphic bindings}
...
depAnalBinds binds_w_dus
  ...
  where
    sccs = depAnal (\(_, defs, _) -> defs)
                   ...
                   -- Note [depAnal determinism] in GHC.Types.Name.Environment.
...
2. Typecheck the builder definition, which needs the typechecked
   ...
   in GHC.Haskell.TypeCheck.Binding.tcValBinds.
