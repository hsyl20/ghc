...
\section[GHC.Haskell.Rename.Pattern]{Renaming of patterns}
...
module GHC.Haskell.Rename.Pattern (-- main entry points
...
import GHC.CoreTypes.Literal            ( inCharRange )
...
newPatName (LetMk is_top fix_env) rdr_name
  = CpsRn (\ thing_inside ->
    ...
    --       before going on to the RHSes (see GHC.Haskell.Rename).
...
rnPatAndThen mk (SplicePat splice)
  = do { eith <- liftCpsFV $ rnSplicePat splice
       ; case eith of   -- See Note [rnSplicePat] in GHC.Haskell.Rename.Splice
