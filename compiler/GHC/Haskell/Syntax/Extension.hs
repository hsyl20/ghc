...
module GHC.Haskell.Syntax.Extension where
...
The Haskell AST is reused across multiple compiler passes. We also have the
...
-- | Used as a data type index for the Haskell AST
data GhcPass (c :: Pass)
...
type family PostTc x ty -- Note [Pass sensitive types] in
...
type family PostRn x ty  -- Note [Pass sensitive types] in
                         -- GHC.Haskell.Syntax.PlaceHolder
