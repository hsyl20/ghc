...
module GHC.Haskell.Syntax.Expression where
...
import GHC.Haskell.Syntax.PlaceHolder ( NameOrRdrName )
import GHC.Haskell.Syntax.Extension
import GHC.Haskell.Syntax.Type
import GHC.Haskell.Syntax.Binding
...
import GHC.Types.RdrName  ( GlobalRdrEnv )
...
is Less Cool because
  * The renamer has to predict which methods are needed.
    See the tedious GHC.Haskell.Rename.Expression.methodNamesCmd.
...
When it calls GHC.Haskell.Rename.Environment.unknownNameSuggestions to
identify these alternatives, the typechecker must provide a GlobalRdrEnv.  If it
provided the current one, which contains top-level declarations for the entire
module, the error message would incorrectly suggest the out-of-scope `bar` and
`bad` as possible alternatives for `bar` (see Trac #11680).  Instead, the
typechecker must use the same GlobalRdrEnv the renamer used when it determined
that `bar` is out-of-scope.
...
data HsExpr p
  ...
  -- For details on above see Note [Api annotations] in
  -- GHC.Haskell.Syntax.Annotation
  | HsPar       (LHsExpr p)   -- ^ Parenthesised expr;
                              -- see Note [Parens in Haskell syntax]

  | SectionL    (LHsExpr p)   -- operand; see Note [Sections in Haskell syntax]
                (LHsExpr p)   -- operator
  | SectionR    (LHsExpr p)   -- operator; see Note [Sections in Haskell syntax]
                (LHsExpr p)   -- operand
  ...
  -- For details on above see Note [Api annotations] in
  -- GHC.Haskell.Syntax.Annotation
  | HsSCC       SourceText            -- Note [Pragma source text] in
                                      -- GHC.Types.BasicTypes
  ...
  -- For details on above see Note [Api annotations] in
  -- GHC.Haskell.Syntax.Annotation
  | HsCoreAnn   SourceText            -- Note [Pragma source text] in
                                      -- GHC.Types.BasicTypes
  ...
  --    immediately in parser.GHC.Haskell.Parser.Syntax.checkCommand
  ...
  | HsTickPragma                      -- A pragma introduced tick
     ...
     ((SourceText,SourceText),(SourceText,SourceText))
        ...
        -- See Note [Pragma source text] in GHC.Types.BasicTypes
     (LHsExpr p)
  ...
  -- GHC.Compiler.HaskellToCore.Expression. This invariant is maintained by
...
Note [Sections in Haskell syntax]
~~~~~~~~~~~~~~~~~~~~~~~~
...
{-
GHC.Haskell records exactly where the user put parens, with HsPar.
...
-- Exported to GHC.Haskell.Syntax.Binding, which can't see the defn of
-- HsMatchContext
...
data HsSplice id
   ...
   | HsQuasiQuote        -- See Note [Quasi-quote overview] in
...
-- GHC.Haskell.Rename.Splice. For how this is used.
--
 ...
 * Pending expression splices (UntypedExpSplice), e.g.,
   ...
   UntypedExpSplice is also used for
     * quasi-quotes, where the pending expression expands to
          $(quoter "...blah...")
       (see GHC.Haskell.Rename.Splice.makePending, HsQuasiQuote case)

     * cross-stage lifting, where the pending expression expands to
          $(lift x)
       (see GHC.Haskell.Rename.Splice.checkCrossStageLifting)
...
data HsMatchContext id -- Not an extensible tag
  ...
  | RecUpd                      -- ^Record update [used only in
                                --    GHC.Compiler.HaskellToCore.Expression
                                --    to tell matchWrapper what sort of
