...
import GHC.Types.OccName          ( HasOccName(..), isTcOcc, isSymOcc )
import GHC.Types.BasicTypes       ( SourceText(..), StringLiteral(..)
                                   , pprWithSourceText )
...
See Note [Representing fields in AvailInfo] in GHC.Types.Available for more
details.
-}
