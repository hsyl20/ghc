...
import GHC.CoreTypes.OccName          ( HasOccName(..), isTcOcc, isSymOcc )
import GHC.CoreTypes.BasicTypes       ( SourceText(..), StringLiteral(..)
                                   , pprWithSourceText )
import GHC.CoreTypes.FieldLabel       ( FieldLbl(..) )
...
data ImportDecl name
  = ImportDecl {
      ideclSourceSrc :: SourceText,    -- Note [Pragma source text] in
...
See Note [Representing fields in AvailInfo] in GHC.CoreTypes.Available for more
details.
-}
