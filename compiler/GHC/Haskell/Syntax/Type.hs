...
Abstract syntax: user-defined types
-}
...
import GHC.Haskell.Syntax.PlaceHolder ( PlaceHolder(..) )
...
import GHC.CoreTypes.Name.Set          ( NameSet, emptyNameSet )
import GHC.CoreTypes.DataCon   ( HsSrcBang(..), HsImplBang(..)
                                    , SrcStrictness(..), SrcUnpackedness(..) )
...
See also Note [Kind and type-variable binders] in GHC.Haskell.Rename.Type
...
* The explicit presence of these wrappers specifies, in the
  GHC.Haskell.Syntax, exactly where implicit quantification is allowed, and
...
data LHsQTyVars pass   -- See Note [HsType binders]
  = HsQTvs { hsq_implicit :: PostRn pass [Name]
           ...
           , hsq_dependent :: PostRn pass NameSet
               ...
               -- See Note [Dependent LHsQTyVars] in
...
data HsImplicitBndrs pass thing   -- See Note [HsType binders]
  = HsIB { hsib_vars :: PostRn pass [Name] -- Implicitly-bound kind & type vars
         ...
         , hsib_closed :: PostRn pass Bool 
          -- Taking the hsib_vars into account, is the payload closed? Used in
          -- GHC.Haskell.TypeCheck.Type.decideKindGeneralisationPlan
    }
...
data HsTyVarBndr pass
  = UserTyVar        -- no explicit kinding
         (Located (IdP pass))
        -- See Note [Located RdrNames] in GHC.Haskell.Syntax.Expression
  | KindedTyVar
         (Located (IdP pass))
...
data HsType pass
  ...
  | HsParTy             (LHsType pass)   -- See Note [Parens in Haskell syntax]
                                         -- in GHC.Haskell.Syntax.Expression
...
-- Note [Literal source text] in GHC.CoreTypes.BasicTypes for SourceText fields in
-- the following
...
Note [Context quantification] in GHC.Haskell.Rename.Type, and Trac #4426.
...
? Is it `(*) Foo Int` or `Foo GHC.Types.* Int`? There's no way to know until
renaming.  So we just take type expressions like this and put each component in
a list, so be sorted out in the renamer. The sorting out is done by
GHC.Haskell.Rename.Type.mkHsOpTyRn. This means that the parser should never
produce HsAppTy or HsOpTy.
...
-- See Note [Sorting out the result type] in GHC.Haskell.Parser.Syntax
updateGadtResult
  :: (Monad m)
     => (SDoc -> m ())
     -> SDoc
...
-- occurrences).
--
-- See Note [HsRecField and HsRecUpdField] in GHC.Haskell.Syntax.Pattern and
-- Note [Disambiguating record fields] in GHC.Haskell.TypeCheck.Expression.
-- See Note [Located RdrNames] in GHC.Haskell.Syntax.Expression
