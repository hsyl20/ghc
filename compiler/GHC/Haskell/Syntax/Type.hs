...
import GHC.Types.DataCon   ( HsSrcBang(..), HsImplBang(..)
                                    , SrcStrictness(..), SrcUnpackedness(..) )
...
data HsImplicitBndrs pass thing   -- See Note [HsType binders]
  = HsIB { hsib_vars :: PostRn pass [Name] -- Implicitly-bound kind & type vars
         ...
         , hsib_closed :: PostRn pass Bool 
          ...
          -- GHC.Haskell.TypeCheck.UserType.decideKindGeneralisationPlan
    }
...
-- Note [Literal source text] in GHC.Types.BasicTypes for SourceText fields in
-- the following
