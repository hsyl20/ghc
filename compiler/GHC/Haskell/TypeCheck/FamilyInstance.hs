...
module GHC.Haskell.TypeCheck.FamilyInstance (
...
import GHC.Types.Instance( roughMatchTcs )
...
import GHC.Types.DataCon ( dataConName )
...
certain that the modules in our `GHC.Types.Base.dep_finsts' are consistent.)
...
tcLookupDataFamInst_maybe fam_inst_envs tc tc_args
  ...
  , let rep_tc = dataFamInstRepTyCon rep_fam
        co     = mkUnbranchedAxInstCo Representational ax rep_args
                                      (mkCoVarCos cvs)
  = ASSERT( null rep_cos ) -- See Note [Constrained family instances] 
...
checkForInjectivityConflicts instEnvs famInst
    ...
    { let axiom = coAxiomSingleBranch fi_ax
          ...
          -- see Note [Verifying injectivity annotation] in
