...
module GHC.Haskell.TypeCheck.Util.CoreType (
  --------------------------------
  -- Types
...
import GHC.CoreTypes.Name as Name -- hiding (varName)
            -- We use this to make dictionaries for type literals.
            -- Perhaps there's a better way to do this?
import GHC.CoreTypes.Name.Set
import GHC.CoreTypes.Var.Environment
import GHC.Builtin.Names
import GHC.Builtin.Types( coercibleClass, unitTyCon, unitTyConKey
...
import GHC.Util.Error( Validity(..), MsgDoc, isValid )
...
-- See Note [ExpType] in GHC.Haskell.TypeCheck.Util.Monadic, where you'll
-- also find manipulators.
data ExpType = Check TcType
             | Infer !InferResult

data InferResult
  = IR { ir_uniq :: Unique  -- For debugging only
       , ir_lvl  :: TcLevel -- See Note [TcLevel of ExpType] 
                            -- in GHC.Haskell.TypeCheck.Util.Monadic
...
--
-- This is defined here to avoid defining it in
-- GHC.Haskell.TypeCheck.Expression .hs-boot file.
data SyntaxOpType
  = SynAny     -- ^ Any type
...
data TcTyVarDetails
  = SkolemTv      -- A skolem
       ...
       Bool       -- True <=> this skolem type variable can be overlapped
                  ...
                  -- See Note [Binding when looking up instances] in
...
-- See Note [Binding when looking up instances] in GHC.CoreTypes.Instance
...
data MetaInfo
   ...
   | FlatMetaTv    -- A flatten meta-tyvar
                   ...
                   -- See Note [The flattening story] in
...
smart-app checking code --- see GHC.Haskell.TypeCheck.Expression.tcIdApp
...
-- See also Note [Scope-check inferred kinds] in
...
* Determinism: when we quantify over type variables we decide the
  ...
  about deterministic sets see Note [Deterministic UniqFM] in
  GHC.Data.UniqueDFM.
...
-- GHC.Interactive.ClosureInspect.zonkRTTIType.  These are "ambiguous" in
...
-- Called only in GHC.Haskell.TypeCheck.Error; see Note [Runtime skolems]
-- there
isRuntimeUnkSkol x
...
-- See Note [Type-checking inside the knot] in
-- GHC.Haskell.TypeCheck.Type
...
-- decomposing tycon applications] in
-- GHC.Haskell.TypeCheck.Solver.Canonicalise for details.
...
-- equality] (in GHC.CoreTypes.Type.Internal) as `eqType`, but Type.eqType
-- believes (* == Constraint), and that is NOT what we want in the type checker!
tcEqType ty1 ty2
...
tc_eq_type view_fun orig_ty1 orig_ty2 = go True orig_env orig_ty1 orig_ty2
  where
    ...
    go vis env (FunTy arg res) ty
      ...
      -- See Note [Equality on AppTys] in GHC.CoreTypes.Type
...
-- This function is here rather than in GHC.Haskell.TypeCheck.Validity
-- because it is called from GHC.Haskell.TypeCheck.Simplify,
-- which itself is imported by GHC.Haskell.TypeCheck.Validity
...
See also GHC.Haskell.TypeCheck.TypeDeclUtil.checkClassCycles.
...
-- GHC.Haskell.TypeCheck.Solver.Canonicalise for the motivation for
-- this function.
...
************************************************************************
*                                                                      *
\subsection[GHC.Builtin.Types-ext-type]{External types}
*                                                                      *
************************************************************************
