...
module GHC.Haskell.TypeCheck.Error(
...
import GHC.Haskell.TypeCheck.MutableType
...
import GHC.Types.Type.Unify                      ( tcMatchTys )
import GHC.Types.Module
import GHC.Haskell.TypeCheck.FamilyInstance
import GHC.Types.FamilyInstance            ( flattenTys )
...
import GHC.Haskell.Syntax.Expression  ( UnboundVar(..) )
import GHC.Haskell.Syntax.Binding     ( PatSynBind(..) )
...
import GHC.Types.ConLike            ( ConLike(..), conLikeWrapId_maybe )
import GHC.Utils
import GHC.Types.Base               ( HscEnv, lookupTypeHscEnv, TypeEnv
                                    , lookupTypeEnv )
import GHC.Types.Name.Environment   ( lookupNameEnv )
...
import GHC.Types.Var.Free ( fvVarList, unionFV )
...
we run into a type mismatch in GHC.Haskell.TypeCheck.Unify, we normally
...
GHC.Haskell.TypeCheck.Error.
GHC.Haskell.TypeCheck.Error.reportTidyWanteds does not print the errors and
...
reportWanteds ctxt tc_lvl (WC { wc_simple = simples, wc_insol = insols, wc_impl = implics })
 ...
 where
    ...
    -- (see GHC.Haskell.TypeCheck.Types.trulyInsoluble) is caught here, otherwise
...
`GHC.Haskell.TypeCheck.Unify` and `tcCheckSatisfiability` in
`GHC.Haskell.TypeCheck.Constraint.Simplify`.

Unfortunately, `GHC.Haskell.TypeCheck.Constraint.Simplify` uses
`GHC.Haskell.TypeCheck.Error` to report errors found during constraint
checking, so checking for subsumption in holes would involve shuffling some code
around in `GHC.Haskell.TypeCheck.Constraint.Simplify`, to make a
...
mkTyVarEqErr' dflags ctxt report ct oriented tv1 co1 ty2
  ...
  , isUserSkolem ctxt tv1
    ...
    -- way round; see
    -- GHC.Haskell.TypeCheck.Constraint.Canonicalise.canEqTyVarTyVar
...
We don't re-use the code in GHC.Haskell.TypeCheck.Constraint.Canonicalise
