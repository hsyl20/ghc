...
import GHC.Haskell.Syntax.Bind     ( PatSynBind(..) )
...
reportWanteds ctxt tc_lvl (WC { wc_simple = simples, wc_impl = implics })
 ...
 where
    ...
    -- (see GHC.Haskell.TypeCheck.Util.insolubleWantedCt) is caught here, otherwise
...
mkTyVarEqErr' dflags ctxt report ct oriented tv1 co1 ty2
  ...
  , isUserSkolem ctxt tv1
    ...
    -- way round; see
    -- GHC.Haskell.TypeCheck.Solver.Canonicalise.canEqTyVarTyVar
...
We don't re-use the code in GHC.Haskell.TypeCheck.Solver.Canonicalise
