...
module GHC.Haskell.TypeCheck.Validity (
...
import GHC.Haskell.TypeCheck.Simplify (simplifyAmbiguityCheck)
import GHC.CoreTypes.Type.Internal
import GHC.Haskell.TypeCheck.Util.CoreType hiding ( sizeType, sizeTypes )
...
-- others:
import GHC.Haskell.Syntax                 -- HsType
import GHC.Haskell.TypeCheck.Monad        -- TcType, amongst others
import GHC.Haskell.TypeCheck.Environment  ( tcGetInstEnvs, tcInitTidyEnv, tcInitOpenTidyEnv )
import GHC.Haskell.TypeCheck.FunctionalDependency
import GHC.CoreTypes.Instance             ( InstMatch, lookupInstEnv )
import GHC.CoreTypes.FamilyInstance       ( isDominatedBy
                                          , injectiveBranches
                                          , InjectivityCheckResult(..) )
import GHC.Haskell.TypeCheck.FamilyInstance( makeInjectivityErrors )
import GHC.CoreTypes.Name
import GHC.CoreTypes.Var.Environment
import GHC.CoreTypes.Var.Set
import GHC.Data.UniqueSet
import GHC.CoreTypes.Var         ( TyVarBndr(..), mkTyVar )
...
import GHC.CoreTypes.Unique      ( mkAlphaTyVarUnique )
...
'GHC.Haskell.TypeCheck.FunctionalDependency.grow'.
-}
...
-- See Note [Extended defaults]" in GHC.Haskell.TypeCheck.Environment
...
GHC.Haskell.TypeCheck.Solver.Interact Note [Instance and Given
overlap].  As that Note discusses, for the most part the clever stuff in
GHC.Haskell.TypeCheck.Solver.Interact means that we don't use a
...
circumstances that GHC.Haskell.TypeCheck.Solver.Interact.matchInstEnv
...
instances] in GHC.Haskell.TypeCheck.Solver.Interact).  However, we
...
derived instance contexts] in GHC.Haskell.Derive.  However the predicate is
...
checkValidCoAxiom ax@(CoAxiom { co_ax_tc = fam_tc, co_ax_branches = branches })
  ...
  where
    ...
    check_branch_compat prev_branches cur_branch
     ...
     -- See Note [Verifying injectivity annotation] in GHC.CoreTypes.FamilyInstance
