...
import GHC.Haskell.TypeCheck.Constraint.Simplify (simplifyAmbiguityCheck)
import GHC.Types.TypeAndCoercion
import GHC.Haskell.TypeCheck.Type hiding ( sizeType, sizeTypes )
...
import GHC.Haskell.TypeCheck              -- TcType, amongst others
import GHC.Haskell.TypeCheck.Environment  ( tcGetInstEnvs )
import GHC.Haskell.TypeCheck.FunctionalDependency
import GHC.Types.Instance                 ( InstMatch, lookupInstEnv )
import GHC.Types.FamilyInstance                ( isDominatedBy
                                                , injectiveBranches
...
import GHC.Types.Var         ( TyVarBndr(..), mkTyVar )
...
GHC.Haskell.TypeCheck.Constraint.Interact Note [Instance and Given
overlap].  As that Note discusses, for the most part the clever stuff in
GHC.Haskell.TypeCheck.Constraint.Interact means that we don't use a
...
circumstances that GHC.Haskell.TypeCheck.Constraint.Interact.matchInstEnv
...
instances] in GHC.Haskell.TypeCheck.Constraint.Interact).  However, we
...
checkValidCoAxiom ax@(CoAxiom { co_ax_tc = fam_tc, co_ax_branches = branches })
  ...
  where
    ...
    check_branch_compat prev_branches cur_branch
     ...
     -- See Note [Verifying injectivity annotation] in GHC.Types.FamilyInstance
