...
module GHC.Haskell.TypeCheck.Constraint.Simplify(
...
import GHC.Data.Bag
import GHC.Types.Class      ( Class, classKey, classTyCon )
...
import GHC.Haskell.TypeCheck.Constraint.Interact
import GHC.Haskell.TypeCheck.Constraint.Canonicalise   ( makeSuperClasses )
import GHC.Haskell.TypeCheck.MutableType   as TcM
import GHC.Haskell.TypeCheck as TcM
import GHC.Haskell.TypeCheck.Constraint.Solve  as TcS
import GHC.Haskell.TypeCheck.Type
import GHC.Data.TrieMap       () -- DV: for now
import GHC.Types.Type
import GHC.Builtin.Types      ( liftedRepTy )
import GHC.Types.Type.Unify        ( tcMatchTyKi )
import GHC.Utils
import GHC.Types.Var
import GHC.Types.Var.Set
import GHC.Data.UniqueSet
import GHC.Types.BasicTypes  ( IntWithInf, intGtLimit )
...
captureTopConstraints thing_inside
  = do { static_wc_var <- TcM.newTcRef emptyWC ;
       ...
       -- See GHC.Haskell.TypeCheck Note [Constraints and errors]
       ...
       ; case mb_res of
           ...
           Left {}   -> do { _ <- reportUnsolved lie; failM } }
                ...
                -- this function is here instead of GHC.Haskell.TypeCheck
...
-- See Note [Overview of implicit CallStacks] in GHC.Haskell.TypeCheck.Evidence
...
How is this implemented? It's complicated! So we'll step through it all:
 ...
 1) `GHC.Types.Instance.lookupInstEnv`
    Performs instance resolution, so this is where we check if a particular
    type-class method call is safe or unsafe. We do this through the return
    type, `ClsInstLookupResult`, where the last parameter is a list of instances
    that are unsafe to overlap. When the method call is safe, the list is null.

 2) `GHC.Haskell.TypeCheck.Constraint.Interact.matchClassInst`
    This module drives the instance resolution / dictionary generation.  The
    return type is `LookupInstResult`, which either says no instance matched, or
    one found, and if it was a safe or unsafe overlap.

 3) `GHC.Haskell.TypeCheck.Constraint.Interact.doTopReactDict`
    Takes a dictionary / class constraint and tries to resolve it by calling (in
    part) `matchClassInst`. The resolving mechanism has a work list (of
    constraints) that it process one at a time. If the constraint can't be
    resolved, it's added to an inert set. When compiling an `-XSafe` or
    `-XTrustworthy` module, we follow this approach as we know compilation
    should fail. These are handled as normal constraint resolution failures from
    here-on (see step 6).
 ...
 4) `GHC.Haskell.TypeCheck.Constraint.Simplify.simplifyTop`:
       ...
       * Once finished, call `getSafeOverlapFailures` to retrieve the
         list of overlapping instances that were successfully resolved, but
         unsafe. Remember, this is only applicable for generating warnings
         (`-Wunsafe`) or inferring a module unsafe. `-XSafe` and `-XTrustworthy`
         cause compilation failure by not resolving the unsafe constraint at
         all.

       * For unresolved constraints (all types), call
         `GHC.Haskell.TypeCheck.Error.reportUnsolved`, while for resolved
         but unsafe overlapping dictionary constraints, call
         `GHC.Haskell.TypeCheck.Error.warnAllUnsolved`. Both functions
         convert constraints into a warning message for the user.

       * In the case of `warnAllUnsolved` for resolved, but unsafe
         dictionary constraints, we collect the generated warning message (pop
         it) and call `GHC.Haskell.TypeCheck.recordUnsafeInfer` to mark the
         module we are compiling as unsafe, passing the warning message along as
         the reason.

 5) `GHC.Haskell.TypeCheck.Error.*Unsolved`
    Generates error messages for constraints by actually calling
    `InstEnv.lookupInstEnv` again! Yes, confusing, but all we know is the
    constraint that is unresolved or unsafe. For dictionary, all we know is that
    we need a dictionary of type C, but not what instances are available and how
    they overlap. So we once again call `lookupInstEnv` to figure that out so we
    can generate a helpful error message.

 6) `GHC.Haskell.TypeCheck.recordUnsafeInfer`
    Save the unsafe result and reason in an IORef called `tcg_safeInfer`.

 7) `GHC.Driver.Main.tcRnModule'`
    Reads `tcg_safeInfer` after type-checking, calling
    `GHC.Driver.Main.markUnsafeInfer` (passing the reason along) when
    safe-inferrence failed.
 ...
 * Gather the constraints for the RHS with ambient level *one more than*
   ...
   in GHC.Haskell.TypeCheck.Binding.tcPolyInfer
...
data InferMode = ApplyMR          -- ^ Apply the monomorphism restriction,
               ...
               | EagerDefaulting  -- ^ See Note [TcRnExprMode] in
...
growThetaTyVars theta tvs
  ...
  where
    ...
    (ips, non_ips) = partition isIPPred theta
                         -- See Note [Inheriting implicit parameters] in
                         -- GHC.Haskell.TypeCheck.Type
...
solveWanteds wc@(WC { wc_simple = simples, wc_insol = insols, wc_impl = implics })
  = do { traceTcS "solveWanteds {" (ppr wc)
       ...
       ; wc1 <- solveSimpleWanteds (simples `unionBags` insols)
                -- Why solve 'insols'?  See Note [Rewrite insolubles] in
                -- GHC.Haskell.TypeCheck.Constraint.Solve
...
-- See Note [The superclass story] in GHC.Haskell.TypeCheck.Constraint.Canonicalise
...
*  To find (a), when we have two Given constraints,
   ...
   the Ord dictionary.  This is done by
   GHC.Haskell.TypeCheck.Constraint.Interact.solveOneFromTheOther
...
* GHC.Haskell.TypeCheck.Error does the actual warning, in
  warnRedundantConstraints.

* We don't report redundant givens for *every* implication; only for those which
  reply True to
  GHC.Haskell.TypeCheck.Constraint.Simplify.warnRedundantGivens:
   ...
   - More subtly, in a function definition
     ...
     other set False in
        - GHC.Haskell.TypeCheck.Binding.tcSpecPrag
        - GHC.Haskell.TypeCheck.Binding.tcTySig

  This decision is taken in setImplicationStatus, rather than
  GHC.Haskell.TypeCheck.Error so that we can discard implication
  constraints that we don't need.  So ics_dead consists only of the *reportable*
  redundant givens.
...
-- invariant (MetaTvInv) in Note [TcLevel and untouchable type variables] in
-- GHC.Haskell.TypeCheck.Type
...
approximateWC to produce a list of candidate constraints.  Then we MUST
  ...
  a) Promote any meta-tyvars that have been floated out by
     ...
     Note [TcLevel and untouchable type variables] in
     GHC.Haskell.TypeCheck.Type.
...
(MetaTvInv) from Note [TcLevel and untouchable type variables] in
GHC.Haskell.TypeCheck.Type.  for the leftover implication.
...
GHC.Haskell.TypeCheck.Constraint.Simplify prior to Oct 2014).
