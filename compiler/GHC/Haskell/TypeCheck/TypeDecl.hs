...
module GHC.Haskell.TypeCheck.TypeDecl (
...
import GHC.Haskell.TypeCheck.TypeDeclUtil
import GHC.Haskell.TypeCheck.Class
...
import GHC.CoreTypes.Type.Internal   -- for checkValidRoles
...
Then:
  ...
  * During GHC.Haskell.TypeCheck.Type.kcTyVar we look in the *local*
    env, to get the known kind for T.

  * But in GHC.Haskell.TypeCheck.Type.ds_type (and ds_var_app in
  ...
  = -- Closed type families are a little tricky, because they contain the definition
    ...
    do { traceTc "Closed type family:" (ppr tc_name)
       ...
       ; branches <- mapM (tcTyFamInstEqn tc_fam_tc Nothing) eqns
         ...
         -- differentiate names.
         -- See [Zonking inside the knot] in GHC.Haskell.TypeCheck.Type
...
checkValidTyCon tc
  ...
  where
    ...
    -- See Note [GADT record selectors] in GHC.Haskell.TypeCheck.TypeDeclUtil
  ...
  = do { -- strip off the duplicates and look for ill-scoped things
       ...
       ; checkValidTelescope (pprTyVars vis_tvs) stripped_tvs extra
         `and_if_that_doesn't_error`
           ...
           -- It reports errors around Note [Dependent LHsQTyVars] in GHC.Haskell.TypeCheck.Type
         when duplicate_vars (
...
-- -dcore-lint is enabled. See Note [Role inference] in GHC.Haskell.TypeCheck.TypeDeclUtil
    ...
    check_dc_roles datacon
      = do { traceTc "check_dc_roles" (ppr datacon <+> ppr (tyConRoles tc))
           ; mapM_ (check_ty_roles role_env Representational) $
                    ...
                    -- GHC.Haskell.TypeCheck.TypeDeclUtil
      where
