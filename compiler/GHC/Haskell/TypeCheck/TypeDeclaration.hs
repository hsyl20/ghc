...
module GHC.Haskell.TypeCheck.TypeDeclaration(
        RolesInfo,
        inferRoles,
        checkSynCycles,
        checkClassCycles,
...
import GHC.Haskell.TypeCheck.Binding ( tcRecSelBinds )
import GHC.Types.TypeAndCoercion         ( Type(..), Coercion(..)
                                          , UnivCoProvenance(..) )
...
import GHC.Types.Coercion                ( ltRole )
...
[Checked in GHC.Haskell.TypeCheck.TypeAndClassDeclaration.checkValidTyCon]
E.g.
        data T where
          T1 { f :: Maybe a } :: T [a]
