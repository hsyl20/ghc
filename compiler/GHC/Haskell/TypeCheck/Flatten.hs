...
module GHC.Haskell.TypeCheck.Flatten(
   FlattenMode(..),
...
GHC.Haskell.TypeCheck.Constraint.Canonicalise.canEqTyVar.

See also Note [Conservative unification check] in
GHC.Haskell.TypeCheck.Unify, which gives other examples where lazy
...
GHC.Haskell.TypeCheck.Constraint.Canonicalise.homogeniseRhsKind).
...
(unlike TyConAppCo), see Note [TcCoercions] in GHC.Haskell.TypeCheck.Evidence.
...
GHC.Haskell.TypeCheck.Constraint.Interact.kick_out, by only kicking out an
inert only if 
