...
module GHC.Haskell.TypeCheck.Binding
...
import {-# SOURCE #-} GHC.Haskell.TypeCheck.Matches
                           ( tcGRHSsPat , tcMatchesFun )
import {-# SOURCE #-} GHC.Haskell.TypeCheck.Expression
                           ( tcMonoExpr )
import {-# SOURCE #-} GHC.Haskell.TypeCheck.PatternSynonym
                           ( tcInferPatSynDecl, tcCheckPatSynDecl
                           , tcPatSynBuilderBind )
import GHC.Core.Syntax     (Tickish (..))
import GHC.Types.CostCentre  (mkUserCC)
import GHC.Config.Flags
import GHC.Data.FastString
import GHC.Haskell.Syntax
import GHC.Types.Base       ( isHsBootOrSig )
import GHC.Haskell.TypeCheck.Signature
...
import GHC.Types.Var.Environment( TidyEnv )
...
import GHC.Haskell.TypeCheck.Validity        ( checkValidType )
...
@tcBindsAndThen@ typechecks a @GHC.Haskell.Syntax.Binding@.  The "and then"
...
tc_group top_lvl sig_fn prag_fn (Recursive, binds) closed thing_inside
  =     -- To maximise polymorphism, we do a new
        ...
        -- See Note [Polymorphic recursion] in GHC.Haskell.Syntax.Binding.
...
-- GHC.Haskell.TypeCheck.Environment.tcExtendIdEnv
  ...
  = checkNoErrs $  -- The checkNoErrs ensures that if the type is ambiguous
    ...
    do { fam_envs <- tcGetFamInstEnvs
       ; let (_co, mono_ty') = normaliseType fam_envs Nominal mono_ty
               ...
               -- GHC.Haskell.TypeCheck.Flatten) so do it here to make it
...
tcVect (HsVectTypeOut _ _ _)
  = panic "GHC.Haskell.TypeCheck.Binding.tcVect: Unexpected 'HsVectTypeOut'"
...
tcVect (HsVectClassOut _)
  = panic "GHC.Haskell.TypeCheck.Binding.tcVect: Unexpected 'HsVectClassOut'"
...
tcVect (HsVectInstOut _)
  = panic "GHC.Haskell.TypeCheck.Binding.tcVect: Unexpected 'HsVectInstOut'"
...
tcMonoBinds is_rec sig_fn no_gen
  ...
  =     -- Note [Single function non-recursive binding special-case]
    ...
    do  { ((co_fn, matches'), rhs_ty)
            <- tcInferInst $ \ exp_ty ->
                  -- tcInferInst: see GHC.Haskell.TypeCheck.Unify,
...
We typecheck pattern bindings as follows.  First tcLhs does this:
  ...
  3. Invoke tcLetPat to typecheck the pattern.
     ...
     - We pass in the current TcLevel.  This is captured by
       GHC.Haskell.TypeCheck.Pattern.tcLetPat, and put into the pc_lvl field
     ...
     - When we come to a binder (GHC.Haskell.TypeCheck.Pattern.tcPatBndr),
