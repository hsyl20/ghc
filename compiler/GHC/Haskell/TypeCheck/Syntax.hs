...
Specialisations of the @GHC.Haskell.Syntax@ for the typechecker

This module is an extension of @GHC.Haskell.Syntax@, for use in the type
checker.
-}
...
module GHC.Haskell.TypeCheck.Syntax (
        -- * Extracting types from GHC.Haskell
        hsLitType, hsLPatType, hsPatType,

        -- * Other GHC.Haskell functions
        mkHsDictLet, mkHsApp,
...
import GHC.Types.TyCon   ( isUnboxedTupleTyCon )
...
************************************************************************
*                                                                      *
       Extracting the type from GHC.Haskell
*                                                                      *
************************************************************************
...
************************************************************************
*                                                                      *
\subsection[BackSubst-HsBinds]{Running a substitution over @GHC.Haskell.Syntax.Binding@}
*                                                                      *
************************************************************************
...
It's all pretty boring stuff, because GHC.Haskell is such a large type, and
...
is justified by the use of newUnique in
GHC.Haskell.TypeCheck.MutableType.instSkolTyCoVarX.)
...
zonkVect _ (HsVectTypeIn _ _ _ _) = panic "zonkVect: HsVectTypeIn"
zonkVect _env (HsVectClassOut c)
  = return $ HsVectClassOut c
zonkVect _ (HsVectClassIn _ _) = panic "zonkVect: HsVectClassIn"
zonkVect _env (HsVectInstOut i)
  = return $ HsVectInstOut i
zonkVect _ (HsVectInstIn _) = panic "zonkVect: HsVectInstIn"
...
* In GHC.Compiler.HaskellToCore.Binding, we quantify over it.
  See GHC.Compiler.HaskellToCore.Binding Note [Free tyvars on rule LHS]
