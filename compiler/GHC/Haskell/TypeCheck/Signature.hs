...
module GHC.Haskell.TypeCheck.Signature(
       TcSigInfo(..),
...
import GHC.Haskell.TypeCheck.Unify         ( tcSkolemise, unifyType )
import GHC.Haskell.TypeCheck.Instantiation ( topInstantiate )
import GHC.Haskell.TypeCheck.Environment   ( tcLookupId )
import GHC.Haskell.TypeCheck.Evidence      ( HsWrapper, (<.>) )
import GHC.Types.Type                      ( mkTyVarBinders )

import GHC.Config.Flags
import GHC.Types.Var      ( TyVar, tyVarName, tyVarKind )
...
* GHC.Haskell: a signature in a binding starts of as a TypeSig, in
  type GHC.Haskell.Syntax.Binding.Sig
...
* tcInstSig: TcIdSigInfo -> TcIdSigInst
  ...
  binder.  You can see this in GHC.Haskell.TypeCheck.Binding.tcLhsId.

  The instantiation does the obvious thing for complete signatures,
  but for /partial/ signatures it starts from the GHC.Haskell, so it
...
impedance-matching check in GHC.Haskell.TypeCheck.Binding.mkExport

See Note [Signature skolems] in GHC.Haskell.TypeCheck.Type

None of this applies to a function binding with a complete
signature, which doesn't use tcInstSig.
See GHC.Haskell.TypeCheck.Binding.tcPolyCheck.
-}
...
3. Note that the HsWrapper can transform *any* function with the right
   ...
   GHC.Haskell.TypeCheck.Instance.mk_meth_spec_prags
-}
