...
import GHC.CoreTypes.Type                      ( mkTyVarBinders )

import GHC.Config.Flags
import GHC.CoreTypes.Var      ( TyVar, tyVarName, tyVarKind )
import GHC.CoreTypes.Id       ( Id, idName, idType, idInlinePragma, setInlinePragma, mkLocalId )
...
* GHC.Haskell: a signature in a binding starts of as a TypeSig, in
  type GHC.Haskell.Syntax.Bind.Sig
...
* tcInstSig: TcIdSigInfo -> TcIdSigInst
  ...
  binder.  You can see this in GHC.Haskell.TypeCheck.Bind.tcLhsId.
...
impedance-matching check in GHC.Haskell.TypeCheck.Bind.mkExport

See Note [Signature skolems] in GHC.Haskell.TypeCheck.Util.CoreType

None of this applies to a function binding with a complete
signature, which doesn't use tcInstSig.
See GHC.Haskell.TypeCheck.Bind.tcPolyCheck.
-}
...
From the TcSpecPrag, in GHC.HaskellToCore.Bind we generate a binding
