...
GHC.Haskell.TypeCheck.Solver.Interact) because we can't have infinitely
many instances.
...
2. In a function call, instead of simply passing the given IP, we first
   ...
   (see GHC.Haskell.TypeCheck.Solver.Interact.interactDict)
...
3. We default any insoluble CallStacks to the empty CallStack. Suppose
   ...
   (see GHC.Haskell.TypeCheck.Simplify.simpl_top and
   GHC.Haskell.TypeCheck.Simplify.defaultCallStacks)
...
- An EvCallStack term desugars to a CoreExpr of type `IP "some str" CallStack`.
  ...
  call-site onto a given stack (See GHC.HaskellToCore.Bind.dsEvCallStack)
