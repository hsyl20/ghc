...
module GHC.Haskell.TypeCheck.Evidence (
...
import GHC.Core.Printer ()   -- Instance OutputableBndr TyVar
...
newtype EvBindMap
  = EvBindMap {
       ev_bind_varenv :: DVarEnv EvBind
            ...
            -- let $dEq = GHC.Classes.$fEqInt in ...
            --
            -- See Note [Deterministic UniqFM] in
            -- GHC.Data.UniqueDFM for explanation why
...
GHC.Haskell.TypeCheck.Constraint.Interact) because we can't have infinitely
many instances.
...
2. In a function call, instead of simply passing the given IP, we first
   ...
   (see GHC.Haskell.TypeCheck.Constraint.Interact.interactDict)
...
3. We default any insoluble CallStacks to the empty CallStack. Suppose
   ...
   (see GHC.Haskell.TypeCheck.Constraint.Simplify.simpl_top and
   GHC.Haskell.TypeCheck.Constraint.Simplify.defaultCallStacks)
...
-- Note [Type-checking overloaded labels] in GHC.Haskell.TypeCheck.Expression.
