...
module GHC.Haskell.TypeCheck.MutableType (
...
-- as it's used in GHC.Haskell.TypeCheck.Syntax in the presence of knots.
...
-- the unifier; see GHC.Haskell.TypeCheck.Unify.nicer_to_update_tv1, and
...
See Note [TcLevel assignment] in GHC.Haskell.TypeCheck.Type.
-}
...
Note [Deterministic UniqFM] in GHC.Data.UniqueDFM.
-}

quantifyTyVars
  :: TcTyCoVarSet     -- Global tvs; already zonked
  -> CandidatesQTvs   -- See Note [Dependent type variables] in
  ...
  = do { traceTc "quantifyTyVars" (vcat [ppr dvs, ppr gbl_tvs])
       ; let all_cvs = filterVarSet isCoVar $ dVarSetToVarSet dep_tkvs
             ...
             nondep_tvs = dVarSetElems $
                 ...
                 -- See Note [Dependent type variables] in
                 -- GHC.Haskell.TypeCheck.Type
...
defaultTyVar default_kind tv
  ...
  where
    ...
    -- on SigTvs; see Note [Signature skolems] in
    -- GHC.Haskell.TypeCheck.Type.
  ...
  = ASSERT2( isMetaTyVar tv, ppr tv )
    do  { when debugIsOn (check_empty tv)
        ...
        ; let uniq        = getUnique tv
                ...
                -- important for GHC.Haskell.TypeCheck.UserType.splitTelescopeTvs to work properly
...
GHC.Haskell.TypeCheck.Constraint.Simplify.tcSimplifyInferCheck).
...
simplifier by GHC.Haskell.TypeCheck.Module.tcRnSrcDecls calling
GHC.Haskell.TypeCheck.Constraint.Simplify.tcSimplifyTop.
...
-- GHC.Haskell.TypeCheck.UserType
zonkTcTypeAndFV ty
...
zonkTyCoVar tv | isTcTyVar tv = zonkTcTyVar tv
   ...
   -- GHC.Haskell.TypeCheck.UserType.tcTyClTyVars, but it seems
...
Why?, for example:
- For CDictCan, the
  @GHC.Haskell.TypeCheck.Constraint.Simplify.expandSuperClasses@ step,
...
@GHC.Haskell.TypeCheck.Constraint.Canonicalise@ even if they come in as
...
-- See Note [SigSkol SkolemInfo] in GHC.Haskell.TypeCheck.Types
