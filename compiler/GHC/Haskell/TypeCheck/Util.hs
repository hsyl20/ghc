...
Various types used during typechecking, please see GHC.Haskell.TypeCheck.Monad as
...
module GHC.Haskell.TypeCheck.Util(
...
import GHC.CoreTypes.TyCon  ( TyCon, tyConKind )
import GHC.CoreTypes.Coercion         ( Coercion, mkHoleCo )
import GHC.CoreTypes.ConLike  ( ConLike(..) )
import GHC.CoreTypes.DataCon  ( DataCon, dataConUserType, dataConOrigArgTys )
import GHC.CoreTypes.PatternSynonym   ( PatSyn, pprPatSynType )
import GHC.CoreTypes.Id               ( idType, idName )
...
*                                                                      *
    ...
    All the combinators for the monad can be found in GHC.Haskell.TypeCheck.Monad
*                                                                      *
************************************************************************
...
--
-- This data type really should be in GHC.CoreTypes.Base, but it needs
...
data TcGblEnv
  = TcGblEnv {
        ...
        tcg_type_env :: TypeEnv,
          ...
          -- Note [The interactive package] in GHC.CoreTypes.Base
        ...
        -- For the latter, see Note [The interactive package] in GHC.CoreTypes.Base
        tcg_tr_module :: Maybe Id,           -- Id for $trModule :: GHC.CoreTypes.Module
...
data PromotionErr
  ...
  | RecDataConPE     -- Data constructor in a recursive loop
                     ...
                     -- GHC.Haskell.TypeCheck.TypeDecl
...
data ImportAvails
   = ImportAvails {
        imp_mods :: ImportedMods,
          ...
          --
          -- See the documentation on ImportedModsVal in GHC.CoreTypes.Base for
...
-- GHC.Haskell.TypeCheck.Simplify uses them, and
-- GHC.Haskell.TypeCheck.Simplify is fairly low down in the
-- module hierarchy
...
It happens, too!  See Note [Polymorphic methods] in GHC.Haskell.TypeCheck.Class.
...
data Ct
  ...
  | CTyEqCan {  -- tv ~ rhs
       ...
       --     GHC.Haskell.TypeCheck.Solver.Flatten
       ...
       --     equalities] in GHC.Haskell.TypeCheck.Solver.Canonicalise
  ...
  | CFunEqCan {  -- F xis ~ fsk
      ...
      cc_fsk    :: TcTyVar  -- [Given]  always a FlatSkolTv
        ...
        -- See Note [The flattening story] in GHC.Haskell.TypeCheck.Solver.Flatten
    }

  | CNonCanonical {        -- See Note [NonCanonical Semantics] 
                           -- in GHC.Haskell.TypeCheck.Solver.Monad
      cc_ev  :: CtEvidence
    }
...
built (in GHC.Haskell.TypeCheck.Solver.Canonicalise).
...
-- set. See Note [Deterministic FV] in GHC.CoreTypes.Var.Free.
tyCoVarsOfCts :: Cts -> TcTyCoVarSet
tyCoVarsOfCts = fvVarSet . tyCoFVsOfCts

-- | Returns free variables of a bag of constraints as a deterministically
-- odered list. See Note [Deterministic FV] in GHC.CoreTypes.Var.Free.
tyCoVarsOfCtsList :: Cts -> [TcTyCoVar]
tyCoVarsOfCtsList = fvVarList . tyCoFVsOfCts

-- | Returns free variables of a bag of constraints as a composable FV
-- computation. See Note [Deterministic FV] in GHC.CoreTypes.Var.Free.
tyCoFVsOfCts :: Cts -> FV
...
-- ordered list. See Note [Deterministic FV] in GHC.CoreTypes.Var.Free.
...
*                                                                      *
                Wanted constraints
     These are forced to be in GHC.Haskell.TypeCheck.Util because
...
the implication in
GHC.Haskell.TypeCheck.Simplify.floatEqualities or
GHC.Haskell.TypeCheck.Simplify.approximateImplications
...
a coercion that is used in a type. (See the comments in Note [Flattening] in GHC.Haskell.TypeCheck.Solver.Flatten
...
data TcEvDest
  ...
  | HoleDest  CoercionHole  -- ^ fill in this hole with the evidence
              ...
              -- See Note [Coercion holes] in GHC.CoreTypes.Type.Internal
...
"shadow" [D] in GHC.Haskell.TypeCheck.Solver.Monad.maybeEmitShadow.

See Note [The improvement story and derived shadows] in
GHC.Haskell.TypeCheck.Solver.Monad
-}
...
-- Note [Flavours with roles] in GHC.Haskell.TypeCheck.Solver.Monad
...
* Then when tidying in GHC.Haskell.TypeCheck.Util.Monadic.tidySkolemInfo, we
