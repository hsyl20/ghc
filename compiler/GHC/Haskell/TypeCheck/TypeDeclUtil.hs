...
module GHC.Haskell.TypeCheck.TypeDeclUtil(
        RolesInfo,
        inferRoles,
        checkSynCycles,
        checkClassCycles,
...
import GHC.Haskell.TypeCheck.Bind ( tcRecSelBinds )
import GHC.CoreTypes.Type.Internal         ( Type(..), Coercion(..)
                                          , UnivCoProvenance(..) )
...
import GHC.CoreTypes.Name.Set hiding (unitFV)
...
import GHC.CoreTypes.Coercion                ( ltRole )
import GHC.CoreTypes.BasicTypes
import GHC.CoreTypes.SrcLoc
import GHC.CoreTypes.Unique                  ( mkBuiltinUnique )
...
[Checked in GHC.Haskell.TypeCheck.TypeDecl.checkValidTyCon]
E.g.
        data T where
          T1 { f :: Maybe a } :: T [a]
