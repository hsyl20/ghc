...
module GHC.Haskell.TypeCheck.Pattern
...
import GHC.Haskell.TypeCheck.Signature( TcPragEnv, lookupPragEnv, addInlinePrags )
...
import GHC.Haskell.TypeCheck.Validity( arityErr )
import GHC.Types.Type ( pprTyVars )
...
tcPatBndr penv@(PE { pe_ctxt = LetPat { pc_lvl    = bind_lvl
  ...
  -- Note [Typechecking pattern bindings] in GHC.Haskell.TypeCheck.Binding
...
-- See Note [Typechecking pattern bindings], item (4) in GHC.Haskell.TypeCheck.Binding
--
...
GHC.Haskell.TypeCheck.Matches.  So we tauify the pat_ty before proceeding.
...
-- 'GHC.Haskell.Rename.Splice.rnSplicePat'.
