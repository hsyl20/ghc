...
import GHC.CoreTypes.Type ( pprTyVars )
...
tcPatBndr penv@(PE { pe_ctxt = LetPat { pc_lvl    = bind_lvl
  ...
  -- Note [Typechecking pattern bindings] in GHC.Haskell.TypeCheck.Bind
...
-- See Note [Typechecking pattern bindings], item (4) in GHC.Haskell.TypeCheck.Bind
--
...
GHC.Haskell.TypeCheck.Match.  So we tauify the pat_ty before proceeding.
...
As explained by [Wrappers for data instance tycons] in GHC.CoreTypes.Id.Make, the
