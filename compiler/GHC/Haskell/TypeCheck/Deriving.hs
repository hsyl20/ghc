...
import GHC.Haskell.TypeCheck.ClassDeclaration ( instDeclCtxt3, tcATDefault, tcMkDeclCtxt)
...
import GHC.Types.Var.Free                    (fvVarList, unionFV, mkFVs)
...
tcATDefault function from TcClassDcl (which is also used in
GHC.Haskell.TypeCheck.Instance to handle the empty instance declaration case).
