...
module GHC.Haskell.TypeCheck.TypeAndClassDeclaration (
        tcTyAndClassDecls, tcAddImplicits,

        -- Functions used by GHC.Haskell.TypeCheck.Instance to check
...
import GHC.Haskell.TypeCheck.Validity
import GHC.Haskell.TypeCheck.Syntax
import GHC.Haskell.TypeCheck.TypeDeclaration
import GHC.Haskell.TypeCheck.ClassDeclaration
import {-# SOURCE #-} GHC.Haskell.TypeCheck.Instance( tcInstDecls1 )
import GHC.Haskell.TypeCheck.Deriving   ( DerivInfo )
...
import GHC.Haskell.Rename.Environment    ( lookupConstructorFields )
import GHC.Haskell.TypeCheck.FamilyInstance
import GHC.Types.FamilyInstance
import GHC.Types.Coercion
import GHC.Types.Type
import GHC.Types.TypeAndCoercion   -- for checkValidRoles
...
Then:
  ...
  * During GHC.Haskell.TypeCheck.UserType.kcTyVar we look in the *local*
    env, to get the known kind for T.

  * But in GHC.Haskell.TypeCheck.UserType.ds_type (and ds_var_app in
...
Annoying to get the error messages to refer back to GHC.Haskell. So we
  ...
  = -- Closed type families are a little tricky, because they contain the definition
    ...
    do { traceTc "Closed type family:" (ppr tc_name)
       ...
       ; branches <- mapM (tcTyFamInstEqn fam_tc_shape Nothing) eqns
         ...
         -- GHC.Haskell.TypeCheck.Validity.checkValidCoAxiom
         -- Example: tc265
         ...
         -- differentiate names.
         -- See [Zonking inside the knot] in GHC.Haskell.TypeCheck.UserType
...
tcTyFamInstEqn :: FamTyConShape -> Maybe ClsInstInfo -> LTyFamInstEqn GhcRn
               -> TcM CoAxBranch
-- Needs to be here, not in GHC.Haskell.TypeCheck.Instance, because closed
  ...
  = do { -- Kind-check and quantify
         ...
         do { (insting_subst, _leftover_binders, args, leftovers, n)
            ...
            ; case leftovers of
              ...
              -- GHC.Haskell.TypeCheck.Validity catches them
...
checkValidTyCon tc
  ...
  where
    ...
    -- See Note [GADT record selectors] in GHC.Haskell.TypeCheck.TypeDeclaration
  ...
  = do { -- strip off the duplicates and look for ill-scoped things
       ...
       ; checkValidTelescope (pprTyVars vis_tvs) stripped_tvs extra
         `and_if_that_doesn't_error`
           ...
           -- It reports errors around Note [Dependent LHsQTyVars] in GHC.Haskell.TypeCheck.UserType
         when duplicate_vars (
...
checkValidClass cls
  ...
  where
    ...
    -- Note [TyVars and TcTyVars during type checking] in
...
2. Internally, GHC uses TypeApplications to instantiate the default method in an
   ...
   GHC.Haskell.TypeCheck.Instance.  Thus, GHC needs to know exactly what
...
-- -dcore-lint is enabled. See Note [Role inference] in GHC.Haskell.TypeCheck.TypeDeclaration
    ...
    check_dc_roles datacon
      = do { traceTc "check_dc_roles" (ppr datacon <+> ppr (tyConRoles tc))
           ; mapM_ (check_ty_roles role_env Representational) $
                    ...
                    -- GHC.Haskell.TypeCheck.TypeDeclaration
      where
