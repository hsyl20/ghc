...
module GHC.Haskell.TypeCheck.UserType (
        -- Type signatures
...
import GHC.Haskell.TypeCheck.Constraint.Simplify ( solveEqualities )
import GHC.Haskell.TypeCheck.Type
import GHC.Haskell.TypeCheck.Syntax        ( zonkSigType )
import GHC.Haskell.TypeCheck.Instantiation ( tcInstBinders, tcInstBinder )
import GHC.Types.Type
import GHC.Types.Kind
import GHC.Types.RdrName                     ( lookupLocalRdrOcc )
...
with GHC.Haskell.TypeCheck.Validity.checkValidType.
...
-- 'GHC.Haskell.Rename.Splice.rnSpliceType' to communicate the splice
...
-- HsSpliced is an annotation produced by
-- 'GHC.Haskell.Rename.Splice.rnSpliceType'.
...
-- See Note [Distinguishing tuple kinds] in GHC.Haskell.Syntax.Type
...
look at the TyCon or Class involved.
  ...
  * GHC.Haskell.TypeCheck.Syntax.zonkTcTypeToType also can safely
  ...
  * It zonks any kinds.  The returned type should have no mutable kind
    or type variables (hence returning Type not TcType):
      - any unconstrained kind variables are defaulted to (Any *) just
        as in GHC.Haskell.TypeCheck.Syntax.
...
tcExplicitTKBndrsX new_tv orig_hs_tvs thing_inside
  = go orig_hs_tvs $ \ tvs ->
    do { (result, bound_tvs) <- thing_inside tvs
         ...
         -- See Note [Bad telescopes] in GHC.Haskell.TypeCheck.Validity.
...
--
-- See also Note [Associated type tyvar names] in GHC.Types.Class
--
...
-- pass in GHC.Haskell.TypeCheck.TypeAndClassDeclaration. (Never in
...
to be 'a', because when pretty printing we'll get
            class C a b where
               data D b a0
(NB: the tidying happens in the conversion to GHC.Interface.Syntax, which
 ...
 * The pattern (T p1 p2) creates a *skolem* type variable 'a_sk',
   ...
   GHC.Haskell.TypeCheck.Error.getSkolemInfo can thereby find the binding
   site for the skolem.
