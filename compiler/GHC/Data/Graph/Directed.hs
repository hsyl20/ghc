...
module GHC.Data.Graph.Directed(
        ...
        -- For backwards compatibility with the simpler version of
...
import GHC.Utils        ( minWith, count )
...
stronglyConnCompG graph = decodeSccs graph forest
  where forest = {-# SCC "GHC.Data.Graph.Directed.scc" #-}
                 scc (gr_int_graph graph)
...
topologicalSortG graph = map (gr_vertex_to_node graph) result
  where result = {-# SCC "GHC.Data.Graph.Directed.topSort" #-} 
                 topSort (gr_int_graph graph)
...
reachableG graph from = map (gr_vertex_to_node graph) result
  where from_vertex = expectJust "reachableG" (gr_node_to_vertex graph from)
        result = {-# SCC "GHC.Data.Graph.Directed.reachable" #-} 
                 reachable (gr_int_graph graph) [from_vertex]
