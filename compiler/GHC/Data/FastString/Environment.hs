...
-- explanation why it's not deterministic and why it matters. Use DFastStringEnv
-- if the set eventually gets converted into a list or folded over in a way
-- where the order changes the generated code.
...
-- See Note [Deterministic UniqFM] in GHC.Data.UniqueDFM for
-- explanation why we need DFastStringEnv.
