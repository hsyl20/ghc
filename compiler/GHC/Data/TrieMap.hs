...
See Note [Deterministic UniqFM] in GHC.Data.UniqueDFM for more
details on how it's made deterministic.
-}
...
data TypeMapX a
  = TM { tm_var    :: VarMap a
    ...
    -- Note that there is no tyconapp case; see Note [Equality on AppTys] in
    -- GHC.CoreTypes.Type

-- | Squeeze out any synonyms, and change TyConApps to nested AppTys. Why the
-- last one? See Note [Equality on AppTys] in GHC.CoreTypes.Type
--
