...
import GHC.CoreTypes.Type         ( Type, isUnliftedType )
import GHC.CoreTypes.TyCon        ( isNewTyCon, isClassTyCon )
import GHC.CoreTypes.DataCon      ( splitDataProductType_maybe )
...
-- See Note [Demand on the worker] in GHC.Core.WorkerWrap
...
we pin a demand on a binder, in GHC.Core.Demand.findBndrDmd.
...
-- Equality needed for fixpoints in GHC.Core.Demand
...
-- See Note [IO hack in the demand analyser] in GHC.Core.Demand
...
binders \pqr and \xyz; see Note [Use one-shot information] in
GHC.Core.Occurence
-}
