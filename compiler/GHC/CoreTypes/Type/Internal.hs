...
\section[GHC.CoreTypes.Type.Internal]{Type and Coercion - friends' interface}
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  GHC.Types
    .Class
    .CoAxiom
    .TypeConstructor imports Class, CoAxiom
    .TypeAndCoercion imports Class, CoAxiom, TyConstructor
  GHC.Builtin.Primitive
    .Types           imports GHC.CoreTypes.Type.Internal ( including mkTyConTy )
  GHC.Types
    .Kind     imports GHC.Builtin.Primitive.Types ( mainly for primitive kinds )
    .Type     imports Kind
...
module GHC.CoreTypes.Type.Internal (
...
import {-# SOURCE #-} GHC.CoreTypes.Coercion
import {-# SOURCE #-} GHC.CoreTypes.ConLike ( ConLike(..), conLikeName )
import {-# SOURCE #-} GHC.CoreToInterface
...
import GHC.CoreTypes.Var.Free

-- others
import GHC.CoreTypes.BasicTypes ( LeftOrRight(..), TyPrec(..), maybeParen, pickLR )
...
funTyCon and all the types in GHC.Builtin.Primitive.Types.

It is also SOURCE-imported into GHC.CoreTypes.Name
...
-- c.f. GHC.Util.PrettyPrint.TyThing.pprTyThing, which prints all the details
...
These functions are here so that they can be used by GHC.Builtin.Primitive.Types,
...
equality types story] in GHC.Builtin.Primitive.Types for background on equality
constraints.
...
-- Note [Deterministic FV] in GHC.CoreTypes.Var.Free.
...
-- See Note [FV naming conventions] in GHC.CoreTypes.Var.Free.
--
-- Eta-expanded because that makes it run faster (apparently)
-- See Note [FV eta expansion] in GHC.CoreTypes.Var.Free for explanation.
...
See Note [Precedence in types] in GHC.CoreTypes.BasicTypes.
-}
...
-- The following two functions differ from mkForAllTys and splitForAllTys in
-- that they expect/preserve the ArgFlag argument. Thes belong to
-- GHC.CoreTypes.Type, but how should they be named?
