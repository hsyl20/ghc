...
module GHC.CoreTypes.Type.Unify (
...
import GHC.CoreTypes.Type            hiding ( getTvSubstEnv )
import GHC.CoreTypes.Coercion        hiding ( getCvSubstEnv )
import GHC.CoreTypes.TyCon
import GHC.CoreTypes.Type.Internal hiding ( getTvSubstEnv, getCvSubstEnv )
import GHC.Util
...
-- second call to tcUnifyTys in
-- GHC.Haskell.TypeCheck.FunctionalDependency.checkClsFD
...
GHC.Haskell.TypeCheck.Unify, and the constraint solver itself in
GHC.Haskell.TypeCheck.Solver.Canonicalise), the pure unifier It does
/not/ work up to ~.
...
algorithm in FamInstEnv (See Note [Flattening] in GHC.CoreTypes.FamilyInstance),
