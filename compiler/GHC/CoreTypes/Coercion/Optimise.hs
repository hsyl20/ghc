...
module GHC.CoreTypes.Coercion.Optimise ( optCoercion, checkAxInstCo ) where
...
checkAxInstCo (AxiomInstCo ax ind cos)
  ...
  where
    ...
    check_no_conflict flat (b@CoAxBranch { cab_lhs = lhs_incomp } : rest)
         -- See Note [Apartness] in GHC.CoreTypes.FamilyInstance
...
etaTyConAppCo_maybe tc co
  ...
  , let n = length tys1    -- in GHC.CoreTypes.Type.Internal
  = ASSERT( tc == tc1 )
