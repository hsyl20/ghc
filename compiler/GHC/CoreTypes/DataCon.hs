...
import GHC.CoreTypes.ForeignCall ( CType )
...
data DataCon
  = MkData {
        ...
        -- Reason: less confusing, and easier to generate
 ...
 * Each argument flag is Inferred or Specified.
   ...
   Note [No Required TyBinder in terms] in GHC.CoreTypes.Type.Internal.)
...
data DataConRep
  ...
  | DCR { dcr_wrap_id :: Id   -- Takes src args, unboxes/flattens,
        ...
        , dcr_stricts :: [StrictnessMark]  -- 1-1 with dcr_arg_tys
                -- See also Note [Data-con worker strictness] in
...
data HsSrcBang =
  HsSrcBang SourceText -- Note [Pragma source text] in GHC.CoreTypes.BasicTypes
            SrcUnpackedness
            SrcStrictness
  deriving Data.Data
...
* However, if T was defined in an imported module, the importing module
  ...
  Also see Note [Bangs on imported data constructors] in GHC.CoreTypes.Id.Make
...
-- are a little dodgy.
  ...
  where
    ...
    rep_ty =
      ...
      -- See Note [Promoted data constructors] in GHC.CoreTypes.TyCon
...
buildAlgTyCon is here because it is called from GHC.Builtin.Types, which can
