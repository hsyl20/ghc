...
import GHC.CoreTypes.SrcLoc ( Located,unLoc )
...
-- instance. See Note [Safe Haskell isSafeOverlap] (in GHC.CoreTypes.Instance)
...
-- See Note [Rules for instance lookup] in GHC.CoreTypes.Instance
data OverlapMode
  ...
  | Incoherent SourceText
    ...
    -- was chosen. See also Note [Incoherent instances] in
...
data TyPrec   -- See Note [Precedence in types] in GHC.CoreTypes.Type.Internal
  = TopPrec         -- No parens
...
GHC.HaskellToCore.Bind.makeCorePair
...
See Note [Rules and inlining/other rules] in GHC.HaskellToCore.
...
-- for compiling pattern-matching (module GHC.HaskellToCore.Match.Literal)
