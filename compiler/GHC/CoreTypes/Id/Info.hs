...
data IdDetails
  ...
  | RecSelId
    ...
    }                           -- See Note [Naughty record selectors] in GHC.Haskell.TypeCheck.TypeDecl
...
In GHC.Interface.Tidy, when the LocalId becomes a GlobalId, its RULES are stripped off
...
isExprLevPoly in GHC.Core.Util for where this info is used. Storing
