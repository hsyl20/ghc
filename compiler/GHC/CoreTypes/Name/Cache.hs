...
-- | The Name Cache
module GHC.CoreTypes.Name.Cache
    ( lookupOrigNameCache
    , extendOrigNameCache
    , extendNameCache
    , initNameCache
...
their cost we use two tricks,
  ...
  a. We specially encode tuple and sum Names in interface files' symbol tables
     ...
     in GHC.Interface.Binary and for details.
...
are two reasons why we might look up an Orig RdrName for built-in syntax,
  ...
  * Template Haskell turns a BuiltInSyntax Name into a TH.NameG
    (GHC.HaskellToCore.Splice.globalVar), and parses a NameG into an
    Orig RdrName (GHC.Haskell.Template.thRdrName).
