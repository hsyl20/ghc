...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The Integer type is special because GHC.Inteface.Tidy uses
...
benefit, as described in GHC.Compilers.HaskellToCore.ListComp: if optimizations needed to make
foldr/build forms efficient are turned off, we'll get reasonably efficient
translations anyway.
-}
...
-- eqString also has a BuiltInRule in GHC.IR.Core.Transform.ConstantFolding.hs:
...
-- There's a built-in rule (in GHC.IR.Core.Transform.ConstantFolding.hs) for
