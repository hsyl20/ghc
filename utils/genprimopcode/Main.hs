...
gen_hs_source (Info defaults entries) =
    ...
    ++ "default ()"  -- If we don't say this then the default type include Integer
                     ...
                     -- coming from GHC.IR.Interface.Load.homeModError
...
with an ad-hoc fix in GHC.IR.Haskell.TypeSystem.Binding: see Note [Compiling GHC.Prim] in GHC.IR.Haskell.TypeSystem.Binding.
...
ppType (TyApp (TyCon "()")          []) = "unitTy"      -- unitTy is GHC.Builtin.Type's name for ()
