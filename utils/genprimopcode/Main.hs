...
gen_hs_source (Info defaults entries) =
    ...
    ++ "default ()"  -- If we don't say this then the default type include Integer
                     ...
                     -- coming from GHC.IR.Interface.Loader.homeModError
...
with an ad-hoc fix in GHC.IR.Haskell.TypeChecker.Binding: see Note [Compiling
GHC.Prim] in GHC.IR.Haskell.TypeChecker.Binding.
